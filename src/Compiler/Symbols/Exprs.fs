...
module ExprTranslationImpl =
    ...
    let wfail (msg, m: range) =
        failwith (msg + sprintf " at %s" (m.ToString()))
...
and [<Sealed>] FSharpObjectExprOverride
    (sgn: FSharpAbstractSignature, gps: FSharpGenericParameter list, args: FSharpMemberOrFunctionOrValue list list, body: FSharpExpr) =
...
and [<Sealed>] FSharpExpr(cenv, f: (unit -> FSharpExpr) option, e: E, m: range, ty) =
    ...
    member x.ImmediateSubExpressions =
        ...
        | E.Call(objOpt, _b, _c, _d, ws, es) ->
            (match objOpt with
             | None -> ws @ es
             | Some x -> x :: ws @ es)
        | E.NewObject(_a, _b, c) -> c
        | E.FSharpFieldGet(objOpt, _b, _c) ->
            (match objOpt with
             | None -> []
             | Some x -> [ x ])
        | E.FSharpFieldSet(objOpt, _b, _c, d) ->
            (match objOpt with
             | None -> [ d ]
             | Some x -> [ x; d ])
        ...
        | E.ILFieldGet(objOpt, _ty, _fieldName) ->
            (match objOpt with
             | None -> []
             | Some x -> [ x ])
        | E.ILFieldSet(objOpt, _ty, _fieldName, d) ->
            (match objOpt with
             | None -> [ d ]
             | Some x -> [ x; d ])
        | E.ObjectExpr(_ty, basecall, overrides, interfaceImpls) ->
            [
                yield basecall
                for m in overrides do
                    yield m.Body
                for _, ms in interfaceImpls do
                    for m in ms do
                        yield m.Body
            ]
        | E.DecisionTree(inputExpr, targetCases) ->
            [
                yield inputExpr
                for _targetVars, targetExpr in targetCases do
                    yield targetExpr
            ]
...
module FSharpExprConvert =
    ...
    let (|StaticInitializationCheck|_|) e =
        ...
        | Expr.Match(_,
                     _,
                     TDSwitch(Expr.Op(TOp.ILAsm([ AI_clt ], _), _, [ Expr.Op(TOp.ValFieldGet rfref, _, _, _); _ ], _), _, _, _),
                     _,
                     _,
                     _) when IsStaticInitializationField rfref -> Some()
        | _ -> None
    ...
    let rec exprOfExprAddr (cenv: SymbolEnv) expr =
        ...
        | Expr.Op(op, tyargs, args, m) ->
            ...
            | TOp.UnionCaseFieldGetAddr(uref, n, _), [ arg ], _ ->
                mkUnionCaseFieldGetProvenViaExprAddr (exprOfExprAddr cenv arg, uref, tyargs, n, m)
            | TOp.ILAsm([ I_ldflda fspec ], retTypes), [ arg ], _ ->
                mkAsmExpr ([ mkNormalLdfld fspec ], tyargs, [ exprOfExprAddr cenv arg ], retTypes, m)
    ...
    and ConvExprPrimLinear cenv env expr contF =
        ...
        | Expr.Sequential(StaticInitializationCheck, x1, NormalSeq, _) -> ConvExprPrim cenv env x1 |> contF
        ...
        | Expr.Sequential(x0, x1, ThenDoSeq, _) -> E.Sequential(ConvExpr cenv env x0, ConvExpr cenv env x1) |> contF

        | ModuleValueOrMemberUse g (vref, vFlags, _f, _fty, tyargs, curriedArgs) when
            (nonNil tyargs || nonNil curriedArgs) && vref.IsMemberOrModuleBinding
            ->
        ...
        | _ -> ConvExprPrim cenv env expr |> contF
    ...
    and ConvModuleValueOrMemberUseLinear (cenv: SymbolEnv) env (expr: Expr, vref, vFlags, tyargs, curriedArgs) contF =
        ...
        let objArgs, curriedArgs =
            ...
            | true, [] ->
                failwith (
                    "warning: unexpected missing object argument when generating quotation for call to F# object member "
                    + vref.LogicalName
                )
        ...
        if
            curriedArgs.Length < curriedArgInfos.Length
            || ((List.truncate curriedArgInfos.Length curriedArgs, curriedArgInfos)
                ||> List.exists2 (fun arg argInfo -> (argInfo.Length > (tryDestRefTupleExpr arg).Length)))
        then
            ...
            let splitCallExpr =
                MakeApplicationAndBetaReduce g (expr, exprTy, [ tyargs ], curriedArgs, m)
            // tailcall
            ...
            let untupledCurriedArgs =
                (curriedArgs, curriedArgInfos)
                ||> List.map2 (fun arg curriedArgInfo ->
            ...
            let contf2 =
                ...
                | _ ->
                    (fun subCallR ->
                        (subCallR, laterArgs)
                        ||> List.fold (fun fR arg -> E.Application(Mk2 cenv arg fR, [], [ ConvExpr cenv env arg ]))
                        |> contF)
            ...
            if isMember then
                ...
                // tailcall
                ConvObjectModelCallLinear
                    cenv
                    env
                    (isNewObj, FSharpMemberOrFunctionOrValue(cenv, vref), enclTyArgs, methTyArgs, witnessArgsR, callArgs)
                    contf2
            else
    ...
    and GetWitnessArgs cenv (env: ExprTranslationEnv) (vref: ValRef) m tps tyargs : FSharpExpr list =
        ...
        if
            g.langVersion.SupportsFeature(Features.LanguageFeature.WitnessPassing)
            && not env.suppressWitnesses
        then
            let witnessExprs =
            ...
            |> List.map (fun arg ->
                match arg with
                | Choice1Of2 traitInfo -> ConvWitnessInfo cenv env m traitInfo
    ...
    and ConvExprPrim (cenv: SymbolEnv) (env: ExprTranslationEnv) expr =
        ...
        | Expr.App(InnerExprPat(Expr.Val _ as ve), _fty, [], [], _) -> ConvExprPrim cenv env ve
        ...
        | Expr.Sequential _ -> ConvExprPrimLinear cenv env expr id

        | ModuleValueOrMemberUse g (vref, vFlags, _f, _fty, tyargs, curriedArgs) (* (nonNil tyargs || nonNil curriedArgs) && *) when
            vref.IsMemberOrModuleBinding
            ->
        ...
        | Expr.Val(vref, _vFlags, m) -> ConvValRef cenv env m vref

        // Simple applications
        | Expr.App(f, _fty, tyargs, args, _m) -> E.Application(ConvExpr cenv env f, ConvTypes cenv tyargs, ConvExprs cenv env args)

        | Expr.Const(c, m, ty) -> ConvConst cenv env m c ty
        ...
        | Expr.LetRec(binds, body, _, _) ->
            ...
            let bindsR =
                List.zip3 vsR (binds |> List.map (fun b -> b.Expr |> ConvExpr cenv env)) dps
        ...
        | Expr.Quote(ast, _, _, _, _) -> E.Quote(ConvExpr cenv env ast)
        ...
        | Expr.Obj(_, ty, _, _, [ TObjExprMethod(TSlotSig(_, ctyp, _, _, _, _), _, tps, [ tmvs ], e, _) as tmethod ], _, m) when
            isDelegateTy g ty
            ->
            let f =
                mkLambdas g m tps tmvs (e, GetFSharpViewOfReturnType g (returnTyOfMethod g tmethod))

            let fR = ConvExpr cenv env f
            let tyargR = ConvType cenv ctyp
            E.NewDelegate(tyargR, fR)

        | Expr.StaticOptimization(_, _, x, _) -> ConvExprPrim cenv env x

        | Expr.TyChoose _ -> ConvExprPrim cenv env (ChooseTyparSolutionsForFreeChoiceTypars g cenv.amap expr)
        ...
        | Expr.Obj(_lambdaId, ty, _basev, basecall, overrides, iimpls, _m) ->
            ...
            let iimplsR =
                iimpls
                |> List.map (fun (intfTy, impls) -> ConvType cenv intfTy, ConvertMethods impls)
        ...
        | Expr.Op(op, tyargs, args, m) ->
            ...
            | TOp.UnionCaseFieldGetAddr _, _tyargs, _ -> E.AddressOf(ConvLValueExpr cenv env expr)

            | TOp.ValFieldGetAddr _, _tyargs, _ -> E.AddressOf(ConvLValueExpr cenv env expr)
            ...
            | TOp.ILAsm([ I_call(Normalcall, mspec, None) ], _), _, [ arg ] when
                mspec.MethodRef.DeclaringTypeRef.Name = "System.String"
                && mspec.Name = "GetHashCode"
                ->
                let ty = tyOfExpr g arg
            ...
            | TOp.ILCall(_, _, _, _, _, _, _, ilMethRef, _, _, _),
              [],
              [ Expr.Op(TOp.ILAsm([ I_ldtoken(ILToken.ILType _) ], _), [ ty ], _, _) ] when
                ilMethRef.DeclaringTypeRef.Name = "System.Type"
                && ilMethRef.Name = "GetTypeFromHandle"
                ->
                let op = mkCallTypeOf g m ty
                ConvExprPrim cenv env op

            | TOp.ILAsm([ EI_ilzero _ ], _), [ ty ], _ -> E.DefaultValue(ConvType cenv ty)
            ...
            | TOp.ILAsm([ ILMulDivOp(binaryOp, isMul) ], _), _, [ arg1; arg2 ] ->
                ...
                let resTy =
                    ...
                    | Some(tcref, ms1), Some(_tcref2, ms2) ->
                        mkWoNullAppTy
                            tcref
                            [
                                TType_measure(Measure.Prod(ms1, (if isMul then ms2 else Measure.Inv ms2), unionRanges ms1.Range ms2.Range))
                            ]
                    | Some _, None -> argTy1
                    | None, Some _ -> argTy2
                    | None, None -> argTy1
            ...
            | TOp.ILAsm(instrs, _), tyargs, args -> E.ILAsm(sprintf "%+A" instrs, ConvTypes cenv tyargs, ConvExprs cenv env args)

            | TOp.ExnConstr tcref, tyargs, args -> E.NewRecord(ConvType cenv (mkWoNullAppTy tcref tyargs), ConvExprs cenv env args)
            ...
            | TOp.ExnFieldGet(tcref, i), [], [ obj ] ->
                ...
                let objR =
                    ConvExpr cenv env (mkCoerceExpr (obj, mkWoNullAppTy tcref [], m, g.exn_ty))
            ...
            | TOp.LValueOp(LAddrOf _, vref), [], [] -> E.AddressOf(ConvExpr cenv env (exprForValRef m vref))

            | TOp.LValueOp(LByrefSet, vref), [], [ e ] -> E.AddressSet(ConvExpr cenv env (exprForValRef m vref), ConvExpr cenv env e)

            | TOp.LValueOp(LSet, vref), [], [ e ] -> E.ValueSet(FSharpMemberOrFunctionOrValue(cenv, vref), ConvExpr cenv env e)

            | TOp.LValueOp(LByrefGet, vref), [], [] -> ConvValRef cenv env m vref

            | TOp.Array, [ ty ], xa -> E.NewArray(ConvType cenv ty, ConvExprs cenv env xa)
            ...
            | TOp.IntegerForLoop(dpFor, dpEquals, dir),
              [],
              [ Expr.Lambda(_, _, _, [ _ ], lim0, _, _)
                Expr.Lambda(_, _, _, [ _ ], SimpleArrayLoopUpperBound, lm, _)
                SimpleArrayLoopBody g (arr, elemTy, body) ] ->
                let lim1 =
                    let len = mkCallArrayLength g lm elemTy arr // Array.length arr
                    mkCallSubtractionOperator g lm g.int32_ty len (mkOne g lm) // len - 1

                E.IntegerForLoop(
                    ConvExpr cenv env lim0,
                    ConvExpr cenv env lim1,
                    ConvExpr cenv env body,
                    dir <> FSharpForLoopDown,
                    dpFor,
                    dpEquals
                )

            | TOp.IntegerForLoop(doFor, doEquals, dir),
              [],
              [ Expr.Lambda(_, _, _, [ _ ], lim0, _, _); Expr.Lambda(_, _, _, [ _ ], lim1, lm, _); body ] ->
                let lim1 =
            ...
            | TOp.RefAddrGet readonly, [ ty ], [ e ] ->
                let replExpr =
                    mkRecdFieldGetAddrViaExprAddr (readonly, e, mkRefCellContentsRef g, [ ty ], m)

                ConvExprPrim cenv env replExpr

            | _ -> wfail (sprintf "unhandled construct in AST", m)

        | Expr.WitnessArg(traitInfo, _m) -> ConvWitnessInfoPrim env traitInfo

        | Expr.DebugPoint(_, innerExpr) -> ConvExprPrim cenv env innerExpr

        | _ -> wfail (sprintf "unhandled construct in AST", expr.Range)
    ...
    and ConvLetBind cenv env (bind: Binding) =
        ...
        | Expr.Op(TOp.ILAsm([ I_isinst _ ], _), [ ty ], [ e ], _) -> None, env.BindIsInstVal bind.Var (ty, e)

        // Remove let <compilerGeneratedVar> = <var> from quotation tree
        | Expr.Val _ when bind.Var.IsCompilerGenerated && (not bind.Var.IsMutable) -> None, env.BindSubstVal bind.Var bind.Expr
        ...
        | Expr.Op(TOp.UnionCaseProof _, _, [ e ], _) when (not bind.Var.IsMutable) -> None, env.BindSubstVal bind.Var e
    ...
    and ConvILCall (cenv: SymbolEnv) env (isNewObj, valUseFlags, ilMethRef, enclTypeArgs, methTypeArgs, callArgs, m) =
        ...
        let makeFSExpr isMember (vr: ValRef) =
            ...
            let findByName =
                ...
                |> List.filter (fun v -> (v.CompiledName g.CompilerGlobalState) = vName)

            match findByName with
            | [ v ] -> makeFSCall isMember v
            | [] ->
                ...
                if enclosingEntity.IsModuleOrNamespace then
                    ...
                    if vName = "GetTag" || vName = "get_Tag" then
                        ...
                        | _ -> failwith "Failed to recognize union type member"
                else
                    let names =
                        enclosingEntity.MembersOfFSharpTyconSorted
            ...
            | _ -> // member is overloaded
                ...
                | Some keyTy ->
                    ...
                    | Some v -> makeFSCall isMember v
                    | _ -> failwith "Failed to recognize F# member"
        ...
        let try1 =
            if tcref.IsILTycon then
                try
                    let mdef =
                        resolveILMethodRefWithRescope unscopeILType tcref.ILTyconRawMetadata ilMethRef
        ...
        | None ->
            try
                ...
                let memberParentName =
                    if tcref.IsModuleOrNamespace then
                        None
                    else
                        Some tcref.LogicalName
                ...
                if isMember then
                    ...
                    | _ ->
                        ...
                        let typars2 =
                            [ 1 .. ilMethRef.GenericArity ]
                            |> List.map (fun _ -> Construct.NewRigidTypar "T" m)
                        ...
                        let argTys =
                            [
                                ilMethRef.ArgTypes
                                |> List.map (ImportILTypeFromMetadataSkipNullness cenv.amap m scoref tinst1 tinst2)
                            ]
    ...
    and ConvObjectModelCallLinear
        cenv
        env
        (isNewObj, v: FSharpMemberOrFunctionOrValue, enclTyArgs, methTyArgs, witnessArgsR: FSharpExpr list, callArgs)
        contF
        =
    ...
    and ConvTargetsLinear cenv env tgs contF =
        ...
        | TTarget(vars, rhs, _) :: rest ->
            ...
            ConvExprLinear cenv env rhs (fun targetR -> ConvTargetsLinear cenv env rest (fun restR -> contF ((varsR, targetR) :: restR)))
    ...
    and ConvDecisionTreePrim cenv env dtreeRetTy x =
        ...
        | TDSwitch(inpExpr, csl, dfltOpt, m) ->
            ...
            (csl, acc)
            ||> List.foldBack (ConvDecisionTreeCase (cenv: SymbolEnv) env m inpExpr dtreeRetTy)
...
type FSharpAssemblyContents(cenv: SymbolEnv, mimpls: CheckedImplFile list) =
    ...
    new(tcGlobals, thisCcu, thisCcuType, tcImports, mimpls) =
        FSharpAssemblyContents(SymbolEnv(tcGlobals, thisCcu, thisCcuType, tcImports), mimpls)
...
and FSharpImplementationFileContents(cenv, mimpl) =
    ...
    let (CheckedImplFile(qname, _, contents, hasExplicitEntryPoint, isScript, _anonRecdTypes, _)) =
        mimpl
    ...
    let rec getBind (bind: Binding) =
        ...
        let valReprInfo =
            InferValReprInfoOfBinding g AllowTypeDirectedDetupling.Yes v bind.Expr

        let tps, _ctorThisValOpt, _baseValOpt, vsl, body, _bodyty =
            IteratedAdjustLambdaToMatchValReprInfo g cenv.amap valReprInfo bind.Expr
        ...
        | TMDefRec(_isRec, _opens, tycons, mbinds, _m) ->
            [
                ...
                for mbind in mbinds do
                    ...
                    | ModuleOrNamespaceBinding.Binding bind -> yield getBind bind
            ]
        | TMDefLet(bind, _m) -> [ yield getBind bind ]
        | TMDefOpens _ -> []
        | TMDefDo(expr, _m) ->
            [
                let expr =
                    FSharpExprConvert.ConvExprOnDemand cenv (ExprTranslationEnv.Empty(g)) expr
...
module FSharpExprPatterns =
    ...
    let (|Const|_|) (e: FSharpExpr) =
        match e.E with
        | E.Const(v, ty) -> Some(v, ty)
        | _ -> None

    let (|TypeLambda|_|) (e: FSharpExpr) =
        match e.E with
        | E.TypeLambda(v, e) -> Some(v, e)
        | _ -> None

    let (|Lambda|_|) (e: FSharpExpr) =
        match e.E with
        | E.Lambda(v, e) -> Some(v, e)
        | _ -> None

    let (|Application|_|) (e: FSharpExpr) =
        match e.E with
        | E.Application(f, tys, e) -> Some(f, tys, e)
        | _ -> None

    let (|IfThenElse|_|) (e: FSharpExpr) =
        match e.E with
        | E.IfThenElse(e1, e2, e3) -> Some(e1, e2, e3)
        | _ -> None

    let (|Let|_|) (e: FSharpExpr) =
        match e.E with
        | E.Let((dp, v, e), b) -> Some((dp, v, e), b)
        | _ -> None

    let (|LetRec|_|) (e: FSharpExpr) =
        match e.E with
        | E.LetRec(ves, b) -> Some(ves, b)
        | _ -> None

    let (|NewRecord|_|) (e: FSharpExpr) =
        match e.E with
        | E.NewRecord(ty, es) -> Some(ty, es)
        | _ -> None

    let (|NewAnonRecord|_|) (e: FSharpExpr) =
        match e.E with
        | E.NewAnonRecord(ty, es) -> Some(ty, es)
        | _ -> None

    let (|NewUnionCase|_|) (e: FSharpExpr) =
        match e.E with
        | E.NewUnionCase(e, tys, es) -> Some(e, tys, es)
        | _ -> None

    let (|NewTuple|_|) (e: FSharpExpr) =
        match e.E with
        | E.NewTuple(ty, es) -> Some(ty, es)
        | _ -> None

    let (|TupleGet|_|) (e: FSharpExpr) =
        match e.E with
        | E.TupleGet(ty, n, es) -> Some(ty, n, es)
        | _ -> None

    let (|Call|_|) (e: FSharpExpr) =
        match e.E with
        | E.Call(a, b, c, d, _e, f) -> Some(a, b, c, d, f)
        | _ -> None

    let (|CallWithWitnesses|_|) (e: FSharpExpr) =
        match e.E with
        | E.Call(a, b, c, d, e, f) -> Some(a, b, c, d, e, f)
        | _ -> None

    let (|NewObject|_|) (e: FSharpExpr) =
        match e.E with
        | E.NewObject(a, b, c) -> Some(a, b, c)
        | _ -> None

    let (|FSharpFieldGet|_|) (e: FSharpExpr) =
        match e.E with
        | E.FSharpFieldGet(a, b, c) -> Some(a, b, c)
        | _ -> None

    let (|AnonRecordGet|_|) (e: FSharpExpr) =
        match e.E with
        | E.AnonRecordGet(a, b, c) -> Some(a, b, c)
        | _ -> None

    let (|FSharpFieldSet|_|) (e: FSharpExpr) =
        match e.E with
        | E.FSharpFieldSet(a, b, c, d) -> Some(a, b, c, d)
        | _ -> None

    let (|UnionCaseGet|_|) (e: FSharpExpr) =
        match e.E with
        | E.UnionCaseGet(a, b, c, d) -> Some(a, b, c, d)
        | _ -> None

    let (|UnionCaseTag|_|) (e: FSharpExpr) =
        match e.E with
        | E.UnionCaseTag(a, b) -> Some(a, b)
        | _ -> None

    let (|UnionCaseTest|_|) (e: FSharpExpr) =
        match e.E with
        | E.UnionCaseTest(a, b, c) -> Some(a, b, c)
        | _ -> None

    let (|NewArray|_|) (e: FSharpExpr) =
        match e.E with
        | E.NewArray(a, b) -> Some(a, b)
        | _ -> None

    let (|Coerce|_|) (e: FSharpExpr) =
        match e.E with
        | E.Coerce(a, b) -> Some(a, b)
        | _ -> None

    let (|Quote|_|) (e: FSharpExpr) =
        match e.E with
        | E.Quote a -> Some a
        | _ -> None

    let (|TypeTest|_|) (e: FSharpExpr) =
        match e.E with
        | E.TypeTest(a, b) -> Some(a, b)
        | _ -> None

    let (|Sequential|_|) (e: FSharpExpr) =
        match e.E with
        | E.Sequential(dp, a) -> Some(dp, a)
        | _ -> None

    let (|DebugPoint|_|) (e: FSharpExpr) =
        match e.E with
        | E.DebugPoint(dp, a) -> Some(dp, a)
        | _ -> None

    let (|FastIntegerForLoop|_|) (e: FSharpExpr) =
        match e.E with
        | E.IntegerForLoop(dpFor, dpEquals, a, b, c, d) -> Some(dpFor, dpEquals, a, b, c, d)
        | _ -> None

    let (|WhileLoop|_|) (e: FSharpExpr) =
        match e.E with
        | E.WhileLoop(dpWhile, a, b) -> Some(dpWhile, a, b)
        | _ -> None

    let (|TryFinally|_|) (e: FSharpExpr) =
        match e.E with
        | E.TryFinally(dpTry, dpFinally, a, b) -> Some(dpTry, dpFinally, a, b)
        | _ -> None

    let (|TryWith|_|) (e: FSharpExpr) =
        match e.E with
        | E.TryWith(dpTry, dpWith, a, b, c, d, e) -> Some(dpTry, dpWith, a, b, c, d, e)
        | _ -> None

    let (|NewDelegate|_|) (e: FSharpExpr) =
        match e.E with
        | E.NewDelegate(ty, e) -> Some(ty, e)
        | _ -> None

    let (|DefaultValue|_|) (e: FSharpExpr) =
        match e.E with
        | E.DefaultValue ty -> Some ty
        | _ -> None

    let (|AddressSet|_|) (e: FSharpExpr) =
        match e.E with
        | E.AddressSet(a, b) -> Some(a, b)
        | _ -> None

    let (|ValueSet|_|) (e: FSharpExpr) =
        match e.E with
        | E.ValueSet(a, b) -> Some(a, b)
        | _ -> None

    let (|AddressOf|_|) (e: FSharpExpr) =
        match e.E with
        | E.AddressOf a -> Some a
        | _ -> None

    let (|ThisValue|_|) (e: FSharpExpr) =
        match e.E with
        | E.ThisValue a -> Some a
        | _ -> None

    let (|BaseValue|_|) (e: FSharpExpr) =
        match e.E with
        | E.BaseValue a -> Some a
        | _ -> None

    let (|ILAsm|_|) (e: FSharpExpr) =
        match e.E with
        | E.ILAsm(a, b, c) -> Some(a, b, c)
        | _ -> None

    let (|ILFieldGet|_|) (e: FSharpExpr) =
        match e.E with
        | E.ILFieldGet(a, b, c) -> Some(a, b, c)
        | _ -> None

    let (|ILFieldSet|_|) (e: FSharpExpr) =
        match e.E with
        | E.ILFieldSet(a, b, c, d) -> Some(a, b, c, d)
        | _ -> None

    let (|ObjectExpr|_|) (e: FSharpExpr) =
        match e.E with
        | E.ObjectExpr(a, b, c, d) -> Some(a, b, c, d)
        | _ -> None

    let (|DecisionTree|_|) (e: FSharpExpr) =
        match e.E with
        | E.DecisionTree(a, b) -> Some(a, b)
        | _ -> None

    let (|DecisionTreeSuccess|_|) (e: FSharpExpr) =
        match e.E with
        | E.DecisionTreeSuccess(a, b) -> Some(a, b)
        | _ -> None

    let (|UnionCaseSet|_|) (e: FSharpExpr) =
        match e.E with
        | E.UnionCaseSet(a, b, c, d, e) -> Some(a, b, c, d, e)
        | _ -> None

    let (|TraitCall|_|) (e: FSharpExpr) =
        match e.E with
        | E.TraitCall(a, b, c, d, e, f) -> Some(a, b, c, d, e, f)
        | _ -> None

    let (|WitnessArg|_|) (e: FSharpExpr) =
        match e.E with
        | E.WitnessArg n -> Some n
        | _ -> None
