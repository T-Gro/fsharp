...
module Impl =
    let protect f =
        DiagnosticsLogger.protectAssemblyExplorationF
            (fun (asmName, path) ->
                invalidOp (
                    sprintf
                        "The entity or value '%s' does not exist or is in an unresolved assembly. You may need to add a reference to assembly '%s'"
                        path
                        asmName
                ))
            f
    ...
    let makeXmlDoc (doc: XmlDoc) = FSharpXmlDoc.FromXmlText doc
    ...
    let entityIsUnresolved (entity: EntityRef) =
        match entity with
        | ERefNonLocal(NonLocalEntityRef(ccu, _)) -> ccu.IsUnresolvedReference && entity.TryDeref.IsNone
        | _ -> false
    ...
    let checkForCrossProjectAccessibility (ilg: ILGlobals) (thisCcu2: CcuThunk, ad2) (thisCcu1, taccess1) =
        ...
        | AccessibleFrom(cpaths2, _) ->
            ...
            let canAccessFromCrossProject (TAccess x1) cpath2 =
                x1
                |> List.forall (fun cpath1 -> canAccessCompPathFromCrossProject cpath1 cpath2)
    ...
    let getApproxFSharpAccessibilityOfMember (declaringEntity: EntityRef) (ilAccess: ILMemberAccess) =
        ...
        | ILMemberAccess.Assembly -> taccessPrivate (CompPath(declaringEntity.CompilationPath.ILScopeRef, SyntaxAccess.Unknown, []))

        | ILMemberAccess.Private -> taccessPrivate declaringEntity.CompilationPath
    ...
    let getApproxFSharpAccessibilityOfEntity (entity: EntityRef) =
        ...
        | FSharpOrArrayOrByrefOrTupleOrExnTypeMetadata -> entity.Accessibility
...
type FSharpDisplayContext(denv: TcGlobals -> DisplayEnv) =
    ...
    member _.WithShortTypeNames shortNames =
        FSharpDisplayContext(fun g ->
            { denv g with
                shortTypeNames = shortNames
            })

    member _.WithPrefixGenericParameters() =
        FSharpDisplayContext(fun g ->
            { denv g with
                genericParameterStyle = GenericParameterStyle.Prefix
            })

    member _.WithSuffixGenericParameters() =
        FSharpDisplayContext(fun g ->
            { denv g with
                genericParameterStyle = GenericParameterStyle.Suffix
            })
...
type FSharpSymbol(cenv: SymbolEnv, item: unit -> Item, access: FSharpSymbol -> CcuThunk -> AccessorDomain -> bool) =
    ...
    member x.GetEffectivelySameAsHash() =
        ItemsAreEffectivelyEqualHash cenv.g x.Item
    ...
    static member Create(cenv, item) : FSharpSymbol =
        ...
        | Item.Event einfo -> FSharpMemberOrFunctionOrValue(cenv, E einfo, item) :> _

        | Item.Property(info = pinfo :: _) -> FSharpMemberOrFunctionOrValue(cenv, P pinfo, item) :> _

        | Item.MethodGroup(_, minfo :: _, _) -> FSharpMemberOrFunctionOrValue(cenv, M minfo, item) :> _

        | Item.CtorGroup(_, cinfo :: _) -> FSharpMemberOrFunctionOrValue(cenv, C cinfo, item) :> _

        | Item.DelegateCtor(AbbrevOrAppTy(tcref, tyargs))
        | Item.Types(_, AbbrevOrAppTy(tcref, tyargs) :: _) -> FSharpEntity(cenv, tcref, tyargs) :> _

        | Item.UnqualifiedType(tcref :: _) -> FSharpEntity(cenv, tcref) :> _

        | Item.ModuleOrNamespaces(modref :: _) -> FSharpEntity(cenv, modref) :> _

        | Item.SetterArg(_id, item) -> FSharpSymbol.Create(cenv, item)

        | Item.CustomOperation(_customOpName, _, Some minfo) -> FSharpMemberOrFunctionOrValue(cenv, M minfo, item) :> _

        | Item.CustomBuilder(_, vref) -> FSharpMemberOrFunctionOrValue(cenv, V vref, item) :> _

        | Item.TypeVar(_, tp) -> FSharpGenericParameter(cenv, tp) :> _

        | Item.Trait traitInfo -> FSharpGenericParameterMemberConstraint(cenv, traitInfo) :> _

        | Item.ActivePatternCase apref ->
            FSharpActivePatternCase(
                cenv,
                apref.ActivePatternInfo,
                apref.ActivePatternVal.Type,
                apref.CaseIndex,
                Some apref.ActivePatternVal,
                item
            )
            :> _

        | Item.ActivePatternResult(apinfo, ty, n, _) -> FSharpActivePatternCase(cenv, apinfo, ty, n, None, item) :> _

        | Item.OtherName(id, ty, _, argOwner, m) -> FSharpParameter(cenv, id, ty, argOwner, m) :> _

        | Item.ImplicitOp(_,
                          {
                              contents = Some(TraitConstraintSln.FSMethSln(vref = vref))
...
type FSharpEntity(cenv: SymbolEnv, entity: EntityRef, tyargs: TType list) =
        ...
        FSharpSymbol(
            ...
            (fun () ->
                ...
                if entity.IsModuleOrNamespace then
                    Item.ModuleOrNamespaces [ entity ]
                elif entity.IsFSharpException then
                    Item.ExnCase entity
                else
                    Item.UnqualifiedType [ entity ]),
    ...
    member _.Namespace =
        ...
        | Some cp when
            ...
            |> List.forall (function
                | _, ModuleOrNamespaceKind.Namespace _ -> true
                | _ -> false)
            ->
...
#if !NO_TYPEPROVIDERS
        if entity.IsTypeAbbrev || entity.IsProvidedErasedTycon || entity.IsNamespace then
            fail ()
#else
        if entity.IsTypeAbbrev || entity.IsNamespace then
            fail ()
#endif
    ...
    member x.QualifiedName = x.CompiledRepresentation.QualifiedName

    member x.BasicQualifiedName = x.CompiledRepresentation.BasicQualifiedName
    ...
    member _.TryFullName =
        if isUnresolved () then
            None
#if !NO_TYPEPROVIDERS
        elif entity.IsTypeAbbrev || entity.IsProvidedErasedTycon then
            None
#else
        elif entity.IsTypeAbbrev then
            None
#endif
        elif entity.IsNamespace then
            Some entity.DemangledModuleOrNamespaceName
        else
            match entity.CompiledRepresentation with
    ...
    member _.IsAbstractClass = isResolved () && isAbstractTycon entity.Deref

    member _.IsFSharpModule = isResolvedAndFSharp () && entity.IsModule
    ...
    member _.IsValueType = isResolved () && entity.IsStructOrEnumTycon

    member _.IsArrayType = isResolved () && isArrayTyconRef cenv.g entity
...
#if !NO_TYPEPROVIDERS
    member _.IsProvided = isResolved () && entity.IsProvided

    member _.IsProvidedAndErased = isResolved () && entity.IsProvidedErasedTycon

    member _.IsStaticInstantiation = isResolved () && entity.IsStaticInstantiationTycon

    member _.IsProvidedAndGenerated = isResolved () && entity.IsProvidedGeneratedTycon
#endif
    member _.IsClass =
        isResolved ()
        && match metadataOfTycon entity.Deref with
    ...
    member _.IsByRef = isResolved () && isByrefTyconRef cenv.g entity

    member _.IsOpaque = isResolved () && entity.IsHiddenReprTycon

    member _.IsInterface = isResolved () && isInterfaceTyconRef entity
    ...
    member _.IsEnum = isResolved () && entity.IsEnumTycon
    ...
    member _.IsFSharp = isResolvedAndFSharp ()

    member _.IsFSharpAbbreviation = isResolvedAndFSharp () && entity.IsTypeAbbrev

    member _.IsFSharpRecord = isResolvedAndFSharp () && entity.IsRecordTycon

    member _.IsFSharpUnion = isResolvedAndFSharp () && entity.IsUnionTycon

    member _.HasAssemblyCodeRepresentation =
        isResolvedAndFSharp ()
        && (entity.IsAsmReprTycon || entity.IsMeasureableReprTycon)
    ...
    override _.Accessibility =
        if isUnresolved () then
            FSharpAccessibility taccessPublic
        else
    ...
    member _.IsAttributeType =
        if isUnresolved () then
            ...
            DiagnosticsLogger.protectAssemblyExploration false
            <| fun () -> ExistsHeadTypeInEntireHierarchy cenv.g cenv.amap range0 ty cenv.g.tcref_System_Attribute
    ...
    member _.IsDisposableType =
        if isUnresolved () then
            ...
            DiagnosticsLogger.protectAssemblyExploration false
            <| fun () -> ExistsHeadTypeInEntireHierarchy cenv.g cenv.amap range0 ty cenv.g.tcref_System_IDisposable
            ...
            <| fun () ->
                ([
                    ...
                    if x.IsFSharpAbbreviation then
                        ...
                        for minfo in
                            GetImmediateIntrinsicMethInfosOfType (None, AccessibleFromSomeFSharpCode) cenv.g cenv.amap range0 entityTy do
                            yield createMember minfo

                    let props =
                        GetImmediateIntrinsicPropInfosOfType (None, AccessibleFromSomeFSharpCode) cenv.g cenv.amap range0 entityTy

                    let events =
                        cenv.infoReader.GetImmediateIntrinsicEventsOfType(None, AccessibleFromSomeFSharpCode, range0, entityTy)
    ...
    member _.XmlDoc =
        if isUnresolved () then
            XmlDoc.Empty |> makeXmlDoc
        else
            entity.XmlDoc |> makeXmlDoc

    member _.ElaboratedXmlDoc =
        if isUnresolved () then
            XmlDoc.Empty |> makeElaboratedXmlDoc
        else
        ...
        let walkParts (parts: (string * ModuleOrNamespaceKind) list) =
            let rec loop (currentPaths: string list list) parts =
                ...
                | (name: string, kind) :: rest ->
                    ...
                    | _ -> loop (mapEachCurrentPath currentPaths name) rest
            ...
            if entity.IsNamespace then
                ...
                let extraOpenPath =
                    ...
                    | Some cpath ->
                        let rec getOpenPath accessPath acc =
                            ...
                            | [] -> acc
                            | (name, ModuleOrNamespaceKind.ModuleOrType) :: accessPath -> getOpenPath accessPath (name :: acc)
                            | (name, ModuleOrNamespaceKind.Namespace _) :: accessPath -> getOpenPath accessPath (name :: acc)
                            | (name, ModuleOrNamespaceKind.FSharpModuleWithSuffix) :: accessPath -> getOpenPath accessPath (name :: acc)
                ...
                let headerL =
                    if List.isEmpty extraOpenPath then
                        ...
                        Layout.(^^) (Layout.wordL (TaggedText.tagKeyword "namespace")) pathL

                let openL =
                    if List.isEmpty openPathL then
                        Layout.emptyL
                    else
                        let openKeywordL =
                            if needOpenType then
                                Layout.(^^) (Layout.wordL (TaggedText.tagKeyword "open")) (Layout.wordL TaggedText.keywordType)
                            else
                                Layout.wordL (TaggedText.tagKeyword "open")

                        Layout.(^^) openKeywordL pathL
...
type FSharpUnionCase(cenv, v: UnionCaseRef) =
    inherit
        FSharpSymbol(
            ...
            (fun _this thisCcu2 ad -> checkForCrossProjectAccessibility cenv.g.ilg (thisCcu2, ad) (cenv.thisCcu, v.UnionCase.Accessibility))
...
type FSharpField(cenv: SymbolEnv, d: FSharpFieldData) =
    inherit
        FSharpSymbol(
            ...
            (fun () ->
                match d with
                | AnonField(anonInfo, tinst, n, m) -> Item.AnonRecdField(anonInfo, tinst, n, m)
                | RecdOrClass v ->
                    checkEntityIsResolved v.TyconRef
    ...
    member _.XmlDocSig =
        ...
        let xmlsig =
            ...
            | ILField f -> GetXmlDocSigOfILFieldInfo cenv.infoReader range0 f
            | AnonField _ -> None
...
type FSharpActivePatternCase(cenv, apinfo: ActivePatternInfo, ty, n, valOpt: ValRef option, item) =
    ...
    inherit FSharpSymbol(cenv, (fun () -> item), (fun _ _ _ -> true))
...
type FSharpGenericParameter(cenv, v: Typar) =
    ...
    inherit FSharpSymbol(cenv, (fun () -> Item.TypeVar(v.Name, v)), (fun _ _ _ad -> true))
    ...
    member _.Constraints =
        v.Constraints
        |> List.map (fun a -> FSharpGenericParameterConstraint(cenv, a))
        |> makeReadOnlyCollection
...
type FSharpGenericParameterMemberConstraint(cenv, info: TraitConstraintInfo) =
    inherit FSharpSymbol(cenv, (fun () -> Item.Trait(info)), (fun _ _ _ad -> true))
    ...
    member _.MemberArgumentTypes =
        info.CompiledObjectAndArgumentTypes
        |> List.map (fun ty -> FSharpType(cenv, ty))
        |> makeReadOnlyCollection
...
type FSharpGenericParameterConstraint(cenv, cx: TyparConstraint) =
    ...
    member _.IsSupportsNullConstraint =
        match cx with
    ...
    member _.SimpleChoices =
        match cx with
        | TyparConstraint.SimpleChoice(tys, _) -> tys |> List.map (fun ty -> FSharpType(cenv, ty)) |> makeReadOnlyCollection
...
type FSharpMemberOrFunctionOrValue(cenv, d: FSharpMemberOrValData, item) =
    ...
    let checkIsResolved () =
        if isUnresolved () then
            ...
            let nm =
                (match v with
                 | VRefNonLocal n -> n.ItemKey.PartialKey.LogicalName
                 | _ -> "<local>")
    ...
    let mkEventSym einfo =
        FSharpMemberOrFunctionOrValue(cenv, E einfo, Item.Event einfo)
            ...
            | Item.CtorGroup(_, methodInfos) ->
                ...
                let methods =
                    if matchParameterNumber then
                        methodInfos |> List.filter (fun methodInfo -> methodInfo.NumArgs <> m.NumArgs)
                    else
                        methodInfos
    ...
    member _.DeclaringEntity: FSharpEntity option =
        ...
        | M m
        | C m -> FSharpEntity(cenv, m.DeclaringTyconRef) |> Some
        | V v ->
    ...
    member _.ApparentEnclosingEntity: FSharpEntity =
        ...
        | M m
        | C m -> createEntity m.ApparentEnclosingType
        | V v ->
    ...
    member _.IsUnionCaseTester =
        ...
        | V v ->
            ...
            && v.IsImplied
            && v.MemberApparentEntity.IsUnionTycon
        | E _
        | C _ -> false
    ...
    member _.EventAddMethod =
        ...
        | M _
        | C _
        | V _ -> invalidOp "the value or member doesn't have an associated add method"
    ...
    member _.EventRemoveMethod =
        ...
        | M _
        | C _
        | V _ -> invalidOp "the value or member doesn't have an associated remove method"
    ...
    member _.EventDelegateType =
        ...
        | M _
        | C _
        | V _ -> invalidOp "the value or member doesn't have an associated event delegate type"
    ...
    member _.EventIsStandard =
        ...
        | E e ->
            ...
            TryDestStandardDelegateType cenv.infoReader range0 AccessibleFromSomewhere dty
            |> Option.isSome
        | P _
        | M _
        | C _
        | V _ -> invalidOp "the value or member is not an event"
        ...
        | P p when p.IsFSharpEventProperty ->
            let minfos1 =
                GetImmediateIntrinsicMethInfosOfType
                    (Some("add_" + p.PropertyName), AccessibleFromSomeFSharpCode)
                    cenv.g
                    cenv.amap
                    range0
                    p.ApparentEnclosingType

            let minfos2 =
                GetImmediateIntrinsicMethInfosOfType
                    (Some("remove_" + p.PropertyName), AccessibleFromSomeFSharpCode)
                    cenv.g
                    cenv.amap
                    range0
                    p.ApparentEnclosingType
    ...
    member _.IsEventAddMethod =
        if isUnresolved () then
            ...
            | M m ->
                ...
                && logicalName.StartsWithOrdinal("add_")
                &&

                let eventName = logicalName[4..] in
                let entityTy = generalizedTyconRef cenv.g m.DeclaringTyconRef in

                not (
                    isNil (
                        cenv.infoReader.GetImmediateIntrinsicEventsOfType(Some eventName, AccessibleFromSomeFSharpCode, range0, entityTy)
                    )
                )
                || let declaringTy = generalizedTyconRef cenv.g m.DeclaringTyconRef in

                   match
                       GetImmediateIntrinsicPropInfosOfType
                           (Some eventName, AccessibleFromSomeFSharpCode)
                           cenv.g
                           cenv.amap
                           range0
                           declaringTy
    ...
    member _.IsEventRemoveMethod =
        if isUnresolved () then
            ...
            | M m ->
                ...
                logicalName.Length > 4
                && logicalName.StartsWithOrdinal("remove_")
                &&

                let eventName = logicalName[7..] in
                let entityTy = generalizedTyconRef cenv.g m.DeclaringTyconRef in
    ...
    member _.IsPropertyGetterMethod =
        if isUnresolved () then
            ...
            | M m ->
                ...
                logicalName.Length > 4
                && logicalName.StartsWithOrdinal("get_")
                &&

                let propName = ChopPropertyName(logicalName) in
                let declaringTy = generalizedTyconRef cenv.g m.DeclaringTyconRef in

                not (
                    isNil (
                        GetImmediateIntrinsicPropInfosOfType
                            (Some propName, AccessibleFromSomeFSharpCode)
                            cenv.g
                            cenv.amap
                            range0
                            declaringTy
    ...
    member _.IsPropertySetterMethod =
        if isUnresolved () then
            ...
            | M m ->
                ...
                logicalName.Length > 4
                && logicalName.StartsWithOrdinal("set_")
                &&
        ...
        if isUnresolved () then
            ...
            && match d with
               | E e ->
                   ...
                   | Some vref -> ValRefIsCompiledAsInstanceMember cenv.g vref
                   | None -> true
               | P p ->
    ...
    member _.ImplementedAbstractSignatures =
        ...
        let sigs =
            ...
            | M m
            | C m -> m.ImplementedSlotSignatures
    ...
    member _.ElaboratedXmlDoc =
        if isUnresolved () then
            ...
            | M m
            | C m -> m.XmlDoc |> makeElaboratedXmlDoc
    ...
    member x.CurriedParameterGroups =
        ...
        | P p ->
            [
                [
                    for ParamData(isParamArrayArg, isInArg, isOutArg, optArgInfo, _callerInfo, nmOpt, _reflArgInfo, pty) in
                        p.GetParamDatas(cenv.amap, range0) do
                        // INCOMPLETENESS: Attribs is empty here, so we can't look at attributes for
                        // either .NET or F# parameters
                        let argInfo: ArgReprInfo =
                            {
                                Name = nmOpt
                                Attribs = []
                                OtherRange = None
                        ...
                        yield
                            [
                                for argTy, argInfo in argTys do
                                    let isParamArrayArg =
                                        HasFSharpAttribute cenv.g cenv.g.attrib_ParamArrayAttribute argInfo.Attribs

                                    let isInArg =
                                        HasFSharpAttribute cenv.g cenv.g.attrib_InAttribute argInfo.Attribs
                                        && isByrefTy cenv.g argTy

                                    let isOutArg =
                                        HasFSharpAttribute cenv.g cenv.g.attrib_OutAttribute argInfo.Attribs
                                        && isByrefTy cenv.g argTy

                                    let isOptionalArg =
                                        HasFSharpAttribute cenv.g cenv.g.attrib_OptionalArgumentAttribute argInfo.Attribs
    ...
    member x.ReturnParameter =
        ...
        | E einfo ->
            ...
            let retTy =
                try
                    PropTypeOfEventInfo cenv.infoReader m AccessibleFromSomewhere einfo
                with _ ->
    ...
    override _.Attributes =
        if isUnresolved () then
            ...
            match d with
            | E einfo ->
                GetAttribInfosOfEvent cenv.amap m einfo
                |> List.map (fun a -> FSharpAttribute(cenv, a))
            | P pinfo ->
                GetAttribInfosOfProp cenv.amap m pinfo
                |> List.map (fun a -> FSharpAttribute(cenv, a))
            | M minfo
            | C minfo ->
                GetAttribInfosOfMethod cenv.amap m minfo
                |> List.map (fun a -> FSharpAttribute(cenv, a))
            | V v ->
                v.Attribs
                |> List.map (fun a -> FSharpAttribute(cenv, AttribInfo.FSAttribInfo(cenv.g, a)))
    ...
    override x.ToString() =
        try
            let prefix =
                (if x.IsEvent then "event "
                 elif x.IsProperty then "property "
                 elif x.IsMember then "member "
                 else "val ")
    ...
    member x.FormatLayout(displayContext: FSharpDisplayContext) =
        ...
        | true, V v ->
            NicePrint.prettyLayoutOfMemberNoInstShort
                { (displayContext.Contents cenv.g) with
                    showMemberContainers = true
                }
                v.Deref
    ...
    member x.GetValSignatureText(displayContext: FSharpDisplayContext, m: range) =
        ...
        let stringValOfPropInfo (p: PropInfo) =
            ...
            if not supportAccessModifiersBeforeGetSet then
                ...
                | _ ->
                    let t =
                        p.GetPropertyType(cenv.amap, m)
                        |> NicePrint.layoutType displayEnv
                        |> LayoutRender.showL
                ...
                let withGetSet =
                    ...
                    | true, false -> $"with %s{getterAccess}get"
                    | false, true -> $"with %s{setterAccess}set"
                    | true, true -> $"with %s{getterAccess}get, %s{setterAccess}set"
        ...
        match d with
        | E _ -> None
        | V v -> NicePrint.stringValOrMember displayEnv cenv.infoReader v |> Some
        | C methInfo
    ...
    member x.GetWitnessPassingInfo() =
        let witnessInfos =
            match d with
            | M(FSMeth(_, _, vref, _)) ->
                let _tps, witnessInfos, _curriedArgInfos, _retTy, _ =
                    GetTypeOfMemberInMemberForm cenv.g vref
            ...
            | V vref ->
                ...
                let _tps, witnessInfos, _curriedArgInfos, _retTy, _ =
                    GetValReprTypeInCompiledForm cenv.g arities numEnclosingTypars vref.Type vref.DefinitionRange
        ...
        | _ ->
            let witnessParams =
                ...
                ||> List.mapFold (fun (used, i) witnessInfo ->
                    ...
                    let argReprInfo: ArgReprInfo =
                        {
                            Attribs = []
                            Name = Some(mkSynId m nm)
                            OtherRange = None
                        }

                    let p =
                        FSharpParameter(cenv, paramTy, argReprInfo, None, m, false, false, false, false, true)
    ...
    member x.TryGetFullCompiledOperatorNameIdents() : string[] option =
        ...
        if IsOperatorDisplayName x.DisplayName && x.DisplayName <> x.CompiledName then
            ...
            |> Option.map (fun enclosingEntityFullName -> Array.append (enclosingEntityFullName.Split '.') [| x.CompiledName |])
        else
            None
...
type FSharpType(cenv, ty: TType) =
    ...
    let isUnresolved () =
        DiagnosticsLogger.protectAssemblyExploration true
        <| fun () ->
    ...
    member _.HasTypeDefinition =
        ...
        && protect
           <| fun () ->
               match stripTyparEqns ty with
               | TType_app _
               | TType_measure(Measure.Const _ | Measure.Prod _ | Measure.Inv _ | Measure.One _) -> true
               | _ -> false
    ...
    member _.IsNullAmbivalent =
        ...
        <| fun () ->
            ...
            | TType_fun(_, _, nullness) ->
                match nullness.Evaluate() with
                | NullnessInfo.AmbivalentToNull -> true
                | _ -> false
            | TType_tuple(_, _) -> false
            | _ -> false
    ...
    member _.GenericArguments =
        ...
        <| fun () ->
            ...
            | TType_fun(domainTy, rangeTy, _) ->
                [| FSharpType(cenv, domainTy); FSharpType(cenv, rangeTy) |]
                |> makeReadOnlyCollection
            | TType_measure(Measure.Const _) -> [||] |> makeReadOnlyCollection
            | TType_measure(Measure.Prod(measure1 = t1; measure2 = t2)) ->
                [| FSharpType(cenv, TType_measure t1); FSharpType(cenv, TType_measure t2) |]
                |> makeReadOnlyCollection
...
*)
    ...
    member _.AbbreviatedType = protect <| fun () -> FSharpType(cenv, stripTyEqns cenv.g ty)
    ...
    member _.GenericParameter =
        ...
        <| fun () ->
            match stripTyparEqns ty with
            | TType_var(tp, _)
            | TType_measure(Measure.Var tp) -> FSharpGenericParameter(cenv, tp)
    ...
    member x.ErasedType = FSharpType(cenv, stripTyEqnsWrtErasure EraseAll cenv.g ty)
    ...
    member _.Instantiate(instantiation: (FSharpGenericParameter * FSharpType) list) =
        let resTy =
            instType (instantiation |> List.map (fun (tyv, ty) -> tyv.TypeParameter, ty.Type)) ty
    ...
    member private ty.AdjustType t = FSharpType(ty.cenv, t)
    ...
    member _.Format(context: FSharpDisplayContext) =
        protect <| fun () -> NicePrint.prettyStringOfTyNoCx (context.Contents cenv.g) ty

    member _.FormatWithConstraints(context: FSharpDisplayContext) =
        protect <| fun () -> NicePrint.prettyStringOfTy (context.Contents cenv.g) ty
    ...
    override _.ToString() =
        protect
        <| fun () -> "type " + NicePrint.prettyStringOfTyNoCx (DisplayEnv.Empty(cenv.g)) ty
    ...
    static member Prettify(parameters: IList<FSharpParameter>) =
        ...
        | h :: _ ->
            ...
            let prettyTys =
                parameters |> List.map (fun p -> p.V) |> PrettyTypes.PrettifyTypes cenv.g |> fst
    ...
    static member Prettify(parameters: IList<IList<FSharpParameter>>) =
        ...
        | Some h ->
            ...
            let prettyTys =
                xs
                |> List.mapSquared (fun p -> p.V)
                |> PrettyTypes.PrettifyCurriedTypes cenv.g
                |> fst

            (xs, prettyTys) ||> List.map2 (List.map2 (fun p pty -> p.AdjustType pty))
        |> List.map makeReadOnlyCollection
        |> makeReadOnlyCollection
    ...
    static member Prettify(parameters: IList<IList<FSharpParameter>>, returnParameter: FSharpParameter) =
        ...
        let prettyTys, prettyRetTy =
            xs
            |> List.mapSquared (fun p -> p.V)
            |> (fun tys -> PrettyTypes.PrettifyCurriedSigTypes cenv.g (tys, returnParameter.V))
            |> fst

        let ps =
            (xs, prettyTys)
            ||> List.map2 (List.map2 (fun p pty -> p.AdjustType pty))
            |> List.map makeReadOnlyCollection
            |> makeReadOnlyCollection
...
type FSharpAttribute(cenv: SymbolEnv, attrib: AttribInfo) =
    ...
    member _.AttributeType = FSharpEntity(cenv, attrib.TyconRef)
    ...
    member _.Format(context: FSharpDisplayContext) =
        ...
        <| fun () ->
            match attrib with
            | AttribInfo.FSAttribInfo(g, attrib) -> NicePrint.stringOfFSAttrib (context.Contents g) attrib
    ...
    override _.ToString() =
        if entityIsUnresolved attrib.TyconRef then
            "attribute ???"
        else
            "attribute " + attrib.TyconRef.CompiledName + "(...)"

    member attr.IsAttribute<'T>() =
        // CompiledName throws exception on DataContractAttribute generated by SQLProvider
        try
            attr.AttributeType.CompiledName = typeof<'T>.Name
        with _ ->
            false
...
type FSharpStaticParameter(cenv, sp: Tainted<TypeProviders.ProvidedParameterInfo>, m) =
    ...
    member _.Name = protect <| fun () -> sp.PUntaint((fun p -> p.Name), m)
    ...
    member _.IsOptional = protect <| fun () -> sp.PUntaint((fun x -> x.IsOptional), m)
    ...
    override x.GetHashCode() = hash x.Name
    override x.ToString() = "static parameter " + x.Name
#endif

type FSharpParameter
    (cenv, paramTy: TType, topArgInfo: ArgReprInfo, ownerOpt, m: range, isParamArrayArg, isInArg, isOutArg, isOptionalArg, isWitnessArg) =
    inherit FSharpSymbol(cenv, (fun () -> Item.OtherName(topArgInfo.Name, paramTy, Some topArgInfo, ownerOpt, m)), (fun _ _ _ -> true))
    ...
    new(cenv, ty, argInfo: ArgReprInfo, m: range) = FSharpParameter(cenv, ty, argInfo, None, m, false, false, false, false, false)
    ...
    member _.AdjustType ty =
        FSharpParameter(cenv, ty, topArgInfo, ownerOpt, m, isParamArrayArg, isInArg, isOutArg, isOptionalArg, isWitnessArg)
    ...
    new(cenv: SymbolEnv, ccu: CcuThunk) =
        let cenv =
            if ccu.IsUnresolvedReference then
                ...
                SymbolEnv(cenv.g, ccu, None, cenv.tcImports)
    ...
    member _.Attributes =
        [
            ...
            | None ->
                ...
                | None -> ()
                | Some tA ->
                    for a in tA.assemblyAttrs do
                        yield FSharpAttribute(cenv, AttribInfo.FSAttribInfo(cenv.g, a))
        ]
        |> makeReadOnlyCollection
...
type FSharpAssembly internal (cenv, ccu: CcuThunk) =
    ...
    new(tcGlobals, tcImports, ccu: CcuThunk) = FSharpAssembly(SymbolEnv(tcGlobals, ccu, None, tcImports), ccu)

    member _.RawCcuThunk = ccu

    member _.QualifiedName =
        match ccu.QualifiedName with
        | None -> ""
        | Some s -> s
    ...
    override x.ToString() =
        ...
        | ILScopeRef.PrimaryAssembly -> cenv.g.ilg.primaryAssemblyRef.QualifiedName
        | scoref -> scoref.QualifiedName
