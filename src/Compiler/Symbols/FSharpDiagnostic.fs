...
module ExtendedData =
    ...
    type FieldNotContainedDiagnosticExtendedData
        ...
        member x.SignatureField =
            FSharpField(symbolEnv, RecdFieldRef.RecdFieldRef(mkLocalTyconRef sigTycon, signatureField.Id.idText))
    ...
    type ValueNotContainedDiagnosticExtendedData internal (symbolEnv: SymbolEnv, signatureValue: Val, implValue: Val) =
        ...
        member x.SignatureValue =
            FSharpMemberOrFunctionOrValue(symbolEnv, mkLocalValRef signatureValue)

        member x.ImplementationValue =
            FSharpMemberOrFunctionOrValue(symbolEnv, mkLocalValRef implValue)
...
type FSharpDiagnostic
    (
        ...
        errorNum: int,
        numberPrefix: string,
        extendedData: IFSharpDiagnosticExtendedData option
    ) =
    member _.Range = m
    ...
    static member CreateFromException
        (diagnostic: PhasedDiagnostic, severity, suggestNames: bool, flatErrors: bool, symbolEnv: SymbolEnv option)
        =
        let extendedData: IFSharpDiagnosticExtendedData option =
            ...
            | Some symbolEnv ->
                ...
                | ErrorFromAddingTypeEquation(_,
                                              ...
                                              expectedType,
                                              actualType,
                                              ConstraintSolverTupleDiffLengths(contextInfo = contextInfo),
                                              _)
                | ErrorFromAddingTypeEquation(_,
                                              displayEnv,
                                              expectedType,
                                              actualType,
                                              ConstraintSolverTypesNotInEqualityRelation(_, _, _, _, _, contextInfo),
                                              _)
                ...
                | ErrorFromAddingTypeEquation(_, displayEnv, expectedType, actualType, _, _) ->
                    Some(
                        TypeMismatchDiagnosticExtendedData(symbolEnv, displayEnv, expectedType, actualType, DiagnosticContextInfo.NoContext)
                    )

                | FunctionValueUnexpected(_, actualType, _) -> Some(ExpressionIsAFunctionExtendedData(symbolEnv, actualType))
                ...
                | ArgumentsInSigAndImplMismatch(sigArg, implArg) -> Some(ArgumentsInSigAndImplMismatchExtendedData(sigArg, implArg))
...
type DiagnosticsScope(flatErrors: bool) =
        ...
        UseDiagnosticsLogger
            { new DiagnosticsLogger("DiagnosticsScope") with
                ...
                member _.DiagnosticSink(diagnostic, severity) =
                    let diagnostic =
                        FSharpDiagnostic.CreateFromException(diagnostic, severity, false, flatErrors, None)
    ...
    member _.Errors =
        diags
        |> List.filter (fun error -> error.Severity = FSharpDiagnosticSeverity.Error)
...
module DiagnosticHelpers =
    ...
    let ReportDiagnostic
        (
            options: FSharpDiagnosticOptions,
            allErrors,
            mainInputFileName,
        ...
        | adjustedSeverity ->
            ...
            let diagnostic =
                FSharpDiagnostic.CreateFromException(diagnostic, adjustedSeverity, suggestNames, flatErrors, symbolEnv)
            ...
            if
                ...
                || fileName = TcGlobals.DummyFileNameForRangesWithoutASpecificLocation
            then
                [ diagnostic ]
            else
                []
