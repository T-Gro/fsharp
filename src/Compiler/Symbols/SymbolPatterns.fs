...
module FSharpSymbolPatterns =
    ...
    let (|Attribute|_|) (entity: FSharpEntity) =
        let isAttribute (entity: FSharpEntity) =
            ...
            let rec isAttributeType (ty: FSharpEntity option) =
                ...
                | Some ty ->
                    try
                        ty.FullName = "System.Attribute" || isAttributeType (getBaseType ty)
                    with _ ->
                        false
    ...
    let (|ValueType|_|) (e: FSharpEntity) =
        if
            e.IsEnum
            || e.IsValueType
            || e.HasAttribute<MeasureAnnotatedAbbreviationAttribute>()
        then
            Some()
        else
            None
    ...
    let (|Class|_|) (original: FSharpEntity, abbreviated: FSharpEntity, _) =
        if
            abbreviated.IsClass
            && (not abbreviated.IsStaticInstantiation || original.IsFSharpAbbreviation)
        then
            Some()
        else
            None
    ...
    let (|AbstractClass|_|) (e: FSharpEntity) =
        if e.HasAttribute<AbstractClassAttribute>() then
            Some()
        else
            None
    ...
    let (|ProvidedType|_|) (e: FSharpEntity) =
        if
            (e.IsProvided || e.IsProvidedAndErased || e.IsProvidedAndGenerated)
            && e.CompiledName = e.DisplayName
        then
            Some()
        else
            None
    ...
    let (|Tuple|_|) (ty: FSharpType) = if ty.IsTupleType then Some() else None
    ...
    let (|Constructor|_|) (func: FSharpMemberOrFunctionOrValue) =
        ...
        | ".ctor"
        | ".cctor" -> func.DeclaringEntity
        | _ -> None

    let (|Function|_|) excluded (func: FSharpMemberOrFunctionOrValue) =
        try
            let ty = func.FullType.StripAbbreviations()
            ...
            if
                ...
                && not excluded
                && not (PrettyNaming.IsOperatorDisplayName func.DisplayName)
            then
                Some()
            else
                None
