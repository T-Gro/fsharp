...
module internal SymbolHelpers =
    ...
    let rec rangeOfItem (g: TcGlobals) preferFlag d =
        match d with
        | Item.Value vref
        | Item.CustomBuilder(_, vref) -> Some(rangeOfValRef preferFlag vref)
        ...
        | Item.Types(_, tys) ->
            tys
            |> List.tryPick (tryNiceEntityRefOfTyOption >> Option.map (rangeOfEntityRef preferFlag))
    ...
    let rec ccuOfItem (g: TcGlobals) d =
        match d with
        | Item.Value vref
        | Item.CustomBuilder(_, vref) -> ccuOfValRef vref
        ...
        | Item.Types(_, tys) ->
            tys
            |> List.tryPick (tryNiceEntityRefOfTyOption >> Option.bind computeCcuOfTyconRef)

        | Item.DelegateCtor(ty) -> ty |> tryNiceEntityRefOfTyOption |> Option.bind computeCcuOfTyconRef
    ...
    let fileNameOfItem (g: TcGlobals) qualProjectDir (m: range) h =
        ...
        if not (FileSystem.IsPathRootedShim file) then
            ...
            | Some ccu -> Path.Combine(ccu.SourceCodeDirectory, file)
            | None ->
        ...
        | FSMeth(_, _, vref, _) ->
            ...
            [
                for ty, argInfo in argInfos do
                    let isPP =
                        HasFSharpAttribute g g.attrib_ProjectionParameterAttribute argInfo.Attribs
    ...
    let rec GetXmlDocHelpSigOfItemForLookup (infoReader: InfoReader) m d =
        ...
        | Item.Value vref
        | Item.CustomBuilder(_, vref) -> mkXmlComment (GetXmlDocSigOfValRef g vref)
        ...
        | Item.Types(_, ty :: _) ->
            match ty with
            | AbbrevOrAppTy(tcref, _) -> mkXmlComment (GetXmlDocSigOfEntityRef infoReader m tcref)
            | _ -> FSharpXmlDoc.None
        ...
        | Item.Property(info = pinfo :: _) -> mkXmlComment (GetXmlDocSigOfProp infoReader m pinfo)
        ...
        | Item.SetterArg(_, item) -> GetXmlDocHelpSigOfItemForLookup infoReader m item
        ...
        | Item.Types(_, []) -> FSharpXmlDoc.None
    ...
    let GetXmlCommentForItemAux (xmlDoc: XmlDoc option) (infoReader: InfoReader) m d =
        match xmlDoc with
        | Some xmlDoc when not xmlDoc.IsEmpty -> FSharpXmlDoc.FromXmlText xmlDoc
    ...
    let FormatTyparMapping denv (prettyTyparInst: TyparInstantiation) =
        [
            for tp, ty in prettyTyparInst ->
                wordL (tagTypeParameter ("'" + tp.DisplayName))
                ^^ wordL (tagText (FSComp.SR.descriptionWordIs ()))
            ...
            member x.Equals(item1, item2) =
                ...
                | item1, item2 ->
                    ...
                    protectAssemblyExploration false (fun () ->
                        ...
                        | Item.ILField(fld1), Item.ILField(fld2) -> ILFieldInfo.ILFieldInfosUseIdenticalDefinitions fld1 fld2
                        ...
                        | Item.TypeVar(nm1, tp1), Item.TypeVar(nm2, tp2) -> (nm1 = nm2) && typarRefEq tp1 tp2
                        | Item.ModuleOrNamespaces(modref1 :: _), Item.ModuleOrNamespaces(modref2 :: _) ->
                            fullDisplayTextOfModRef modref1 = fullDisplayTextOfModRef modref2
                        ...
                        | Item.ActivePatternCase(APElemRef(_apinfo1, vref1, idx1, _)),
                          Item.ActivePatternCase(APElemRef(_apinfo2, vref2, idx2, _)) -> idx1 = idx2 && valRefEq g vref1 vref2
                        | Item.UnionCase(UnionCaseInfo(_, ur1), _), Item.UnionCase(UnionCaseInfo(_, ur2), _) -> g.unionCaseRefEq ur1 ur2
                        | Item.RecdField(RecdFieldInfo(_, RecdFieldRef(tcref1, n1))),
                          Item.RecdField(RecdFieldInfo(_, RecdFieldRef(tcref2, n2))) -> (tyconRefEq g tcref1 tcref2) && (n1 = n2) // there is no direct function as in the previous case
                        | Item.Property(info = pi1s), Item.Property(info = pi2s) ->
                            (pi1s, pi2s) ||> List.forall2 PropInfo.PropInfosUseIdenticalDefinitions
                        | Item.Event evt1, Item.Event evt2 -> EventInfo.EventInfosUseIdenticalDefinitions evt1 evt2
                        | Item.AnonRecdField(anon1, _, i1, _), Item.AnonRecdField(anon2, _, i2, _) -> anonInfoEquiv anon1 anon2 && i1 = i2
                        | Item.Trait traitInfo1, Item.Trait traitInfo2 -> (traitInfo1.MemberLogicalName = traitInfo2.MemberLogicalName)
                        | Item.CtorGroup(_, meths1), Item.CtorGroup(_, meths2) ->
                            (meths1, meths2) ||> List.forall2 MethInfo.MethInfosUseIdenticalDefinitions
            ...
            member x.GetHashCode item =
                ...
                protectAssemblyExploration 1027 (fun () ->
                    ...
                    | ItemWhereTypIsPreferred ty ->
                        ...
                        | _ -> 1010
                    | Item.ILField(fld) -> fld.ComputeHashCode()
                    ...
                    | Item.CtorGroup(name, meths) ->
                        name.GetHashCode()
                        + (meths |> List.fold (fun st a -> st + a.ComputeHashCode()) 0)
                    | Item.Value vref
                    | Item.CustomBuilder(_, vref) -> hash vref.LogicalName
    ...
    let RemoveDuplicateItems g (items: ItemWithInst list) =
        if isNil items then
            ...
            |> IPartialEqualityComparer.partialDistinctBy (
                IPartialEqualityComparer.On (fun item -> item.Item) (ItemDisplayPartialEquality g)
            )
    ...
    let rec FullNameOfItem g item =
        ...
        | Item.Value vref
        | Item.CustomBuilder(_, vref) -> fullDisplayTextOfValRef vref
        ...
        | Item.ILField finfo ->
            buildString (fun os ->
                NicePrint.outputType denv os finfo.ApparentEnclosingType
                bprintf os ".%s" finfo.FieldName)
        | Item.Event einfo ->
            buildString (fun os ->
                NicePrint.outputTyconRef denv os einfo.DeclaringTyconRef
                bprintf os ".%s" einfo.EventName)
        | Item.Property(info = pinfo :: _) ->
            buildString (fun os ->
                NicePrint.outputTyconRef denv os pinfo.DeclaringTyconRef
        ...
        | Item.MethodGroup(_, _, Some minfo) ->
            buildString (fun os ->
                NicePrint.outputTyconRef denv os minfo.DeclaringTyconRef
                bprintf os ".%s" minfo.DisplayName)
        ...
        | Item.ModuleOrNamespaces(modref :: _ as modrefs) ->
            ...
            if definiteNamespace then
                fullDisplayTextOfModRef modref
            else
                modref.DisplayName
        | Item.TypeVar _
    ...
    let rec GetXmlCommentForItem (infoReader: InfoReader) m item =
        ...
        | Item.ImplicitOp(_, sln) ->
            ...
            | Some(TraitConstraintSln.FSMethSln(vref = vref)) -> GetXmlCommentForItem infoReader m (Item.Value vref)
            ...
            | None -> GetXmlCommentForItemAux None infoReader m item

        | Item.Value vref
        | Item.CustomBuilder(_, vref) ->
            let doc =
                if valRefInThisAssembly g.compilingFSharpCore vref || vref.XmlDoc.NonEmpty then
                    Some vref.XmlDoc
                else
                    None
        ...
        | Item.UnionCase(ucinfo, _) ->
            let doc =
                if
                    tyconRefUsesLocalXmlDoc g.compilingFSharpCore ucinfo.TyconRef
                    || ucinfo.UnionCase.XmlDoc.NonEmpty
                then
        ...
        | Item.ExnCase ecref ->
            let doc =
                if tyconRefUsesLocalXmlDoc g.compilingFSharpCore ecref || ecref.XmlDoc.NonEmpty then
                    Some ecref.XmlDoc
                else
                    None
        ...
        | Item.Event einfo ->
            let doc =
                if einfo.HasDirectXmlComment || einfo.XmlDoc.NonEmpty then
                    Some einfo.XmlDoc
                else
                    None
        ...
        | Item.Property(info = pinfos) ->
            ...
            let doc =
                if pinfo.HasDirectXmlComment || pinfo.XmlDoc.NonEmpty then
                    Some pinfo.XmlDoc
                else
                    None
        ...
        | Item.MethodGroup(_, minfo :: _, _) -> GetXmlCommentForMethInfoItem infoReader m item minfo
        ...
        | Item.ModuleOrNamespaces(modref :: _ as modrefs) ->
            ...
            if not definiteNamespace then
                let doc =
                    if entityRefInThisAssembly g.compilingFSharpCore modref || modref.XmlDoc.NonEmpty then
                        Some modref.XmlDoc
                    else
                        None
        ...
        | Item.SetterArg(_, item) -> GetXmlCommentForItem infoReader m item
    ...
    let rec GetF1Keyword (g: TcGlobals) item =
        ...
        let rec getKeywordForMethInfo (minfo: MethInfo) =
            ...
            | ILMeth(_, minfo, _) ->
                ...
                let paramString =
                    ...
                    if nGenericParams > 0 then
                        "``" + (nGenericParams.ToString())
                    else
                        ""
        ...
        | Item.CustomBuilder(_, vref) ->
            ...
            if v.IsModuleBinding && v.HasDeclaringEntity then
                ...
                sprintf "%s.%s%s" (tyconRef |> ticksAndArgCountTextOfTyconRef) (v.CompiledName g.CompilerGlobalState) paramsString
                |> Some
            else
                None

        | Item.ActivePatternCase apref -> GetF1Keyword g (Item.Value apref.ActivePatternVal)

        | Item.UnionCase(ucinfo, _) ->
            (ucinfo.TyconRef |> ticksAndArgCountTextOfTyconRef) + "." + ucinfo.DisplayName
            |> Some

        | Item.RecdField rfi -> (rfi.TyconRef |> ticksAndArgCountTextOfTyconRef) + "." + rfi.DisplayName |> Some
        ...
        | Item.ILField finfo ->
            ...
            | ILFieldInfo(tinfo, fdef) ->
                (tinfo.TyconRefOfRawMetadata |> ticksAndArgCountTextOfTyconRef)
                + "."
                + fdef.Name
                |> Some
#if !NO_TYPEPROVIDERS
    ...
    let SelectMethodGroupItems2 g (m: range) (item: ItemWithInst) : ItemWithInst list =
        ...
        | Item.MethodGroup(nm, minfos, orig) ->
            ...
            |> List.map (fun minfo ->
                {
                    Item = Item.MethodGroup(nm, [ minfo ], orig)
                    TyparInstantiation = item.TyparInstantiation
                })
        | Item.CtorGroup(nm, cinfos) ->
            cinfos
            |> List.map (fun minfo ->
                {
                    Item = Item.CtorGroup(nm, [ minfo ])
                    TyparInstantiation = item.TyparInstantiation
                })
        ...
        | Item.Event _ -> []
        | Item.RecdField rfinfo ->
            if isForallFunctionTy g rfinfo.FieldType then
                [ item ]
            else
                []
