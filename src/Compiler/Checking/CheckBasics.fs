...
type TcPatPhase2Input =
    ...
    member x.WithRightPath() =
        (let (TcPatPhase2Input(a, _)) = x in TcPatPhase2Input(a, false))
...
type TcFileState =
    {
        ...
        // forward call
        TcSimplePats:
            TcFileState -> bool -> CheckConstraints -> TType -> TcEnv -> TcPatLinearEnv -> SynSimplePats -> string list * TcPatLinearEnv
        ...
        TcComputationExpression:
            TcFileState -> TcEnv -> OverallTy -> UnscopedTyparEnv -> range * Expr * TType * SynExpr -> Expr * UnscopedTyparEnv
    }
