...
let fail () =
    failwith "This custom attribute has an argument that cannot yet be converted using this API"
...
let GetAttribInfosOfEntity g amap m (tcref: TyconRef) =
    ...
    //| None -> None
#endif
    | ILTypeMetadata(TILObjectReprData(scoref, _, tdef)) -> tdef.CustomAttrs |> AttribInfosOfIL g amap scoref m
    | FSharpOrArrayOrByrefOrTupleOrExnTypeMetadata -> tcref.Attribs |> List.map (fun a -> FSAttribInfo(g, a))

let rec GetAttribInfosOfMethod amap m minfo =
    match minfo with
    | ILMeth(g, ilminfo, _) ->
        ilminfo.RawMetadata.CustomAttrs
        |> AttribInfosOfIL g amap ilminfo.MetadataScope m
...
let GetAttribInfosOfProp amap m pinfo =
    match pinfo with
    | ILProp ilpinfo ->
        ilpinfo.RawMetadata.CustomAttrs
        |> AttribInfosOfIL ilpinfo.TcGlobals amap ilpinfo.ILTypeInfo.ILScopeRef m
...
let GetAttribInfosOfEvent amap m einfo =
    match einfo with
    | ILEvent ileinfo ->
        ileinfo.RawMetadata.CustomAttrs
        |> AttribInfosOfIL einfo.TcGlobals amap ileinfo.ILTypeInfo.ILScopeRef m
...
let MethInfoHasAttribute g m attribSpec minfo =
    TryBindMethInfoAttribute g m attribSpec minfo (fun _ -> Some()) (fun _ -> Some()) (fun _ -> Some())
    |> Option.isSome
...
let private CheckCompilerFeatureRequiredAttribute (g: TcGlobals) cattrs msg m =
    ...
    | Some([ ILAttribElem.String(Some featureName) ], _) when featureName = "RequiredMembers" -> CompleteD
    | _ -> ErrorD(ObsoleteDiagnostic(true, None, msg, None, m))
        ...
        | Some(Attrib(unnamedArgs = [ AttribStringArg(s) ])) ->
            let isExperimentalAttributeDisabled (s: string) =
                if g.compilingFSharpCore then
                    ...
                    && (s.IndexOf("--langversion:preview", StringComparison.OrdinalIgnoreCase) >= 0)
...
let private CheckProvidedAttributes (g: TcGlobals) m (provAttribs: Tainted<IProvidedCustomAttributeProvider>) =
    ...
    match
        provAttribs.PUntaint((fun a -> a.GetAttributeConstructorArgs(provAttribs.TypeProvider.PUntaintNoFailure(id), tref.FullName)), m)
    with
    ...
    | Some([ None ], _) -> WarnD(ObsoleteDiagnostic(false, None, None, None, m))
    | Some _ -> WarnD(ObsoleteDiagnostic(false, None, None, None, m))
    | None -> CompleteD
#endif
...
let CheckFSharpAttributesForHidden g attribs =
    not (isNil attribs)
    && (match TryFindFSharpAttribute g g.attrib_CompilerMessageAttribute attribs with
        | Some(Attrib(_, _, [ AttribStringArg _; AttribInt32Arg messageNumber ], ExtractAttribNamedArg "IsHidden" (AttribBoolArg v), _, _, _)) ->
...
let CheckProvidedAttributesForUnseen (provAttribs: Tainted<IProvidedCustomAttributeProvider>) m =
    provAttribs.PUntaint(
        (fun a ->
            a.GetAttributeConstructorArgs(provAttribs.TypeProvider.PUntaintNoFailure(id), !!typeof<ObsoleteAttribute>.FullName).IsSome),
        m
    )
#endif
...
#if !NO_TYPEPROVIDERS
    | ProvidedProp(amap, pi, m) -> CheckProvidedAttributes amap.g m (pi.PApply((fun st -> (st :> IProvidedCustomAttributeProvider)), m))
...
let CheckILFieldAttributes g (finfo: ILFieldInfo) m =
    match finfo with
    | ILFieldInfo(_, pd) -> CheckILAttributes g false pd.CustomAttrs m |> CommitOperationResult
#if !NO_TYPEPROVIDERS
    | ProvidedField(amap, fi, m) ->
        CheckProvidedAttributes amap.g m (fi.PApply((fun st -> (st :> IProvidedCustomAttributeProvider)), m))
                ...
                (fun fsAttribs ->
                    let res =
                        trackErrors {
                            ...
                            if
                                Option.isNone tyargsOpt
                                && HasFSharpAttribute g g.attrib_RequiresExplicitTypeArgumentsAttribute fsAttribs
                            then
...
let PropInfoIsUnseen m pinfo =
    ...
    | FSProp _ -> failwith "CheckPropInfoAttributes: unreachable"
#if !NO_TYPEPROVIDERS
    | ProvidedProp(_amap, pi, m) -> CheckProvidedAttributesForUnseen (pi.PApply((fun st -> (st :> IProvidedCustomAttributeProvider)), m)) m
#endif
...
let CheckValAttributes g (x: ValRef) m = CheckFSharpAttributes g x.Attribs m
            ...
            | _ ->
                let exists =
                    ExistsInEntireHierarchyOfType
                        (fun t -> typeEquiv g t (mkWoNullAppTy attr.TyconRef []))
                        g
                        amap
                        m
...
let IsSecurityCriticalAttribute g (Attrib(tcref, _, _, _, _, _, _)) =
    (tyconRefEq g tcref g.attrib_SecurityCriticalAttribute.TyconRef
     || tyconRefEq g tcref g.attrib_SecuritySafeCriticalAttribute.TyconRef)
...
let IsAssemblyVersionAttribute (g: TcGlobals) (Attrib(tcref, _, _, _, _, _, _)) =
    ...
    | None -> false
    | Some attr -> attr.TyconRef.CompiledRepresentationForNamedType.QualifiedName = tcref.CompiledRepresentationForNamedType.QualifiedName
