...
type AccessorDomain =
    ...
    static member CustomEquals(g: TcGlobals, ad1: AccessorDomain, ad2: AccessorDomain) =
        match ad1, ad2 with
        | AccessibleFrom(cs1, tc1), AccessibleFrom(cs2, tc2) ->
            (cs1 = cs2)
            && (match tc1, tc2 with
                | None, None -> true
                | Some tc1, Some tc2 -> tyconRefEq g tc1 tc2
                | _ -> false)
...
let IsAccessible ad taccess =
    ...
    | AccessibleFrom(cpaths, _tcrefViewedFromOption) -> List.exists (canAccessFrom taccess) cpaths
...
let private IsILMemberAccessible g amap m (tcrefOfViewedItem: TyconRef) ad access =
    match ad with
    | AccessibleFromEverywhere -> access = ILMemberAccess.Public
    ...
    | AccessibleFrom(cpaths, tcrefViewedFromOption) ->
        ...
        let accessibleByFamily =
            ((access = ILMemberAccess.Family || access = ILMemberAccess.FamilyOrAssembly)
             && match tcrefViewedFromOption with
                | None -> false
                | Some tcrefViewedFrom -> ExistsHeadTypeInEntireHierarchy g amap m (generalizedTyconRef g tcrefViewedFrom) tcrefOfViewedItem)
        ...
        (access = ILMemberAccess.Public)
        || accessibleByFamily
        || accessibleByInternalsVisibleTo
        || accessibleByFamilyAndAssembly
...
let private IsTyconAccessibleViaVisibleTo ad (tcrefOfViewedItem: TyconRef) =
    ...
    | AccessibleFrom(cpaths, _tcrefViewedFromOption) -> canAccessFromOneOf cpaths tcrefOfViewedItem.CompilationPath
    ...
    let rec check parentTycon path =
        let ilTypeDefAccessible =
            ...
            | None ->
                ...
                | [] ->
                    assert false
                    true // in this case path should have at least one element
...
let private IsILTypeAndMemberAccessible g amap m adType ad (ty: ILTypeInfo) access =
    IsILTypeInfoAccessible amap m adType ty.TyconRefOfRawMetadata
    && IsILMemberAccessible g amap m ty.TyconRefOfRawMetadata ad access
...
let rec IsTypeAccessible g amap m ad ty =
    ...
    | ValueNone -> true
    | ValueSome(tcref, tinst) -> IsEntityAccessible amap m ad tcref && IsTypeInstAccessible g amap m ad tinst
...
let IsProvidedMemberAccessible (amap: Import.ImportMap) m ad ty access =
    ...
    if IsTypeAccessible g amap m ad ty then
        ...
        | ValueNone -> true
        | ValueSome tcrefOfViewedItem -> IsILMemberAccessible g amap m tcrefOfViewedItem ad access
    else
        false
...
let ComputeILAccess isPublic isFamily isFamilyOrAssembly isFamilyAndAssembly =
    ...
    elif isFamilyOrAssembly then
        ILMemberAccess.FamilyOrAssembly
    elif isFamilyAndAssembly then
...
let IsILFieldInfoAccessible g amap m ad x =
    ...
    | ProvidedField(amap, tpfi, m) ->
        let access =
            tpfi.PUntaint((fun fi -> ComputeILAccess fi.IsPublic fi.IsFamily fi.IsFamilyOrAssembly fi.IsFamilyAndAssembly), m)
...
let private IsILMethInfoAccessible g amap m adType ad ilminfo =
    ...
    | ILMethInfo(_, CSharpStyleExtension(declaring = declaringTyconRef), mdef, _) ->
        IsILMemberAccessible g amap m declaringTyconRef ad mdef.Access
...
let IsValAccessible ad (vref: ValRef) = vref.Accessibility |> IsAccessible ad
...
let rec IsTypeAndMethInfoAccessible amap m accessDomainTy ad =
    function
    ...
    | ProvidedMeth(amap, tpmb, _, m) as etmi ->
        let access =
            tpmb.PUntaint((fun mi -> ComputeILAccess mi.IsPublic mi.IsFamily mi.IsFamilyOrAssembly mi.IsFamilyAndAssembly), m)

        IsProvidedMemberAccessible amap m ad etmi.ApparentEnclosingType access
#endif
let IsMethInfoAccessible amap m ad minfo =
    IsTypeAndMethInfoAccessible amap m ad ad minfo

let IsPropInfoAccessible g amap m ad =
    function
