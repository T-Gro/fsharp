...
let mkLambdaNoType g m uv e = mkLambda m uv (e, tyOfExpr g e)
...
let tyConfirmsToSeq g ty =
    ...
    | ValueSome tcref -> tyconRefEq g tcref g.tcref_System_Collections_Generic_IEnumerable
    | _ -> false
            ...
            phase2 =
                (fun ctxt ->
                    ...
                    let generate =
                        mkSequential
                            m
                            (mkSequential m (mkValSet m vref e |> addDebugPoint) generateBody)
    ...
    let rec ConvertSeqExprCode
        ...
        | SeqYield g (e, m) ->
            ...
            Some
                {
                    phase2 =
                        (fun (pcVar, currVar, _nextv, pcMap) ->
                            let generate =
                                mkSequential
                                    m
                            ...
                            let dispose =
                                mkLabelled m label (Expr.Op(TOp.Goto currentDisposeContinuationLabel, [], [], m))
                            ...
                            let checkDispose =
                                mkLabelled
                                    ...
                                    (Expr.Op(
                                        ...
                                        [],
                                        [
                                            mkBool g m (not (noDisposeContinuationLabel = currentDisposeContinuationLabel))
                                        ],
                                        m
                                    ))
        ...
        | SeqAppend g (e1, e2, m) ->
            ...
            let res1 =
                ConvertSeqExprCode false false noDisposeContinuationLabel currentDisposeContinuationLabel e1

            let res2 =
                ConvertSeqExprCode false isTailCall noDisposeContinuationLabel currentDisposeContinuationLabel e2
        ...
        | SeqWhile g (guardExpr, innerExpr, spWhile, m) ->
            let resBody =
                ConvertSeqExprCode false false noDisposeContinuationLabel currentDisposeContinuationLabel innerExpr
                ...
                Some
                    {
                        phase2 =
                            (fun ctxt ->
                                ...
                                let generate =
                                    mkWhile g (spWhile, NoSpecialWhileLoopMarker, guardExpr, generate2, m)
        ...
        | SeqUsing g (resource, v, body, elemTy, spBind, m) ->
            let reduction =
                mkLet
                    ...
                    v
                    resource
                    (mkCallSeqFinally g m elemTy body (mkUnitDelayLambda g m (mkCallDispose g m v.Type (exprForVal m v))))
        ...
        | SeqForEach g (inp, v, body, genElemTy, mFor, mIn, spIn) ->
            ...
            let mIn =
                match spIn with
                | DebugPointAtInOrTo.Yes m -> m.NoteSourceConstruct(NotedSourceConstruct.While)
                | DebugPointAtInOrTo.No -> mIn
        ...
        | SeqTryFinally g (e1, compensation, spTry, spFinally, m) ->
            ...
            let resBody =
                ConvertSeqExprCode false false noDisposeContinuationLabel innerDisposeContinuationLabel e1
            ...
            | Some res1 ->
                ...
                Some
                    {
                        phase2 =
                            (fun (pcVar, _currv, _, pcMap as ctxt) ->
                                let generate1, dispose1, checkDispose1 = res1.phase2 ctxt

                                let generate =
                                    // copy the compensation expression - one copy for the success continuation and one for the exception
                                    let compensation =
                                        copyExpr g CloneAllAndMarkExprValsAsCompilerGenerated compensation
                                    ...
                                    mkSequential
                                        ...
                                        (mkLabelled
                                            ...
                                            (mkSequential
                                                m
                                                (mkValSet m pcVar (mkInt32 g m pcMap[currentDisposeContinuationLabel])
                                                 |> addFinallyDebugPoint)
                                                (mkSequential m compensation (Expr.Op(TOp.Goto currentDisposeContinuationLabel, [], [], m)))))
        ...
        | Expr.Let(bind, bodyExpr, m, _) when
            ...
            let resBodyOpt =
                ConvertSeqExprCode false isTailCall noDisposeContinuationLabel currentDisposeContinuationLabel bodyExpr
...
*)
        ...
        | Expr.Match(spBind, mExpr, pt, targets, m, ty) ->
            ...
            let tglArray =
                targets
                |> Array.map (fun (TTarget(_vs, targetExpr, _)) ->
                    ConvertSeqExprCode false isTailCall noDisposeContinuationLabel currentDisposeContinuationLabel targetExpr)
            ...
            if tglArray |> Array.forall Option.isSome then
                ...
                let asyncVars =
                    ...
                    ||> Array.fold (fun fvs (TTarget(_vs, _, _), res) ->
                        if res.entryPoints.IsEmpty then
                            ...
                            unionFreeVars fvs res.asyncVars)
                ...
                let stateVars =
                    ...
                    |> List.collect (fun (TTarget(vs, _, _), res) ->
                        let stateVars =
                            vs
                            |> List.filter (fun v -> res.asyncVars.FreeLocals.Contains(v))
                ...
                Some
                    {
                        phase2 =
                            (fun ctxt ->
                                ...
                                let dispose =
                                    if isNil disposals then
                                        mkUnit g m
                                    else
                                        List.reduce (mkSequential m) disposals

                                let checkDispose =
                                    if isNil checkDisposes then
                                        mkFalse g m
                                    else
                                        List.reduce (mkSequential m) checkDisposes
        ...
        | Expr.DebugPoint(dp, innerExpr) ->
            let resInnerExpr =
                ConvertSeqExprCode isWholeExpr isTailCall noDisposeContinuationLabel currentDisposeContinuationLabel innerExpr
                ...
                | SeqElemTy g amap m inpElemTy ->
                    ...
                    if isTailCall then
                        ...
                        Some
                            {
                                phase2 =
                                    (fun (pcVar, _currv, nextVar, pcMap) ->
                                        let generate =
                                            mkSequential
                                                m
                                                (mkValSet m pcVar (mkInt32 g m pcMap[label]))
                                        ...
                                        let checkDispose = mkLabelled m label (Expr.Op(TOp.Return, [], [ mkFalse g m ], m))
                                        generate, dispose, checkDispose)
                                entryPoints = [ label ]
                                stateVars = []
                                significantClose = false
                        ...
                        ConvertSeqExprCode
                            ...
                            (mkCallSeqCollect
                                ...
                                inpElemTy
                                inpElemTy
                                (mkLambdaNoType g m v (mkCallSeqSingleton g m inpElemTy ve))
                                arbitrarySeqExpr)
                | _ -> None
    ...
    | Seq g (e, ty) ->
        ...
        | Some res ->
            ...
            let pc2lab =
                Map.ofList ((pcInit, initLabel) :: (pcDone, noDisposeContinuationLabel) :: List.zip pcs labs)

            let lab2pc =
                Map.ofList ((initLabel, pcInit) :: (noDisposeContinuationLabel, pcDone) :: List.zip labs pcs)
            ...
            let checkDisposeExprWithCleanup =
                mkSequential m checkDisposeExprCore (mkLabelled m noDisposeContinuationLabel (Expr.Op(TOp.Return, [], [ mkFalse g m ], m)))
            ...
            let handleExceptionsInDispose disposalExpr =
                ...
                // exn <- e
                let assignToExn =
                    Expr.Op(TOp.LValueOp(LValueOperation.LSet, exnVref), [], [ eE ], m)
                ...
                let exceptionCatcher =
                    mkTryWith
                        ...
                        (disposalExpr,
                         efV,
                         Expr.Const((Const.Bool true), m, g.bool_ty),
                         eV,
                         assignToExn,
                         m,
                         g.unit_ty,
                ...
                let whileLoop =
                    ...
                    let dtree =
                        TDSwitch(
                            ...
                            [
                                mkCase (
                                    (DecisionTreeTest.Const(Const.Int32 pcDone)),
                                    addResultTarget (Expr.Op(TOp.Goto doneDisposeLabel, [], [], m))
                                )
                            ],
                ...
                // --loop--
                // if exn != null then raise exn
                mkLet DebugPointAtBinding.NoneAtLet m exnV (Expr.Const(Const.Zero, m, g.exn_ty)) (mkCompGenSequential m whileLoop doRaise)

            // Add the jump table to the GenerateNext method
            let generateExprWithJumpTable = addJumpTable false generateExprWithCleanup
            ...
            Some(
                ...
                currVarRef,
                stateVars,
                generateExprWithJumpTable,
                disposalExprWithJumpTable,
                checkDisposeExprWithJumpTable,
                ty,
                m
            )
