...
let LowerStateMachineStackGuardDepth =
    StackGuard.GetDepthOption "LowerStateMachines"
...
let sm_verbose =
    try
        not (isNull (System.Environment.GetEnvironmentVariable "FSharp_StateMachineVerbose"))
    with _ ->
        false
#else
                    ...
                    mkSequential
                        ...
                        (if typeEquiv g (tyOfExpr g generateBody) g.bool_ty then
                             mkCond
                                 ...
                                 g.bool_ty
                                 generateBody
                                 (mkCompGenSequential m (mkValSet m vref (mkDefault (m, vref.Type))) (mkTrue g m))
                                 (mkFalse g m)
                         else
                             generateBody)
...
let isExpandVar g (v: Val) =
    isReturnsResumableCodeTy g v.TauType && not v.IsCompiledAsTopLevel
...
type LowerStateMachine(g: TcGlobals) =
    ...
    let rec BindResumableCodeDefinitions (env: env) expr =
        ...
        | Expr.Let(defn, bodyExpr, _, _) when isStateMachineBindingVar g defn.Var ->
            if sm_verbose then
                printfn "binding %A --> %A..." defn.Var defn.Expr

            let envR =
                { env with
                    ResumableCodeDefns = env.ResumableCodeDefns.Add defn.Var defn.Expr
                }
        ...
        | IfUseResumableStateMachinesExpr g (thenExpr, _) ->
            if sm_verbose then
                printfn "eliminating 'if __useResumableCode...'"
    ...
    let rec TryReduceApp (env: env) expr (args: Expr list) =
        if isNil args then
            ...
            | Expr.TyLambda _
            | Expr.Lambda _ ->
                let macroTypars, macroParamsCurried, macroBody, _rty =
                    stripTopLambda (expr, tyOfExpr g expr)
                ...
                if not (isNil macroTypars) then
                    ...
                    let macroVal2 =
                        mkLambdas g m macroTypars macroParams (macroBody, tyOfExpr g macroBody)
                    ...
                    if args.Length < macroParams.Length then
                        ...
                        let expandedExpr =
                            MakeApplicationAndBetaReduce g (macroVal2, (tyOfExpr g macroVal2), [], nowArgs, m)

                        if sm_verbose then
                            printfn "reduced application f = %A nowArgs= %A --> %A" macroVal2 nowArgs expandedExpr

                        if isNil laterArgs then
                            Some expandedExpr
                        else
                            if sm_verbose then
                                printfn "application was partial, reducing further args %A" laterArgs
            ...
            | Expr.Const(Const.Zero, m, ty) -> Some(Expr.Const(Const.Zero, m, ty))
            ...
            | Expr.Match(spBind, mExpr, dtree, targets, m, ty) ->
                ...
                let targets2 =
                    targets
                    |> Array.choose (fun (TTarget(vs, targetExpr, flags)) ->
                        ...
                        let targetExpr2Opt =
                            ...
                            | Expr.Op(TOp.ILAsm([ I_throw ], [ _oldTy ]), a, b, c), Some newTy ->
                                ...
                                Some targetExpr2
                            | Expr.Sequential(DebugPoints((Expr.Op(TOp.ILCall(_, _, _, _, _, _, _, ilMethodRef, _, _, _), _, _, _) as e1),
                                                          rebuild1),
                                              Expr.Const(Const.Zero, m, _oldTy),
                                              a,
                                              c),
                              Some newTy when ilMethodRef.Name = "Throw" ->
                                let targetExpr2 =
                                    Expr.Sequential(e1, rebuild1 (Expr.Const(Const.Zero, m, newTy)), a, c)
    ...
    let rec TryReduceExpr (env: env) expr args remake =
        if sm_verbose then
            printfn "expanding defns and reducing %A..." expr
        ...
        | Expr.Val(defnRef, _, _) when env.ResumableCodeDefns.ContainsVal defnRef.Deref ->
            ...
            if sm_verbose then
                printfn "found resumable code %A --> %A" defnRef defn
            // Expand the resumable code definition
            match TryReduceApp env defn args with
            | Some expandedExpr ->
                if sm_verbose then
                    printfn "expanded resumable code %A --> %A..." defnRef expandedExpr

                Some expandedExpr
            | None -> Some(remake defn)

        // defn.Invoke x --> let arg = x in [defn][arg/x]
        | ResumableCodeInvoke g (_, f, args2, _, rebuild) ->
            if sm_verbose then
                printfn "found delegate invoke in possible reduction, f = %A, args now %A..." f (args2 @ args)
        ...
        | Expr.App(f, _fty, _tyargs, args2, _m) ->
            if sm_verbose then
                printfn "found function invoke in possible reduction, f = %A, args now %A..." f (args2 @ args)
        ...
        | _ ->
            ...
            | Some expandedExpr ->
                if sm_verbose then
                    printfn "reduction = %A, args = %A --> %A..." expr args expandedExpr
    ...
    let ConvertStateMachineLeafExpression (env: env) expr =
        if sm_verbose then
            printfn "ConvertStateMachineLeafExpression for %A..." expr

        expr |> RewriteExpr(makeRewriteEnv env)

    let ConvertStateMachineLeafDecisionTree (env: env) expr =
        if sm_verbose then
            printfn "ConvertStateMachineLeafDecisionTree for %A..." expr
    ...
    let rec RepeatBindAndApplyOuterDefinitions (env: env) expr =
        if sm_verbose then
            printfn "RepeatBindAndApplyOuterDefinitions for %A..." expr
    ...
    let (|ExpandedStateMachineInContext|_|) inputExpr =
        ...
        match expr with
        | StructStateMachineExpr g (dataTy,
            ...
            let env =
                { env with
                    TemplateStructTy = Some templateStructTy
                }

            if sm_verbose then
                printfn "Found struct machine..."

            if sm_verbose then
                printfn "Found struct machine jump table call..."
    ...
    let addPcJumpTable m (pcs: int list) (pc2lab: Map<int, ILCodeLabel>) pcExpr expr =
        if pcs.IsEmpty then
            ...
            let mkGotoLabelTarget lab =
                mbuilder.AddResultTarget(Expr.Op(TOp.Goto lab, [], [], m))
    ...
    let rec ConvertResumableCode
        env
        (pcValInfo: ((Val * Expr) * Expr) option)
        expr
        : Result<StateMachineConversionFirstPhaseResult, string> =
        if sm_verbose then
        ...
        let res =
            match expr with
            | ResumableCodeInvoke g (_, _, _, m, _) -> Result.Error(FSComp.SR.reprResumableCodeInvokeNotReduced (!!m.ToString()))

            // Eliminate 'if __useResumableCode ...' within.
            | IfUseResumableStateMachinesExpr g (thenExpr, _) -> ConvertResumableCode env pcValInfo thenExpr
            ...
            | ResumeAtExpr g pcExpr -> ConvertResumableResumeAt env (pcExpr, expr.Range)
            ...
            | SequentialResumableCode g (e1, e2, _m, recreate) -> ConvertResumableSequential env pcValInfo (e1, e2, _m, recreate)

            // The expanded code for state machines may use while loops...
            | WhileExpr(sp1, sp2, guardExpr, bodyExpr, m) -> ConvertResumableWhile env pcValInfo (sp1, sp2, guardExpr, bodyExpr, m)
            ...
            | TryFinallyExpr(sp1, sp2, ty, e1, e2, m) -> ConvertResumableTryFinally env pcValInfo (sp1, sp2, ty, e1, e2, m)
            ...
            // control-flow match
            | Expr.Match(spBind, mExpr, dtree, targets, m, ty) -> ConvertResumableMatch env pcValInfo (spBind, mExpr, dtree, targets, m, ty)
            ...
            | Expr.Let(bind, bodyExpr, m, _) ->
                ...
                if
                    not bind.Var.IsCompiledAsTopLevel
                    && IsGenericValWithGenericConstraints g bind.Var
                then
                    if sm_verbose then
            ...
            | Expr.LetRec _ -> Result.Error(FSComp.SR.reprResumableCodeContainsLetRec ())

            | Expr.DebugPoint(dp, innerExpr) -> ConvertResumableDebugPoint env pcValInfo (dp, innerExpr)
        ...
        if sm_verbose then
            ...
            | Result.Ok res ->
                ...
                printfn
                    "Phase 1 Done, resumableVars = %A"
                    (res.resumableVars.FreeLocals
                     |> Zset.elements
                     |> List.map (fun v -> v.CompiledName(g.CompilerGlobalState))
                     |> String.concat ",")

                printfn
                    "Phase 1 Done, stateVars = %A"
                    (res.stateVars
                     |> List.map (fun v -> v.CompiledName(g.CompilerGlobalState))
                     |> String.concat ",")

                printfn
                    "Phase 1 Done, thisVars = %A"
                    (res.thisVars
                     |> List.map (fun v -> v.CompiledName(g.CompilerGlobalState))
                     |> String.concat ",")
    ...
    and ConvertResumableEntry env pcValInfo (noneBranchExpr, someVar, someBranchExpr, _rebuild) =
        if sm_verbose then
            printfn "ResumableEntryMatchExpr"
        // printfn "found sequential"
        let reenterPC = genPC ()

        let envSome =
            { env with
                ResumableCodeDefns = env.ResumableCodeDefns.Add someVar (mkInt g someVar.Range reenterPC)
            }
        ...
        | Result.Ok resNone, Result.Ok resSome ->
            let resumableVars =
                unionFreeVars (freeInExpr CollectLocals resNone.phase1) resSome.resumableVars

            let m = someBranchExpr.Range

            let recreate reenterLabOpt e1 e2 =
                let lab =
                    (match reenterLabOpt with
                     | Some l -> l
                     | _ -> generateCodeLabel ())
    ...
    and ConvertResumableResumeAt env (pcExpr, m) =
        ...
        | Int32Expr contIdPC ->
            ...
            |> Result.Ok
        | _ -> Result.Error(FSComp.SR.reprResumableCodeContainsDynamicResumeAtInBody ())

    and ConvertResumableSequential env pcValInfo (e1, e2, _m, recreate) =
        if sm_verbose then
            printfn "SequentialResumableCode"
        // printfn "found sequential"
        ...
        | Result.Ok resg, Result.Ok resb ->
            ...
            let resumableVars =
                if eps.IsEmpty then
                    ...
                    unionFreeVars (freeInExpr CollectLocals resg.phase1) (freeInExpr CollectLocals resb.phase1)
                ...
                phase2 =
                    (fun ctxt ->
                        ...
                        let ebR2 =
                            ...
                            | None -> ebR
                            | Some((pcVal, _), _) -> mkCompGenThenDoSequential m ebR (mkValSet m (mkLocalValRef pcVal) (mkZero g m))
    ...
    and ConvertResumableIntegerForLoop env pcValInfo (spFor, spTo, style, e1, e2, v, e3, m) =
                    ...
                    phase2 =
                        (fun ctxt ->
                            ...
                            let e3R2 =
                                match pcValInfo with
                                | None -> e3R
                                | Some((pcVal, _), _) -> mkCompGenThenDoSequential m e3R (mkValSet m (mkLocalValRef pcVal) (mkZero g m))
        ...
        | _, Result.Error err, _
        | _, _, Result.Error err -> Result.Error err
    ...
    and ConvertResumableTryWith env pcValInfo (spTry, spWith, resTy, bodyExpr, filterVar, filterExpr, handlerVar, handlerExpr, m) =
        ...
        | Result.Ok resBody, Result.Ok resFilter, Result.Ok resHandler ->
            ...
            if epsNope.Length > 0 then
                ...
                {
                    phase1 =
                        mkTryWith g (resBody.phase1, filterVar, resFilter.phase1, handlerVar, resHandler.phase1, m, resTy, spTry, spWith)
                    phase2 =
                        (fun ctxt ->
                            ...
                            if innerPcs.IsEmpty then
                                ...
                                let outerLabsForInnerPcs =
                                    pcsAndLabs
                                    |> List.filter (fun (pc, _outerLab) -> innerPcSet.Contains pc)
                                    |> List.map snd
                                // generate the inner labels
                                let pcsAndInnerLabs =
                                    pcsAndLabs
                                    |> List.map (fun (pc, l) -> (pc, if innerPcSet.Contains pc then generateCodeLabel () else l))
                                ...
                                let coreExpr =
                                    mkTryWith
                                        g
                                        (vBodyRWithJumpTable, filterVar, filterExprR, handlerVar, handlerExprR, m, resTy, spTry, spWith)
                                // Place all the outer labels just before the try
                                let labelledExpr =
                                    (coreExpr, outerLabsForInnerPcs) ||> List.fold (fun e l -> mkLabelled m l e)
                    ...
                    resumableVars =
                        unionFreeVars
                            resBody.resumableVars
                            (unionFreeVars (freeInExpr CollectLocals resFilter.phase1) (freeInExpr CollectLocals resHandler.phase1))
                }
                |> Result.Ok
        | Result.Error err, _, _
        | _, Result.Error err, _
    ...
    and ConvertResumableMatch env pcValInfo (spBind, mExpr, dtree, targets, m, ty) =
        ...
        let tglArray =
            targets
            |> Array.map (fun (TTarget(_vs, targetExpr, _)) -> ConvertResumableCode env pcValInfo targetExpr)
        ...
        | true ->
            ...
            let resumableVars =
                ...
                ||> Array.fold (fun fvs (TTarget(_vs, _, _), res) ->
                    if res.entryPoints.IsEmpty then
                        fvs
                    else
                        unionFreeVars fvs res.resumableVars)

            let stateVars =
                (targets, tglArray)
                ||> Array.zip
                |> Array.toList
                |> List.collect (fun (TTarget(vs, _, _), res) ->
                    let stateVars =
                        vs
                        |> List.filter (fun v -> res.resumableVars.FreeLocals.Contains(v))
...

            {
                phase1 =
                    let gtgs =
                        (targets, tglArray)
                        ||> Array.map2 (fun (TTarget(vs, _, _)) res ->
    ...
    and ConvertResumableLet env pcValInfo (bind, bodyExpr, m) =
        ...
        | Result.Ok resBody ->
            ...
            if
                isByrefTy g bind.Var.Type
                && (match env.TemplateStructTy with
                    | None -> false
                    | Some ty -> typeEquiv g ty (destByrefTy g bind.Var.Type))
            then
                RepresentBindingAsThis bind resBody m |> Result.Ok
            elif
                bind.Var.IsCompiledAsTopLevel
                || not (resBody.resumableVars.FreeLocals.Contains(bind.Var))
                || bind.Var.LogicalName.StartsWithOrdinal(stackVarPrefix)
            then
                if sm_verbose then
                    printfn "LetExpr (non-control-flow, rewrite rhs, RepresentBindingAsTopLevelOrLocal)"

                RepresentBindingAsTopLevelOrLocal bind resBody m |> Result.Ok
            else
                if sm_verbose then
                    printfn "LetExpr (non-control-flow, rewrite rhs, RepresentBindingAsStateVar)"
                // printfn "found state variable %s" bind.Var.DisplayName
                RepresentBindingAsStateVar g bind resBody m |> Result.Ok
    ...
    member _.Apply(overallExpr, altExprOpt) =
        ...
        let fallback msg =
            ...
            | None -> LoweredStateMachineResult.NoAlternative msg
            | Some altExpr -> LoweredStateMachineResult.UseAlternative(msg, altExpr)
                ...
                if frees |> Zset.exists (isExpandVar g) then
                    ...
                    | Result.Ok phase1 ->
                        ...
                        let moveNextExprWithJumpTable =
                            match pcValInfo with
                            | None -> moveNextExprR
                            | Some((v, pcValExprR), pcExprR) ->
                                mkCompGenLet m v pcExprR (addPcJumpTable m pcs pc2lab pcValExprR moveNextExprR)
