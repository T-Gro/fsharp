...
let DecideLambda exprF cenv valReprInfo expr exprTy z =
    ...
    | Expr.Lambda _
    | Expr.TyLambda _ ->
        let _tps, ctorThisValOpt, baseValOpt, vsl, body, _bodyty =
            destLambdaWithValReprInfo cenv.g cenv.amap valReprInfo (expr, exprTy)
...
let DecideExprOp exprF noInterceptF (z: Zset<Val>) (expr: Expr) (op, tyargs, args) =
    ...
    | TOp.While _, _, [ Expr.Lambda(_, _, _, [ _ ], e1, _, _); Expr.Lambda(_, _, _, [ _ ], e2, _, _) ] -> exprF (exprF z e1) e2

    | TOp.TryFinally _, [ _ ], [ Expr.Lambda(_, _, _, [ _ ], e1, _, _); Expr.Lambda(_, _, _, [ _ ], e2, _, _) ] -> exprF (exprF z e1) e2
...
let DecideExpr cenv exprF noInterceptF z expr =
    ...
    | Expr.Op(c, tyargs, args, _m) -> DecideExprOp exprF noInterceptF z expr (c, tyargs, args)
...
let DecideBindings cenv z binds =
    (z, binds) ||> List.fold (DecideBinding cenv)
