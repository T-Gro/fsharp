...
#if DEBUG
let braceL x =
    leftL (tagText "{") ^^ x ^^ rightL (tagText "}")

let seqL xL xs =
    Seq.fold (fun z x -> z @@ xL x) emptyL xs

let namemapL xL xmap =
    NameMap.foldBack (fun nm x z -> xL nm x @@ z) xmap emptyL
...
let rec exprValueInfoL g exprVal =
    ...
    | CurriedLambdaValue(_lambdaId, _arities, _bsize, expr, _ety) ->
        wordL (tagText "lam") ++ exprL expr (* (sprintf "lam(size=%d)" bsize) *)
    | ConstExprValue(_size, x) -> exprL x

and exprValueInfosL g vinfos =
    commaListL (List.map (exprValueInfoL g) (Array.toList vinfos))

and moduleInfoL g (x: LazyModuleInfo) =
    let x = x.Force()

    braceL (
        (wordL (tagText "Modules: ")
         @@ (x.ModuleOrNamespaceInfos
             |> namemapL (fun nm x -> wordL (tagText nm) ^^ moduleInfoL g x)))
        @@ (wordL (tagText "Values:")
            @@ (x.ValInfos.Entries |> seqL (fun (vref, x) -> valRefL vref ^^ valInfoL g x)))
    )

and valInfoL g (x: ValInfo) =
    braceL (
        (wordL (tagText "ValExprInfo: ") @@ exprValueInfoL g x.ValExprInfo)
        @@ (wordL (tagText "ValMakesNoCriticalTailcalls:")
            @@ wordL (tagText (if x.ValMakesNoCriticalTailcalls then "true" else "false")))
    )
#endif
...
let SizeOfValueInfo valueInfo =
    let rec loop acc valueInfo =
        ...
        | SizeValue(vdepth, _v) ->
            assert (vdepth >= 0)
            acc + vdepth // terminate recursion at CACHED size nodes
...
let rec MakeValueInfoWithCachedSize vdepth v =
    ...
    | _ ->
        if vdepth > minDepthForASizeNode then
            SizeValue(vdepth, v)
        else
            v (* add nodes to stop recursion *)
...
let BoundValueInfoBySize vinfo =
    let rec bound depth x =
        if depth < 0 then
            ...
            match x with
            | SizeValue(vdepth, vinfo) ->
                if vdepth < depth then
                    x
                else
                    MakeSizedValueInfo(bound depth vinfo)
    ...
    if vdepth > maxDepth then
        MakeSizedValueInfo(bound trimDepth vinfo)
    else
        MakeValueInfoWithCachedSize vdepth vinfo
...
[<Literal>]
let crossAssemblyOptimizationDefault = true

[<Literal>]
let debugPointsForPipeRightDefault = true
...
type OptimizationSettings =
    ...
    member x.JitOptimizationsEnabled =
        match x.jitOptUser with
        | Some f -> f
        | None -> jitOptDefault

    /// Determines if intra-assembly optimization is enabled
    member x.LocalOptimizationsEnabled =
        match x.localOptUser with
        | Some f -> f
        | None -> localOptDefault
    ...
    member x.crossAssemblyOpt() =
        ...
        && x.crossAssemblyOptimizationUser
           |> Option.defaultValue crossAssemblyOptimizationDefault
...
let rec IsPartialExprVal x =
    ...
    | TupleValue args
    | RecdValue(_, args)
    | UnionCaseValue(_, args) -> Array.exists IsPartialExprVal args
    | ConstValue _
    | CurriedLambdaValue _
    | ConstExprValue _ -> false
    | ValValue(_, a)
...
let EmptyModuleInfo =
    notlazy
        {
            ValInfos = ValInfos([])
            ModuleOrNamespaceInfos = Map.empty
        }
                ...
                ValInfos(
                    seq {
                        for minfo in minfos do
                            yield! minfo.Force().ValInfos.Entries
                    }
                )
...
let rec BindValueInSubModuleFSharpCore (mp: string[]) i (v: Val) vval ss =
    if i < mp.Length then
        { ss with
            ModuleOrNamespaceInfos = BindValueInModuleForFslib mp[i] mp (i + 1) v vval ss.ModuleOrNamespaceInfos
        }
    else
        // REVIEW: this line looks quadratic for performance when compiling FSharp.Core
        { ss with
            ValInfos = ValInfos(Seq.append ss.ValInfos.Entries (Seq.singleton (mkLocalValRef v, vval)))
        }
...
let BindValueForFslib (nlvref: NonLocalValOrMemberRef) v vval env =
    { env with
        globalModuleInfos =
            BindValueInGlobalModuleForFslib nlvref.AssemblyName nlvref.EnclosingEntity.nlr.Path 0 v vval env.globalModuleInfos
    }

let UnknownValInfo =
    {
        ValExprInfo = UnknownValue
        ValMakesNoCriticalTailcalls = false
    }

let mkValInfo info (v: Val) =
    {
        ValExprInfo = info.Info
        ValMakesNoCriticalTailcalls = v.MakesNoCriticalTailcalls
    }
...
let BindExternalLocalVal cenv (v: Val) vval env =
    ...
    let vval =
        if v.IsMutable then
            { vval with ValExprInfo = UnknownValue }
        else
            vval
    ...
    let env =
        ...
        | _ ->
            { env with
                localExternalVals = env.localExternalVals.Add(v.Stamp, vval)
            }
...
let rec BindValsInModuleOrNamespace cenv (mval: LazyModuleInfo) env =
    ...
    let env =
        (mval.ModuleOrNamespaceInfos, env)
        ||> NameMap.foldBackRange (BindValsInModuleOrNamespace cenv)

    let env =
        (env, mval.ValInfos.Entries)
        ||> Seq.fold (fun env (v: ValRef, vval) -> BindExternalLocalVal cenv v.Deref vval env)
...
let BindTypar tyv typeinfo env =
    { env with
        typarInfos = (tyv, typeinfo) :: env.typarInfos
    }
...
let BindTyparsToUnknown (tps: Typar list) env =
    if isNil tps then
        ...
        let nms =
            PrettyTypes.PrettyTyparNames (fun _ -> true) (env.typarInfos |> List.map (fun (tp, _) -> tp.Name)) tps
...
let TryGetInfoForCcu env (ccu: CcuThunk) =
    env.globalModuleInfos.TryFind(ccu.AssemblyName)
...
let IsPartialExpr cenv env m x =
    let rec isPartialExpression x =
        ...
        | Expr.LetRec(bindings, body, _, _) ->
            body :: (bindings |> List.map (fun (TBind(_, expr, _)) -> expr))
            |> List.exists isPartialExpression
...
let MakeValueInfoForValue g m vref vinfo =
    ...
    let rec check x =
        ...
        | ValValue(vref2, detail) ->
            if valRefEq g vref vref2 then
                error (Error(FSComp.SR.optRecursiveValValue (showL (exprValueInfoL g vinfo)), m))
            else
                check detail
...
let mkAssemblyCodeValueInfo g instrs argvals tys =
    ...
    | [], [ v ], [ ty ] ->
        ...
        | ConstValue(Const.Bool a, _) ->
            if typeEquiv g ty g.bool_ty then
                v
            elif typeEquiv g ty g.sbyte_ty then
                mkInt8Val g (if a then 1y else 0y)
            elif typeEquiv g ty g.int16_ty then
                mkInt16Val g (if a then 1s else 0s)
            elif typeEquiv g ty g.int32_ty then
                mkInt32Val g (if a then 1 else 0)
            elif typeEquiv g ty g.byte_ty then
                mkUInt8Val g (if a then 1uy else 0uy)
            elif typeEquiv g ty g.uint16_ty then
                mkUInt16Val g (if a then 1us else 0us)
            elif typeEquiv g ty g.uint32_ty then
                mkUInt32Val g (if a then 1u else 0u)
            else
                UnknownValue
        | ConstValue(Const.SByte a, _) ->
            if typeEquiv g ty g.sbyte_ty then
                v
            elif typeEquiv g ty g.int16_ty then
                mkInt16Val g (Unchecked.int16 a)
            elif typeEquiv g ty g.int32_ty then
                mkInt32Val g (Unchecked.int32 a)
            else
                UnknownValue
        | ConstValue(Const.Byte a, _) ->
            if typeEquiv g ty g.byte_ty then
                v
            elif typeEquiv g ty g.uint16_ty then
                mkUInt16Val g (Unchecked.uint16 a)
            elif typeEquiv g ty g.uint32_ty then
                mkUInt32Val g (Unchecked.uint32 a)
            else
                UnknownValue
        | ConstValue(Const.Int16 a, _) ->
            if typeEquiv g ty g.int16_ty then
                v
            elif typeEquiv g ty g.int32_ty then
                mkInt32Val g (Unchecked.int32 a)
            else
        ...
        | ConstValue(Const.UInt16 a, _) ->
            if typeEquiv g ty g.uint16_ty then
                v
            elif typeEquiv g ty g.uint32_ty then
                mkUInt32Val g (Unchecked.uint32 a)
            else
        ...
        | ConstValue(Const.UInt32 a, _) ->
            if typeEquiv g ty g.uint32_ty then
                v
            elif typeEquiv g ty g.int32_ty then
                mkInt32Val g (Unchecked.int32 a)
            else
        ...
        | ConstValue(Const.Int64 a, _) ->
            if typeEquiv g ty g.int64_ty then
                ...
                mkUInt64Val g (Unchecked.uint64 a)
            else
                UnknownValue
        | ConstValue(Const.UInt64 a, _) ->
            if typeEquiv g ty g.uint64_ty then
                v
            elif typeEquiv g ty g.int64_ty then
                mkInt64Val g (Unchecked.int64 a)
            else
                UnknownValue
        | _ -> UnknownValue
    | _ -> UnknownValue
...
let AddFunctionSizes l =
    l |> List.sumBy (fun x -> x.FunctionSize)

/// list/array combinators - zipping (_, _) return type
let OrEffects l = List.exists (fun x -> x.HasEffect) l

let OrTailcalls l =
    List.exists (fun x -> x.MightMakeCriticalTailcall) l
    ...
    let rec abstractExprInfo ivalue =
        ...
        | ValValue(vref2, detail) ->
            ...
            then
                ...
                ValValue(vref2, detailR)

        // Check for escape in lambda
        | CurriedLambdaValue(_, _, _, expr, _)
        | ConstExprValue(_, expr) when
        ...
        | RecdValue(tcref, vinfos) ->
            if
                hiddenTyconRepr tcref.Deref
                || Array.exists (tcref.MakeNestedRecdFieldRef >> hiddenRecdField) tcref.AllFieldsArray
            then
                UnknownValue
            else
                RecdValue(tcref, Array.map abstractExprInfo vinfos)

        | UnionCaseValue(ucref, vinfos) ->
            let tcref = ucref.TyconRef

            if
                hiddenTyconRepr ucref.Tycon
                || tcref.UnionCasesArray
                   |> Array.exists (tcref.MakeNestedUnionCaseRef >> hiddenUnionCase)
            then
                UnknownValue
            else
                UnionCaseValue(ucref, Array.map abstractExprInfo vinfos)
...
let AbstractOptimizationInfoToEssentials =
    ...
    and abstractLazyModulInfo ss =
        ss |> InterruptibleLazy.force |> abstractModulInfo |> notlazy
...
let AbstractExprInfoByVars (boundVars: Val list, boundTyVars) ivalue =
    ...
    | _ ->
        ...
        let rec abstractExprInfo ivalue =
            ...
            | ValValue(VRefLocal v2, detail) when
                ...
                || (not (isNil boundTyVars)
                    && let ftyvs = freeInVal CollectTypars v2 in
            ...
            | ConstExprValue(_, expr) when
                (let fvs =
                 ...
                 || (not (isNil boundTyVars)
                     && List.exists (Zset.memberOf fvs.FreeTyvars.FreeTypars) boundTyVars)
        ...
        let rec abstractModulInfo ss =
            {
                ModuleOrNamespaceInfos =
                    ss.ModuleOrNamespaceInfos
                    |> NameMap.map (InterruptibleLazy.force >> abstractModulInfo >> notlazy)
                ValInfos = ss.ValInfos.Map(fun (vref, e) -> check vref (abstractValInfo e))
            }
...
let RemapOptimizationInfo g tmenv =
    ...
    let rec remapExprInfo ivalue =
        ...
        | CurriedLambdaValue(uniq, arity, sz, expr, ty) ->
            CurriedLambdaValue(uniq, arity, sz, remapExpr g CloneAll tmenv expr, remapPossibleForallTy g tmenv ty)
    ...
    let rec remapModulInfo ss =
        {
            ...
            ValInfos =
                ss.ValInfos.Map(fun (vref, vinfo) ->
                    ...
                    if vinfo.ValMakesNoCriticalTailcalls then
                        vrefR.Deref.SetMakesNoCriticalTailcalls()
...
let IlAssemblyCodeInstrHasEffect i =
    match i with
    | (AI_nop | AI_ldc _ | AI_add | AI_sub | AI_mul | AI_xor | AI_and | AI_or | AI_ceq | AI_cgt | AI_cgt_un | AI_clt | AI_clt_un | AI_conv _ | AI_shl | AI_shr | AI_shr_un | AI_neg | AI_not | AI_ldnull)
    | I_ldstr _
    | I_ldtoken _ -> false
    | _ -> true

let IlAssemblyCodeHasEffect instrs =
    List.exists IlAssemblyCodeInstrHasEffect instrs
...
let TryEliminateBinding cenv _env bind e2 _m =
    ...
    if
        not (cenv.optimizing && cenv.settings.EliminateImmediatelyConsumedLocals())
        && not vspec1.IsCompilerGenerated
    then
        None
    elif vspec1.IsFixed then
        None
    elif
        vspec1.LogicalName.StartsWithOrdinal stackVarPrefix
        || vspec1.LogicalName.Contains suffixForVariablesThatMayNotBeEliminated
    then
        None
    else
        ...
        let rec GetImmediateUseContext rargsl argsr =
            match argsr with
            | Expr.Val(VRefLocal vspec2, _, _) :: argsr2 when valEq vspec1 vspec2 && IsUniqueUse vspec2 (List.rev rargsl @ argsr2) ->
                Some(List.rev rargsl, argsr2)
        ...
        | Expr.Val(VRefLocal vspec2, _, _) when IsUniqueUse vspec2 [] -> Some(e1 |> recreate0)
        ...
        | Expr.Sequential(DebugPoints(Expr.App(DebugPoints(Expr.Val(VRefLocal vspec2, _, _), recreate2), f0ty, c, args, d), recreate1),
                          rest,
                          NormalSeq,
                          m) when IsUniqueUse vspec2 (rest :: args) ->
            Some(
                Expr.Sequential(recreate1 (Expr.App(recreate2 e1, f0ty, c, args, d)), rest, NormalSeq, m)
                |> recreate0
            )

        // Immediate consumption of delegate via an application in a sequential, e.g. 'let part1 = e in part1.Invoke(args); rest'
        // See https://github.com/fsharp/fslang-design/blob/master/tooling/FST-1034-lambda-optimizations.md
        | Expr.Sequential(DebugPoints(DelegateInvokeExpr g (delInvokeRef,
                                                            delInvokeTy,
                                                            tyargs,
                                                            DebugPoints(Expr.Val(VRefLocal vspec2, _, _), recreate2),
                                                            delInvokeArg,
                                                            _),
                                      recreate1),
                          rest,
                          NormalSeq,
                          m) when IsUniqueUse vspec2 [ rest; delInvokeArg ] ->
            let invoke =
                MakeFSharpDelegateInvokeAndTryBetaReduce g (delInvokeRef, recreate2 e1, delInvokeTy, tyargs, delInvokeArg, m)
        ...
        | Expr.Match(spMatch, _exprm, TDSwitch(DebugPoints(Expr.Val(VRefLocal vspec2, _, _), recreate1), cases, dflt, _), targets, m, ty2) when
            (valEq vspec1 vspec2
             && let fvs =
                 accFreeInTargets CollectLocals targets (accFreeInSwitchCases CollectLocals cases dflt emptyFreeVars) in
            ...
            Some(
                Expr.Match(spMatch, e1.Range, TDSwitch(recreate1 e1, cases, dflt, m), targets, m, ty2)
                |> recreate0
            )
        ...
        | Expr.App(f, f0ty, tyargs, args, m) ->
            ...
            | Some(f :: largs, rargs) ->
                Some(
                    MakeApplicationAndBetaReduce g (f, f0ty, [ tyargs ], largs @ (e1 :: rargs), m)
                    |> recreate0
                )
            | None -> None
        ...
        | Expr.Op(c1, tyargs1, [ DebugPoints(Expr.Op(c2, tyargs2, [ arg1; arg2 ], m2), recreate1); arg3 ], m1) ->
            ...
            | Some([], [ arg2; arg3 ]) ->
                Some(
                    Expr.Op(c1, tyargs1, [ Expr.Op(c2, tyargs2, [ e1; arg2 ], m2) |> recreate1; arg3 ], m1)
                    |> recreate0
                )
            | Some([ arg1 ], [ arg3 ]) ->
                Some(
                    Expr.Op(c1, tyargs1, [ Expr.Op(c2, tyargs2, [ arg1; e1 ], m2) |> recreate1; arg3 ], m1)
                    |> recreate0
                )
            | Some([ arg1; arg2 ], []) ->
                Some(
                    Expr.Op(c1, tyargs1, [ Expr.Op(c2, tyargs2, [ arg1; arg2 ], m2) |> recreate1; e1 ], m1)
                    |> recreate0
                )
...
let rec (|KnownValApp|_|) expr =
    ...
    | Expr.App(KnownValApp(vref, typeArgs1, otherArgs1), _, typeArgs2, otherArgs2, _) ->
        ValueSome(vref, typeArgs1 @ typeArgs2, otherArgs1 @ otherArgs2)
    | _ -> ValueNone
...
let rec CountBoolLogicTree (targets: DecisionTreeTarget[], costOuterCaseTree, costOuterDefaultTree, testBool as data) tree =
    ...
    | TDSuccess([], idx) ->
        ...
        | ConstantBoolTarget result ->
            (if result = testBool then
                 costOuterCaseTree
             else
                 costOuterDefaultTree),
            0
    ...
    | TDSuccess([], idx) ->
        ...
        | TTarget([], exp, _) ->
            mkBoolSwitch
                exp.Range
                exp
                (if testBool then outerCaseTree else outerDefaultTree)
                (if testBool then outerDefaultTree else outerCaseTree)
...
let rec CombineBoolLogic expr =
    ...
    | Expr.Match(outerSP,
                 outerMatchRange,
                 TDBoolSwitch(DebugPoints(Expr.Match(_innerSP, _innerMatchRange, innerTree, innerTargets, _innerDefaultRange, _innerMatchTy),
                                          _),
                              outerTestBool,
                              outerCaseTree,
                              outerDefaultTree,
        ...
        let costOuterCaseTree =
            match outerCaseTree with
            | TDSuccess _ -> 0
            | _ -> 1

        let costOuterDefaultTree =
            match outerDefaultTree with
            | TDSuccess _ -> 0
            | _ -> 1

        let tc, ec =
            CountBoolLogicTree (innerTargets, costOuterCaseTree, costOuterDefaultTree, outerTestBool) innerTree
...
let ExpandStructuralBindingRaw cenv expr =
    ...
    match expr with
    | Expr.Let(TBind(v, rhs, tgtSeqPtOpt), body, m, _) when (isRefTupleExpr rhs && CanExpandStructuralBinding v) ->
...
let TryRewriteBranchingTupleBinding g (v: Val) rhs tgtSeqPtOpt body m =
    let rec dive g m (requisites: Lazy<_>) expr =
        ...
        | Expr.Match(sp, inputRange, decision, targets, fullRange, ty) ->
            ...
            let rewrittenTargets =
                targets
                |> Array.choose (fun (TTarget(vals, targetExpr, flags)) ->
        ...
        | Expr.Op(TOp.Tuple tupInfo, _, tupleElements, m) when not (evalTupInfoIsStruct tupInfo) ->
            ...
            List.map2 (mkValSet m) vrefs tupleElements |> mkSequentials g m |> Some
    ...
    let requisites =
        lazy
            (let argTys = destRefTupleTy g v.Type
             ...
             let ves, binds =
                 List.mapi2 (MakeMutableStructuralBindingForTupleElement v) inits argTys
                 |> List.unzip
    ...
    | Some rewrittenRhs ->
        ...
        let rhsAndTupleBinding =
            mkCompGenSequential m rewrittenRhs (mkLet tgtSeqPtOpt m v (mkRefTupled g m ves argTys) body)
...
let ExpandStructuralBinding cenv expr =
    ...
    match expr with
    | Expr.Let(TBind(v, rhs, tgtSeqPtOpt), body, m, _) when
        (isRefTupleTy g v.Type
        ...
        | None ->
            ...
            TryRewriteBranchingTupleBinding g v rhs tgtSeqPtOpt body m
    ...
    | Expr.Let(TBind(v, Expr.Op(TOp.UnionCase uc, _, [ arg ], _), tgtSeqPtOpt), body, m, _) when
        isOptionTy g v.Type
        && not (ExprIsValue arg)
...
let (|QueryRun|_|) g expr =
    match expr with
    | Expr.App(Expr.Val(vref, _, _), _, _, [ _builder; arg ], _) when valRefEq g vref g.query_run_value_vref -> ValueSome(arg, None)
...
let (|QueryFor|_|) g =
    function
    | InstanceMethodApp g g.query_for_vref ([ srcTy; qTy; resTy; _qInnerTy ], _builder, [ src; selector ]) ->
        ValueSome(qTy, srcTy, resTy, src, selector)
    | _ -> ValueNone
...
let (|QuerySelect|_|) g =
    function
    | InstanceMethodApp g g.query_select_vref ([ srcTy; qTy; resTy ], _builder, [ src; selector ]) ->
        ValueSome(qTy, srcTy, resTy, src, selector)
    | _ -> ValueNone
...
let (|AnyRefTupleTrans|) e =
    match e with
    | Expr.Op(TOp.Tuple tupInfo, tys, es, m) when not (evalTupInfoIsStruct tupInfo) ->
        (es, (fun es -> Expr.Op(TOp.Tuple tupInfo, tys, es, m)))
    | _ ->
        [ e ],
        (function
        | [ e ] -> e
...
[<return: Struct>]
let (|AnyQueryBuilderOpTrans|_|) g =
    function
    | Expr.App(Expr.Val(vref, _, _) as v, vty, tyargs, [ builder; AnyRefTupleTrans(src :: rest, replaceArgs) ], m) when
        (match vref.ApparentEnclosingEntity with
         | Parent tcref -> tyconRefEq g tcref g.query_builder_tcref
         | ParentNone -> false)
        ->
...
let rec tryRewriteToSeqCombinators g (e: Expr) =
    ...
    | QueryFor g (_qTy,
                  sourceElemTy,
                  resultElemTy,
                  QuerySourceEnumerable g (_, source),
                  Expr.Lambda(_, _, _, [ resultSelectorVar ], resultSelector, mLambda, _)) ->
        ...
        | Some newResultSelector ->
            Some(
                mkCallSeqCollect
                    ...
                    sourceElemTy
                    resultElemTy
                    (mkLambda mLambda resultSelectorVar (newResultSelector, tyOfExpr g newResultSelector))
                    source
            )
        | _ -> None
    ...
    | Expr.Let(bind, bodyExpr, m, _) ->
        ...
        | Some newBodyExpr -> Some(Expr.Let(bind, newBodyExpr, m, newCache ()))
        | None -> None
    ...
    | Expr.Match(spBind, mExpr, pt, targets, m, _ty) ->
        let targets =
            targets
            |> Array.map (fun (TTarget(vs, e, flags)) ->
    ...
    | QueryRun g (bodyOfRun, reqdResultInfo) ->
        ...
        | Expr.Quote(quotedExpr, _, true, _, _) -> // true = isFromQueryExpression
            ...
            let rec loopOuter (e: Expr) =
                ...
                | QueryYieldFrom g (qTy, resultElemTy, _) when typeEquiv g qTy (mkWoNullAppTy g.tcref_System_Collections_IEnumerable []) ->
                    ...
                    | Some newSource ->
                        ...
                        Some(
                            mkCallSeq
                                ...
                                newSource.Range
                                resultElemTy
                                (mkCallSeqDelay g newSource.Range resultElemTy (mkUnitDelayLambda g newSource.Range newSource)),
            ...
            | Some(resultExpr, exprIsEnumerableInfo) ->
                let resultExprAfterConvertToResultTy =
                    ...
                    | Some _, Some _
                    | None, None -> resultExpr // the expression is a QuerySource, the result is a QuerySource, nothing to do
                    | Some resultElemTy, None ->
                        let enumerableTy =
                            TType_app(g.tcref_System_Collections_IEnumerable, [], g.knownWithoutNull)
...
let IsILMethodRefSystemStringConcat (mref: ILMethodRef) =
    ...
    && (mref.ArgCount >= 2
        && mref.ArgCount <= 4
        && mref.ArgTypes
           |> List.forall (fun ilTy -> ilTy.IsNominal && ilTy.TypeRef.Name = "System.String"))
    ...
    && (mref.ArgCount = 1
        && mref.ArgTypes
           |> List.forall (fun ilTy ->
               match ilTy with
               | ILType.Array(shape, ilTy) when
                   shape = ILArrayShape.SingleDimensional
                   && ilTy.IsNominal
                   && ilTy.TypeRef.Name = "System.String"
                   ->
                   true
               | _ -> false))
...
let inline IsStateMachineExpr g overallExpr =
    ...
    | Expr.App(funcExpr = Expr.Val(valRef = valRef)) -> isReturnsResumableCodeTy g valRef.TauType
    | _ -> false
...
let rec OptimizeExpr cenv (env: IncrementalOptimizationEnv) expr =
    ...
    <| fun () ->
        ...
        if IsDebugPipeRightExpr cenv expr then
            ...
            let env =
                { env with
                    disableMethodSplitting = env.disableMethodSplitting || isStateMachineE
                }
            ...
            | Expr.Sequential _
            | Expr.DebugPoint _
            | Expr.Let _ -> OptimizeLinearExpr cenv env expr id
            ...
            | Expr.Quote(ast, splices, isFromQueryExpression, m, ty) ->
                let doData data =
                    map3Of4 (List.map (OptimizeExpr cenv env >> fst)) data
            ...
            | Expr.Obj(_, ty, basev, createExpr, overrides, iimpls, m) ->
                match expr with
                | NewDelegateExpr g (lambdaId, vsl, body, _, remake) -> OptimizeNewDelegateExpr cenv env (lambdaId, vsl, body, remake)
                | _ -> OptimizeObjectExpr cenv env (ty, basev, createExpr, overrides, iimpls, m)

            | Expr.Op(op, tyargs, args, m) -> OptimizeExprOp cenv env (op, tyargs, args, m)
            ...
            | Expr.TyChoose _ -> OptimizeExpr cenv env (ChooseTyparSolutionsForFreeChoiceTypars g cenv.amap expr)

            | Expr.Match(spMatch, mExpr, dtree, targets, m, ty) -> OptimizeMatch cenv env (spMatch, mExpr, dtree, targets, m, ty)

            | Expr.LetRec(binds, bodyExpr, m, _) -> OptimizeLetRec cenv env (binds, bodyExpr, m)

            | Expr.StaticOptimization(staticConditions, expr2, expr3, m) ->
                let d = DecideStaticOptimizations g staticConditions false

                if d = StaticOptimizationAnswer.Yes then
                    OptimizeExpr cenv env expr2
                elif d = StaticOptimizationAnswer.No then
                    OptimizeExpr cenv env expr3
                else
...
and OptimizeObjectExpr cenv env (ty, baseValOpt, basecall, overrides, iimpls, m) =
    ...
    exprR,
    {
        TotalSize =
            closureTotalSize
            + basecallinfo.TotalSize
            + AddTotalSizes overrideinfos
            + AddTotalSizes iimplsinfos
...
and MakeOptimizedSystemStringConcatCall cenv env m args =
    ...
    let rec optimizeArg argExpr accArgs =
        ...
        | Expr.Op(TOp.ILCall(_, _, _, _, _, _, _, ilMethRef, _, _, _), _, [ Expr.Op(TOp.Array, _, args, _) ], _), _ when
            IsILMethodRefSystemStringConcatArray ilMethRef
            ->
            optimizeArgs args accArgs

        | Expr.Op(TOp.ILCall(_, _, _, _, _, _, _, ilMethRef, _, _, _), _, args, _), _ when IsILMethodRefSystemStringConcat ilMethRef ->
            optimizeArgs args accArgs
    ...
    match expr with
    | Expr.Op(TOp.ILCall(_, _, _, _, _, _, _, ilMethRef, _, _, _) as op, tyargs, args, m) when
        IsILMethodRefSystemStringConcat ilMethRef
        || IsILMethodRefSystemStringConcatArray ilMethRef
        ->
        OptimizeExprOpReductions cenv env (op, tyargs, args, m)
    | _ -> OptimizeExpr cenv env expr
...
and OptimizeExprOp cenv env (op, tyargs, args, m) =
    ...
    | TOp.ILCall(_, _, _, _, _, _, _, ilMethRef, _, _, _), _, [ arg ] when
        (ilMethRef.DeclaringTypeRef.Name = g.ilg.typ_Array.TypeRef.Name
    ...
    | TOp.ILCall(_, _, _, _, _, _, _, ilMethRef, _, _, _), _, [ Expr.Op(TOp.Array, _, args, _) ] when
        IsILMethodRefSystemStringConcatArray ilMethRef
        ->
        MakeOptimizedSystemStringConcatCall cenv env m args
    | TOp.ILCall(_, _, _, _, _, _, _, ilMethRef, _, _, _), _, args when IsILMethodRefSystemStringConcat ilMethRef ->
        MakeOptimizedSystemStringConcatCall cenv env m args
    ...
    let cost, value_ =
        ...
        | TOp.Tuple tupInfo ->
            ...
            if isStruct then
                ...
                1, MakeValueInfoForTuple(Array.ofList argValues)

        | TOp.AnonRecd anonInfo ->
            let isStruct = evalAnonInfoIsStruct anonInfo
            if isStruct then 0, value_ else 1, value_
        ...
        | TOp.ILAsm(instrs, retTypes) -> min instrs.Length 1, mkAssemblyCodeValueInfo g instrs argValues retTypes
        ...
        | TOp.Recd(ctorInfo, tcref) ->
            ...
            let value_ =
                ...
                | RecdExpr ->
                    if argValues.Length <> finfos.Length then
                        value_
                    else
                        MakeValueInfoForRecord
                            tcref
                            (Array.ofList (
                                (argValues, finfos)
                                ||> List.map2 (fun x f -> if f.IsMutable then UnknownValue else x)
                            ))
        ...
        | TOp.Return ->
            assert false
            error (InternalError("unexpected goto/label/return in optimization", m))
...
and OptimizeConst cenv env expr (c, m, ty) =
    ...
    | Some e -> OptimizeExpr cenv env e
    | None ->
        expr,
        {
            TotalSize =
...
and TryOptimizeRecordFieldGet cenv _env (e1info, (RecdFieldRef(rtcref, _) as r), _tinst, m) =
    ...
    | Some finfos when cenv.settings.EliminateRecdFieldGet && not e1info.HasEffect ->
        ...
        | None ->
            ...
            if n >= finfos.Length then
                errorR (InternalError("TryOptimizeRecordFieldGet: term argument out of range", m))
...
and TryOptimizeTupleFieldGet cenv _env (_tupInfo, e1info, tys, n, m) =
    ...
    | Some tups when cenv.settings.EliminateTupleFieldGet && not e1info.HasEffect ->
        ...
        if len <> tys.Length then
            errorR (InternalError("error: tuple lengths don't match", m))

        if n >= len then
            errorR (InternalError("TryOptimizeTupleFieldGet: tuple index out of range", m))
...
and TryOptimizeUnionCaseGet cenv _env (e1info, cspec, _tys, n, m) =
    ...
    | StripUnionCaseValue(cspec2, args) when
        cenv.settings.EliminateUnionCaseFieldGet()
        && not e1info.HasEffect
        && g.unionCaseRefEq cspec cspec2
        ->
        if n >= args.Length then
            errorR (InternalError("TryOptimizeUnionCaseGet: term argument out of range", m))
...
and OptimizeFastIntegerForLoop cenv env (spFor, spTo, v, e1, dir, e2, e3, m) =
    ...
    let e2R, dir =
        ...
        | FSharpForLoopUp,
          Expr.Op(TOp.ILAsm([ (AI_sub | AI_sub_ovf) ], _),
                  _,
                  [ Expr.Op(TOp.ILAsm([ I_ldlen; (AI_conv DT_I4) ], _), _, [ arre ], _); Expr.Const(Const.Int32 1, _, _) ],
                  _) when not (snd (OptimizeExpr cenv env arre)).HasEffect ->
        ...
        | FSharpForLoopUp,
          Expr.Op(TOp.ILAsm([ (AI_sub | AI_sub_ovf) ], _),
                  _,
                  [ Expr.Op(TOp.ILCall(_, _, _, _, _, _, _, mth, _, _, _), _, [ arre ], _) as lenOp; Expr.Const(Const.Int32 1, _, _) ],
                  _) when
            mth.Name = "get_Length"
            && (mth.DeclaringTypeRef.FullName = "System.Span`1"
                || mth.DeclaringTypeRef.FullName = "System.ReadOnlySpan`1")
        ...
        | FSharpForLoopUp, Expr.Const(Const.Int32 n, _, _) when n < System.Int32.MaxValue -> mkIncr g e2R.Range e2R, CSharpForLoopUp
    ...
    if cenv.settings.EliminateForLoop && not eff then
        ...
        exprR,
        {
            TotalSize = AddTotalSizes einfos + forAndWhileLoopSize
...
and OptimizeLetRec cenv env (binds, bodyExpr, m) =
    ...
    let bindsRR, bindinfos =
        ...
        let fvs =
            List.fold (fun acc x -> unionFreeVars acc (fst x |> freeInBindingRhs CollectLocals)) fvs0 bindsR
...
and OptimizeLinearExpr cenv env expr contf =
    ...
    let expr =
        if cenv.settings.ExpandStructuralValues() then
            ExpandStructuralBinding cenv expr
        else
            expr
    ...
    | _ ->
                 ...
                 << (fun (e2R, e2info) ->
                     if
                         ...
                         (cenv.settings.EliminateSequential
                          || (match stripDebugPoints e1R with
                     ...
                     then
                         ...
                         Expr.Sequential(e1R, e2R, flag, m),
                         {
                             ...
                             MightMakeCriticalTailcall =
                                 (if flag = NormalSeq then
                                      e2info.MightMakeCriticalTailcall
                                  else
                                      e1info.MightMakeCriticalTailcall || e2info.MightMakeCriticalTailcall)
            ...
            OptimizeLinearExpr
                ...
                (contf
                 << (fun (bodyR, bodyInfo) ->
                     ...
                     if
                         ValueIsUsedOrHasEffect
                             cenv
                             (fun () -> (freeInExpr (CollectLocalsWithStackGuard()) bodyR).FreeLocals)
                             (bindR, bindingInfo)
                     then
            ...
            OptimizeLinearExpr
                ...
                (contf
                 << (fun (e2, e2info) ->
                     ...
                     let e2, e2info =
                         ConsiderSplitToMethod cenv.settings.abstractBigTargets cenv.settings.bigTargetSize cenv env (e2, e2info)
        ...
        | LinearOpExpr(op, tyargs, argsHead, argLast, m) ->
            let argsHeadR, argsHeadInfosR =
                OptimizeList (OptimizeExprThenConsiderSplit cenv env) argsHead
            // tailcall
            OptimizeLinearExpr
                cenv
                env
        ...
        | Expr.DebugPoint(m, innerExpr) when not (IsDebugPipeRightExpr cenv innerExpr) ->
            OptimizeLinearExpr cenv env innerExpr (contf << (fun (innerExprR, einfo) -> Expr.DebugPoint(m, innerExprR), einfo))
...
and OptimizeTryFinally cenv env (spTry, spFinally, e1, e2, m, ty) =
    ...
    if cenv.settings.EliminateTryWithAndTryFinally && not e1info.HasEffect then
        ...
        mkTryFinally g (e1R, e2R, m, ty, spTry, spFinally), info
...
and OptimizeTryWith cenv env (e1, vf, ef, vh, eh, m, ty, spTry, spWith) =
    ...
    if cenv.settings.EliminateTryWithAndTryFinally && not e1info.HasEffect then
        ...
        let envinner =
            BindInternalValToUnknown cenv vf (BindInternalValToUnknown cenv vh env)
...
and CopyExprForInlining cenv isInlineIfLambda expr (m: range) =
    ...
    if isInlineIfLambda then
        ...
        expr |> copyExpr g CloneAllAndMarkExprValsAsCompilerGenerated |> remarkExpr m
...
and TryOptimizeVal cenv env (vOpt: ValRef option, shouldInline, inlineIfLambda, valInfoForVal, m) =
    ...
    | SizeValue(_, detail) -> TryOptimizeVal cenv env (vOpt, shouldInline, inlineIfLambda, detail, m)
    ...
    | ValValue(vR, detail) ->
        ...
        | None ->
            ...
            | Some v when
                ...
                && vR.IsCompilerGenerated
                && not vR.IsCompiledAsTopLevel
                && not v.IsCompiledAsTopLevel
                ->
    ...
    | ConstExprValue(_size, expr) -> Some(remarkExpr m (copyExpr g CloneAllAndMarkExprValsAsCompilerGenerated expr))
    ...
    | TupleValue _
    | UnionCaseValue _
    | RecdValue _ when shouldInline -> failwith "tuple, union and record values cannot be marked 'inline'"
...
and TryOptimizeValInfo cenv env m vinfo =
    if vinfo.HasEffect then
        None
    else
        TryOptimizeVal cenv env (None, false, false, vinfo.Info, m)
...
and AddValEqualityInfo g m (v: ValRef) info =
    ...
    if not v.IsMutable || IsKnownOnlyMutableBeforeUse v then
        { info with
            Info = MakeValueInfoForValue g m v info.Info
        }
    else
        info
...
and StripToNominalTyconRef cenv ty =
    ...
    | _ ->
        if isRefTupleTy g ty then
            ...
            failwith "StripToNominalTyconRef: unreachable"
...
and TakeAddressOfStructArgumentIfNeeded cenv (vref: ValRef) ty args m =
    ...
    if vref.IsInstanceMember && isStructTy g ty then
        ...
        | objArg :: rest ->
            ...
            let wrap, objArgAddress, _readonly, _writeonly =
                mkExprAddrOfExpr g true false NeverMutates objArg None m
...
and DevirtualizeApplication cenv env (vref: ValRef) ty tyargs args m nullHandlerOpt =
    ...
    let wrap, args =
        ...
        | Some nullHandler when g.checkNullness && TypeNullIsExtraValueNew g vref.Range ty -> nullHandler g m, args
        | _ -> TakeAddressOfStructArgumentIfNeeded cenv vref ty args m

    let transformedExpr =
        wrap (MakeApplicationAndBetaReduce g (exprForValRef m vref, vref.Type, (if isNil tyargs then [] else [ tyargs ]), args, m))
...
and TryDevirtualizeApplication cenv env (f, tyargs, args, m) =
    ...
    | Expr.Val(v, _, _), [ ty ], _ when CanDevirtualizeApplication cenv v g.generic_comparison_inner_vref ty args ->
        ...
        | Some(_, vref), [ x; y ] ->
            let nullHandler g m =
                AugmentTypeDefinitions.mkBindNullComparison g m x y
    ...
    | Expr.Val(v, _, _), [ ty ], _ when CanDevirtualizeApplication cenv v g.generic_comparison_withc_inner_vref ty args ->
        ...
        | Some vref, [ comp; x; y ] ->
            ...
            let args2 =
                [
                    x
                    mkRefTupledNoTypes g m [ mkCoerceExpr (y, g.obj_ty_ambivalent, m, ty); comp ]
                ]
    ...
    | Expr.Val(v, _, _), [ ty ], _ when CanDevirtualizeApplication cenv v g.generic_equality_er_inner_vref ty args ->
        ...
        | Some(_, vref), [ x; y ] ->
            let nullHandler g m =
                AugmentTypeDefinitions.mkBindThisNullEquals g m x y
    ...
    | Expr.Val(v, _, _), [ ty ], _ when
        CanDevirtualizeApplication cenv v g.generic_equality_per_inner_vref ty args
        && not (isRefTupleTy g ty)
        ->
        let tcref, tyargs = StripToNominalTyconRef cenv ty
        ...
        | Some(_, _, _, Some withcEqualsExactVal), [ x; y ] ->
            let args2 =
                [ x; mkRefTupledNoTypes g m [ y; (mkCallGetGenericPEREqualityComparer g m) ] ]
                ...
                let args2 =
                    [
                        ...
                        mkRefTupledNoTypes
                            ...
                            m
                            [
                                mkCoerceExpr (y, g.obj_ty_ambivalent, m, ty)
                                (mkCallGetGenericPEREqualityComparer g m)
                            ]
                    ]
    ...
    | Expr.Val(v, _, _), [ ty ], _ when CanDevirtualizeApplication cenv v g.generic_hash_inner_vref ty args ->
        ...
        | Some(_, withcGetHashCodeVal, _, _), [ x ] ->
            ...
            let nullHandler g m =
                AugmentTypeDefinitions.mkBindNullHash g m x
    ...
    | Expr.Val(v, _, _), [ ty ], _ when valRefEq g v g.unbox_vref && canUseUnboxFast g m ty ->
    ...
    | Expr.Val(v, _, _), [ ty ], _ when valRefEq g v g.istype_vref && canUseTypeTestFast g ty ->
    ...
    | Expr.Val(vref, _, _), _, _ when valRefEq g vref g.methodhandleof_vref ->
        Some(
            MakeApplicationAndBetaReduce g (exprForValRef m vref, vref.Type, (if isNil tyargs then [] else [ tyargs ]), args, m),
            {
                TotalSize = 1
                FunctionSize = 1
                HasEffect = false
...
and TryInlineApplication cenv env finfo (tyargs: TType list, args: Expr list, m) =
    ...
    | StripLambdaValue(lambdaId, arities, size, f2, f2ty) when
        ...
        if isBaseCall then
            ...
            if isValFromLazyExtensions then
                None
            else
                ...
                let isSecureMethod =
                    ...
                    | ValValue(vref, _) ->
                        vref.Attribs
                        |> List.exists (fun a ->
                            (IsSecurityAttribute g cenv.amap cenv.casApplied a m)
                            || (IsSecurityCriticalAttribute g a))
                    | _ -> false

                if isSecureMethod then
                    None
                else
                    ...
                    if isGetHashCode then
                        ...
                        if isApplicationPartialExpr then
                            None
                        else
                            ...
                            let argsR =
                                args |> List.map (fun e -> let eR, _einfo = OptimizeExpr cenv env e in eR)
                            ...
                            Some(
                                OptimizeExpr
                                    cenv
                                    { env with
                                        dontInline = Zset.add lambdaId env.dontInline
                                    }
                                    exprR
                            )
...
and StripPreComputationsFromComputedFunction g f0 args mkApp =
    ...
    let rec strip (f: Expr) : Expr list * (Expr list -> Expr) =
        ...
        | Expr.Match(spMatch, mExpr, dtree, targets, dflt, _ty) when targets.Length <= 2 ->
            ...
            let remake (newExprs: Expr list) =
                let newExprsInChunks, _ =
                    ((newExprs, 0), chunkSizes)
                    ||> Array.mapFold (fun (acc, i) chunkSize ->
                        let chunk = acc[0 .. chunkSize - 1]
                        let acc = acc[chunkSize..]
                        chunk, (acc, i + chunkSize))

                let targetsR =
                    (newExprsInChunks, targetRemakes)
                    ||> Array.map2 (fun newExprsChunk targetRemake -> targetRemake newExprsChunk)
        ...
        | _ ->
            [ f ],
            (fun newExprs ->
                (assert (List.isSingleton newExprs))
                List.head newExprs)
...
and OptimizeFuncInApplication cenv env f0 mWithArgs =
    ...
    match f0 with
    | Expr.Val(v, _vFlags, _) -> OptimizeVal cenv env f0 (v, mWithArgs)
    | _ -> OptimizeExpr cenv env f0
...
and OptimizeApplication cenv env (f0, f0ty, tyargs, args, m) =
    ...
    | None ->
        ...
        with
        | Choice1Of2 remade -> OptimizeExpr cenv env remade
        | Choice2Of2(newf0, remake) ->
            ...
            | _ ->
                ...
                | _ ->
                    ...
                    newExpr,
                    {
                        TotalSize = finfo.TotalSize + AddTotalSizes arginfos
...
and getPipes g expr acc =
    ...
    //    let test () = DP(x) |> DP(f)
    match stripDebugPoints expr with
    | OpPipeRight g (resType, xExpr, fExpr, m) -> getPipes g xExpr (([ xExpr.Range ], resType, fExpr, m) :: acc)
    | OpPipeRight2 g (resType, x1Expr, x2Expr, fExpr, m) -> [ x1Expr; x2Expr ], (([ x1Expr.Range; x2Expr.Range ], resType, fExpr, m) :: acc)
...
and OptimizeDebugPipeRights cenv env expr =
    ...
    let pipesBinder =
        List.foldBack
            (fun (i, (xsRange, resType, fExpr: Expr, _)) binder ->
                ...
                let name =
                    $"Pipe #%d{env.methEnv.pipelineCount} stage #%d{i + 1} at line %d{fRange.StartLine}"
    ...
    let inputVals, inputValExprs =
        ...
        |> List.mapi (fun i x0R ->
            let nm =
                $"Pipe #%d{env.methEnv.pipelineCount} input"
                + (if nxs0R > 1 then " #" + string (i + 1) else "")
                + $" at line %d{x0R.Range.StartLine}"
...
and OptimizeFSharpDelegateInvoke cenv env (delInvokeRef, delExpr, delInvokeTy, tyargs, delInvokeArg, m) =
    ...
    match
        StripPreComputationsFromComputedFunction g optf0 [ delInvokeArg ] (fun f delInvokeArgsR ->
            MakeFSharpDelegateInvokeAndTryBetaReduce g (delInvokeRef, f, delInvokeTy, tyargs, List.head delInvokeArgsR, m))
    with
    | Choice1Of2 remade -> OptimizeExpr cenv env remade
    | Choice2Of2(newf0, remake) ->

        let newDelInvokeArgs, arginfos =
            OptimizeExprsThenConsiderSplits cenv env [ delInvokeArg ]

        let newDelInvokeArg = List.head newDelInvokeArgs

        let reducedExpr =
            MakeFSharpDelegateInvokeAndTryBetaReduce g (delInvokeRef, newf0, delInvokeTy, tyargs, newDelInvokeArg, m)
...
and OptimizeLambdas (vspec: Val option) cenv env valReprInfo expr exprTy =
    ...
    | Expr.TyLambda(lambdaId, _, _, m, _) ->
        ...
        let tps, ctorThisValOpt, baseValOpt, vsl, body, bodyTy =
            IteratedAdjustLambdaToMatchValReprInfo g cenv.amap valReprInfo expr
        ...
        exprR,
        {
            TotalSize = bsize + estimatedSize (* estimate size of new syntactic closure - expensive, in contrast to a method *)
            FunctionSize = 1
            HasEffect = false
            MightMakeCriticalTailcall = false
            Info = value_
...
and OptimizeNewDelegateExpr cenv env (lambdaId, vsl, body, remake) =
    ...
    exprR,
    {
        TotalSize = bsize + closureTotalSize (* estimate size of new syntactic closure - expensive, in contrast to a method *)
        FunctionSize = 1
        HasEffect = false
        MightMakeCriticalTailcall = false
        Info = value_
...
and ReshapeExpr cenv (shape, e) =
    ...
    | TupleValue subshapes, Expr.Val(_vref, _vFlags, m) ->
        ...
        mkRefTupled
            g
            m
            (List.mapi (fun i subshape -> ReshapeExpr cenv (subshape, mkTupleFieldGet g (tupInfoRef, e, tinst, i, m))) subshapes)
            tinst
    | _ -> e
...
and ConsiderSplitToMethod flag threshold cenv env (e, einfo) =
    ...
    if ComputeSplitToMethodCondition flag threshold cenv env (e, einfo) then
        ...
        mkInvisibleLet m fv (mkLambda m uv (e, ty)) (primMkApp (fe, (mkFunTy g g.unit_ty ty)) [] [ mkUnit g m ] m),
...
and OptimizeMatchPart2 cenv (spMatch, mExpr, dtreeR, targetsR, dinfo, tinfos, m, ty) =
    let newExpr, newInfo =
        RebuildOptimizedMatch(spMatch, mExpr, m, ty, dtreeR, targetsR, dinfo, tinfos)

    let newExpr2 =
        if not cenv.settings.LocalOptimizationsEnabled then
            newExpr
        else
            CombineBoolLogic newExpr
...
and OptimizeDecisionTreeTarget cenv env _m (TTarget(vs, expr, flags)) =
    ...
    let exprR, einfo =
        ConsiderSplitToMethod cenv.settings.abstractBigTargets cenv.settings.bigTargetSize cenv env (exprR, einfo)
...
and OptimizeDecisionTree cenv env m x =
    ...
    | TDSwitch(e, cases, dflt, m) ->
        ...
        let e =
            if typeEquiv g (tyOfExpr g e) g.bool_ty then
                copyExpr g CloneAll e
            else
                e
...
and TryOptimizeDecisionTreeTest cenv test vinfo =
    ...
    | DecisionTreeTest.Const c1, StripConstValue c2 ->
        if c1 = Const.Zero || c2 = Const.Zero then
            None
        else
            Some(c1 = c2)
    ...
    let cases, dflt =
        if cenv.settings.EliminateSwitch && not einfo.HasEffect then
            ...
            match
                (List.tryFind
                    (function
                    | TCase(d2, _) when TryOptimizeDecisionTreeTest cenv d2 einfo.Info = Some true -> true
                    | _ -> false)
                    cases)
            with
            ...
            | _ ->
                ...
                |> List.filter (function
                    | TCase(d2, _) when TryOptimizeDecisionTreeTest cenv d2 einfo.Info = Some false -> false
                    | _ -> true),
                dflt
        else
            cases, dflt
...
and OptimizeSwitchFallback cenv env (eR, einfo, cases, dflt, m) =
    ...
    let info =
        { info with
            ...
            FunctionSize = info.FunctionSize + size
        }
...
and OptimizeBinding cenv isRec env (TBind(vref, expr, spBind)) =
    ...
    try
        ...
        let env =
            if isRec then
                { env with
                    dontSplitVars = env.dontSplitVars.Add vref ()
                }
            else
                env
        ...
        let exprOptimized, einfo =
            let env =
                if vref.IsCompilerGenerated && Option.isSome env.latestBoundId then
                    env
                else
                    { env with
                        latestBoundId = Some vref.Id
                    }

            let cenv =
                if vref.InlineInfo.ShouldInline then
            ...
            let exprOptimized, einfo =
                OptimizeLambdas (Some vref) cenv env arityInfo expr vref.Type

            let size = localVarSize

            exprOptimized,
            { einfo with
                FunctionSize = einfo.FunctionSize + size
                TotalSize = einfo.TotalSize + size
            }
            ...
            | CurriedLambdaValue(_, arities, size, body, _) ->
                if size > (cenv.settings.lambdaInlineThreshold + arities + 2) then
                    ...
                    if fvs.UsesMethodLocalConstructs then
                        ...
                        fvs.FreeLocals.ToArray()
                        |> Seq.fold (fun acc v -> if not acc then v.Accessibility.IsPrivate else acc) false
                    then
        ...
        let einfo =
            if vref.ShouldInline || vref.InlineIfLambda then
                ...
                { einfo with Info = cut einfo.Info }

        let einfo =
            if
                (not vref.ShouldInline
                 && not vref.InlineIfLambda
                 && not cenv.settings.KeepOptimizationValues)
                ||
                     ...
                     | Some mbrTyconRef ->
                         ...
                         if mbrTyconRef.TryDeref.IsSome then
                             ...
                             ExistsSameHeadTypeInHierarchy
                                 ...
                                 g.system_MarshalByRefObject_ty.Value
                         else
                             false
                 | ParentNone -> false)
                ||
        ...
        if vref.ShouldInline && IsPartialExprVal einfo.Info then
            errorR (
                InternalError(
                    "the inline value '"
                    + vref.LogicalName
                    + "' was not inferred to have a known value",
                    vref.Range
                )
            )
...
and OptimizeModuleExprWithSig cenv env mty def =
    ...
    let _renaming, hidden as rpi =
        ComputeRemappingFromImplementationToSignature g def mty
    ...
    let def =
        if not cenv.settings.LocalOptimizationsEnabled then
            ...
            let deadSet =
                Zset.addList (dead |> List.map (fun (bind, _) -> bind.Var)) (Zset.empty valOrder)
            ...
            let rec elimModTy (mtyp: ModuleOrNamespaceType) =
                let mty =
                    ModuleOrNamespaceType(
                        kind = mtyp.ModuleOrNamespaceKind,
            ...
            let rec elimModuleDefn x =
                ...
                | TMDefRec(isRec, opens, tycons, mbinds, m) ->
                    ...
                    TMDefRec(isRec, opens, tycons, mbinds, m)
                | TMDefLet(bind, m) ->
                    if Zset.contains bind.Var deadSet then
                        TMDefRec(false, [], [], [], m)
                    else
                        x
                | TMDefOpens _ -> x
            ...
            and elimModuleBinding modBind =
                ...
                | ModuleOrNamespaceBinding.Binding bind ->
                    if bind.Var |> Zset.memberOf deadSet then
                        None
                    else
                        Some modBind
...
and mkValBind (bind: Binding) info = (mkLocalValRef bind.Var, info)
...
and OptimizeModuleContents cenv (env, bindInfosColl) input =
    ...
    | TMDefRec(isRec, opens, tycons, mbinds, m) ->
        let env =
            if isRec then
                BindInternalValsToUnknown cenv (allValsOfModDef input) env
            else
                env

        let mbindInfos, (env, bindInfosColl) =
            OptimizeModuleBindings cenv (env, bindInfosColl) mbinds
        ...
        let binds =
            ...
            |> List.choose (function
                | Choice1Of2(x, _) -> Some x
                | _ -> None)

        let binfos =
            minfos
            |> List.choose (function
                | Choice1Of2(_, x) -> Some x
                | _ -> None)
        ...
        (TMDefRec(isRec, opens, tycons, mbinds, m),
         notlazy
             {
                 ValInfos = ValInfos(List.map2 (fun bind binfo -> mkValBind bind (mkValInfo binfo bind.Var)) binds binfos)
                 ModuleOrNamespaceInfos = NameMap.ofList minfos
             }),
        (env, bindInfosColl)

    | TMDefOpens _openDecls -> (input, EmptyModuleInfo), (env, bindInfosColl)
    ...
    | TMDefLet(bind, m) ->
        ...
        (TMDefLet(bindR, m),
         notlazy
             {
                 ValInfos = ValInfos [ mkValBind bind (mkValInfo binfo bind.Var) ]
    ...
    | TMDefDo(e, m) ->
        ...
        (TMDefDo(eR, m), EmptyModuleInfo), (env, bindInfosColl)

    | TMDefs defs ->
        let (defs, info), (env, bindInfosColl) =
            OptimizeModuleDefs cenv (env, bindInfosColl) defs
...
and OptimizeModuleBinding cenv (env, bindInfosColl) x =
    ...
    | ModuleOrNamespaceBinding.Module(mspec, def) ->
        ...
        let (def, info), (_, bindInfosColl) =
            OptimizeModuleContents cenv (env, bindInfosColl) def

        let env = BindValsInModuleOrNamespace cenv info env
        (ModuleOrNamespaceBinding.Module(mspec, def), Choice2Of2(id.idText, info)), (env, bindInfosColl)

and OptimizeModuleDefs cenv (env, bindInfosColl) defs =
    let defs, (env, bindInfosColl) =
        List.mapFold (OptimizeModuleContents cenv) (env, bindInfosColl) defs
...
and OptimizeImplFileInternal cenv env isIncrementalFragment hidden implFile =
    let (CheckedImplFile(qname, signature, contents, hasExplicitEntryPoint, isScript, anonRecdTypes, namedDebugPointsForInlinedCode)) =
        implFile
    ...
    let env, contentsR, minfo, hidden =
        ...
        if isIncrementalFragment then
            ...
            let (defR, minfo), (_env, _bindInfosColl) =
                OptimizeModuleContents cenv (env, []) contents
    ...
    let implFileR =
        CheckedImplFile(qname, signature, contentsR, hasExplicitEntryPoint, isScript, anonRecdTypes, namedDebugPointsForInlinedCode)
...
let OptimizeImplFile (settings, ccu, tcGlobals, tcVal, importMap, optEnv, isIncrementalFragment, emitTailcalls, hidden, mimpls) =
    ...
    let optimizeDuringCodeGen disableMethodSplitting expr =
        let env =
            { env with
                disableMethodSplitting = env.disableMethodSplitting || disableMethodSplitting
            }
...
and p_LazyModuleInfo x st = p_lazy p_ModuleInfo x st
...
let rec u_ExprInfo st =
    let rec loop st =
        ...
        | 5 ->
            u_tup4 u_int u_int u_expr u_ty st
            |> (fun (b, c, d, e) -> CurriedLambdaValue(newUnique (), b, c, d, e))
...
and u_ValInfo st =
    ...
    {
        ValExprInfo = a
        ValMakesNoCriticalTailcalls = b
    }

and u_ModuleInfo st =
    let a, b =
        u_tup2 (u_array (u_tup2 u_vref u_ValInfo)) (u_namemap u_LazyModuleInfo) st

    {
        ValInfos = ValInfos a
        ModuleOrNamespaceInfos = b
    }
