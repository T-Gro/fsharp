...
type ValFlags(flags: int64) =
    ...
    new
        ...
        let flags =
            ...
            ||| (if isCompGen then
                     0b00000000000000001000L
                 else
                     0b000000000000000000000L)
    ...
    member x.RecursiveValInfo =
        match (flags &&& 0b00000001100000000000L) with
    ...
    member x.IsCompiledAsStaticPropertyWithoutField =
        (flags &&& 0b00010000000000000000L) <> 0L

    member x.WithIsCompiledAsStaticPropertyWithoutField =
        ValFlags(flags ||| 0b00010000000000000000L)
...
type TyparRigidity =
    ...
    member x.ErrorIfUnified =
        match x with
        | TyparRigidity.Rigid -> true
        | _ -> false

    member x.WarnIfUnified =
        match x with
        | TyparRigidity.WillBeRigid
        | TyparRigidity.WarnIfNotRigid -> true
        | _ -> false

    member x.WarnIfMissingConstraint =
        match x with
...
type TyparFlags(flags: int32) =
    ...
    new
        ...
        TyparFlags(
            (if isFromError then 0b00000000000000010 else 0)
            ...
            ||| (if comparisonDependsOn then 0b00000001000000000 else 0)
            ||| (match dynamicReq with
                 | TyparDynamicReq.No -> 0b00000000000000000
                 | TyparDynamicReq.Yes -> 0b00000010000000000)
            ||| (if equalityDependsOn then 0b00000100000000000 else 0)
            |||
            ...
            (if supportsNullFlex then 0b00100000000000000 else 0)
        )
    ...
    member x.ComparisonConditionalOn = (flags &&& 0b00000001000000000) <> 0x0
    ...
    member x.EqualityConditionalOn = (flags &&& 0b00000100000000000) <> 0x0
    ...
    member x.IsCompatFlex = (flags &&& 0b00010000000000000) <> 0x0
    ...
    member x.IsSupportsNullFlex = (flags &&& 0b00100000000000000) <> 0x0
    ...
    member x.WithStaticReq staticReq =
        TyparFlags(
            ...
            x.ComparisonConditionalOn,
            x.IsSupportsNullFlex
        )
...
type EntityFlags(flags: int64) =
    ...
    new(usesPrefixDisplay, isModuleOrNamespace, preEstablishedHasDefaultCtor, hasSelfReferentialCtor, isStructRecordOrUnionType) =
        EntityFlags(
            (if isModuleOrNamespace then 0b000000000000001L else 0L)
            ||| (if usesPrefixDisplay then 0b000000000000010L else 0L)
            ||| (if preEstablishedHasDefaultCtor then
                     0b000000000000100L
                 else
                     0L)
...
type CompilationPath =
    ...
    member x.NestedPublicPath(id: Ident) =
        PubPath(Array.append (Array.ofList x.MangledPath) [| id.idText |])
    ...
    member x.DemangledPath =
        x.AccessPath
        |> List.map (fun (nm, k) -> CompilationPath.DemangleEntityName nm k)
...
type Entity =
    ...
    member x.IsStaticInstantiationTycon =
        ...
        && let _nm, args = DemangleProvidedTypeName x.LogicalName in
           args.Length > 0
#endif
    ...
    member x.GetDisplayName(coreName, ?withStaticParameters, ?withUnderscoreTypars) =
        ...
        if x.IsModuleOrNamespace then
            x.DemangledModuleOrNamespaceName
#if !NO_TYPEPROVIDERS
            ...
            | tps ->
                ...
                let nm =
                    if coreName || isArray then
                        nm
                    else
                        ConvertLogicalNameToDisplayName nm
    ...
    member x.SetOtherXmlDoc xmlDoc =
        ...
        | _ ->
            x.entity_opt_data <-
                Some
                    { Entity.NewEmptyEntityOptData() with
                        entity_other_xmldoc = Some xmlDoc
                    }
    ...
    member x.XmlDoc =
        ...
        | TProvidedTypeRepr info ->
            let lines =
                info.ProvidedType.PUntaintNoFailure(fun st ->
                    (st :> IProvidedCustomAttributeProvider).GetXmlDocAttributes(info.ProvidedType.TypeProvider.PUntaintNoFailure id))
    ...
    member x.IsNamespace =
        x.IsModuleOrNamespace
        && (match x.ModuleOrNamespaceType.ModuleOrNamespaceKind with
            | Namespace _ -> true
            | _ -> false)

    /// Indicates if the entity has an implicit namespace
    member x.IsImplicitNamespace =
        (match x.ModuleOrNamespaceType.ModuleOrNamespaceKind with
    ...
    member x.AllInstanceFieldsAsList =
        x.AllFieldsAsList |> List.filter (fun f -> not f.IsStatic)
    ...
    member x.TrueFieldsAsList =
        x.AllFieldsAsList |> List.filter (fun f -> not f.IsCompilerGenerated)

    /// Get a list of all instance fields for F#-defined record, struct and class fields in this type definition,
    /// excluding compiler-generate fields.
    member x.TrueInstanceFieldsAsList =
        x.AllFieldsAsList
        |> List.filter (fun f -> not f.IsStatic && not f.IsCompilerGenerated)
    ...
    member x.Link(tg: EntityData) =
        ...
        | Some tg ->
            x.entity_opt_data <-
                Some
                    {
                        entity_compiled_name = tg.entity_compiled_name
    ...
    member x.IsLinked =
        match box x.entity_attribs with
        | null -> false
        | _ -> true
    ...
    member x.ILTyconInfo =
        match x.TypeReprInfo with
        | TILObjectRepr data -> data
        | _ -> failwith "not a .NET type definition"
    ...
    member x.SetIsByRefLike b =
        x.entity_flags <- x.entity_flags.WithIsByRefLike b
    ...
    member x.SetIsReadOnly b =
        x.entity_flags <- x.entity_flags.WithIsReadOnly b
    ...
    member x.SetIsAssumedReadOnly b =
        x.entity_flags <- x.entity_flags.WithIsAssumedReadOnly b
    ...
    member x.IsMeasureableReprTycon =
        match x.TypeReprInfo with
        | TMeasureableRepr _ -> true
        | _ -> false
    ...
    member x.IsHiddenReprTycon =
        match x.TypeAbbrev, x.TypeReprInfo with
        | None, TNoRepr -> true
        | _ -> false

    /// Indicates if this is an F#-defined interface type definition
    member x.IsFSharpInterfaceTycon =
        x.IsFSharpObjectModelTycon
        && match x.FSharpTyconRepresentationData.fsobjmodel_kind with
    ...
    member x.IsFSharpStructOrEnumTycon =
        ...
        | TFSharpTyconRepr info ->
            ...
            | TFSharpRecord
            | TFSharpUnion -> x.IsStructRecordOrUnionTycon
            | TFSharpClass
            | TFSharpInterface
            | TFSharpDelegate _ -> false
            | TFSharpStruct
            | TFSharpEnum -> true
        | _ -> false

    /// Indicates if this is a .NET-defined struct or enum type definition, i.e. a value type definition
    member x.IsILStructOrEnumTycon = x.IsILTycon && x.ILTyconRawMetadata.IsStructOrEnum
    ...
    member x.ImmediateInterfacesOfFSharpTycon = x.TypeContents.tcaug_interfaces
    ...
    member x.MembersOfFSharpTyconByName = x.TypeContents.tcaug_adhoc

    /// Gets any implicit hash/equals (with comparer argument) methods added to an F# record, union or struct type definition.
    member x.GeneratedHashAndEqualsWithComparerValues =
        x.TypeContents.tcaug_hash_and_equals_withc
    ...
    member x.CompiledRepresentation =
        ...
        | TProvidedTypeRepr info when info.IsErased -> failwith "No compiled representation for provided erased type"
        ...
        cached x.CompiledReprCache (fun () ->
            ...
            | _ ->
                ...
                | _ ->
                    ...
                    let ilTypeRef =
                        match x.TypeReprInfo with
                        | TILObjectRepr(TILObjectReprData(ilScopeRef, ilEnclosingTypeDefs, ilTypeDef)) ->
                            mkRefForNestedILTypeDef ilScopeRef (ilEnclosingTypeDefs, ilTypeDef)
    ...
    member x.CompiledRepresentationForNamedType =
        ...
        | CompiledTypeRepr.ILAsmOpen _ ->
            invalidOp (FSComp.SR.tastTypeHasAssemblyCodeRepresentation (x.DisplayNameWithStaticParametersAndUnderscoreTypars))

    /// Indicates if we have pre-determined that a type definition has a default constructor.
    member x.PreEstablishedHasDefaultConstructor =
        x.entity_flags.PreEstablishedHasDefaultConstructor
    ...
    member x.SetIsStructRecordOrUnion b =
        let flags = x.entity_flags in

        x.entity_flags <-
            EntityFlags(
                flags.IsPrefixDisplay,
                flags.IsModuleOrNamespace,
                flags.PreEstablishedHasDefaultConstructor,
                flags.HasSelfReferentialConstructor,
                b
            )
...
type TyconAugmentation =
    ...
    member tcaug.SetHashAndEqualsWith x =
        tcaug.tcaug_hash_and_equals_withc <- Some x
...
type TProvidedTypeInfo =
    ...
    member info.BaseTypeForErased(m, objTy) =
        if info.IsErased then
            info.LazyBaseType.Force(m, objTy)
        else
            failwith "expect erased type"
...
type TyconRecdFields =
    ...
    member x.FieldByIndex n =
        if n >= 0 && n < x.FieldsByIndex.Length then
            x.FieldsByIndex[n]
        else
            failwith "FieldByIndex"
...
type TyconUnionCases =
    ...
    member x.GetUnionCaseByIndex n =
        if n >= 0 && n < x.CasesByIndex.Length then
            x.CasesByIndex[n]
        else
            invalidArg "n" "GetUnionCaseByIndex"
...
type UnionCase =
    ...
    member x.SetOtherXmlDoc xmlDoc = x.OtherXmlDoc <- xmlDoc
...
type RecdField =
    ...
    member v.SetOtherXmlDoc(xmlDoc: XmlDoc) = v.rfield_otherxmldoc <- xmlDoc
...
type ModuleOrNamespaceType(kind: ModuleOrNamespaceKind, vals: QueueList<Val>, entities: QueueList<Entity>) =
    ...
    let mutable tyconsByDemangledNameAndArityCache: LayeredMap<NameArityPair, Tycon> option =
        None
    ...
    let mutable allValsAndMembersByPartialLinkageKeyCache: MultiMap<ValLinkagePartialKey, Val> option =
        None
    ...
    member _.TypeDefinitions =
        entities
        |> Seq.filter (fun x -> not x.IsFSharpException && not x.IsModuleOrNamespace)
        |> Seq.toList

    /// Get a list of F# exception definitions defined within this module, namespace or type.
    member _.ExceptionDefinitions =
        entities |> Seq.filter (fun x -> x.IsFSharpException) |> Seq.toList

    /// Get a list of module and namespace definitions defined within this module, namespace or type.
    member _.ModuleAndNamespaceDefinitions =
        entities |> Seq.filter (fun x -> x.IsModuleOrNamespace) |> Seq.toList

    /// Get a list of type and exception definitions defined within this module, namespace or type.
    member _.TypeAndExceptionDefinitions =
        entities |> Seq.filter (fun x -> not x.IsModuleOrNamespace) |> Seq.toList
    ...
    member mtyp.TypesByDemangledNameAndArity =
        cacheOptByref &tyconsByDemangledNameAndArityCache (fun () ->
            LayeredMap.Empty.AddMany(
                mtyp.TypeAndExceptionDefinitions
                |> List.map (fun (tc: Tycon) -> Construct.KeyTyconByDecodedName tc.LogicalName tc)
                |> List.toArray
            ))
    ...
    member mtyp.TypesByAccessNames =
        cacheOptByref &tyconsByAccessNamesCache (fun () ->
            LayeredMultiMap.Empty.AddMany(
                mtyp.TypeAndExceptionDefinitions
                |> List.toArray
                |> Array.collect (fun (tc: Tycon) -> Construct.KeyTyconByAccessNames tc.LogicalName tc)
            ))
    ...
    member mtyp.TypesByMangledName =
        ...
        cacheOptByref &tyconsByMangledNameCache (fun () -> List.foldBack addTyconByMangledName mtyp.TypeAndExceptionDefinitions Map.empty)
    ...
    member _.AllEntitiesByCompiledAndLogicalMangledNames: NameMap<Entity> =
        let addEntityByMangledName (x: Entity) tab =
            ...
            if name1 = name2 then tab else NameMap.add name2 x tab

        cacheOptByref &allEntitiesByMangledNameCache (fun () -> QueueList.foldBack addEntityByMangledName entities Map.empty)
    ...
    member _.AllValsAndMembersByPartialLinkageKey =
        ...
        cacheOptByref &allValsAndMembersByPartialLinkageKeyCache (fun () -> QueueList.foldBack addValByMangledName vals MultiMap.empty)
    ...
    member _.AllValsByLogicalName =
        ...
        cacheOptByref &allValsByLogicalNameCache (fun () -> QueueList.foldBack addValByName vals Map.empty)
    ...
    member mtyp.ExceptionDefinitionsByDemangledName =
        ...
        cacheOptByref &exconsByDemangledNameCache (fun () -> List.foldBack add mtyp.ExceptionDefinitions Map.empty)
    ...
    member _.ModulesAndNamespacesByDemangledName =
        ...
        cacheOptByref &modulesByDemangledNameCache (fun () -> QueueList.foldBack add entities Map.empty)
...
let getSyntaxAccessForCompPath (TAccess a) =
    match a with
    | CompPath(_, sa, _) :: _ -> sa
    | _ -> TypedTree.SyntaxAccess.Unknown
...
type Accessibility =
    ...
    member public x.IsPublic =
        match x with
        | Public -> true
        | _ -> false

    member public x.IsInternal =
        match x with
        | Internal -> true
        | _ -> false

    member public x.IsPrivate =
        match x with
        | Private -> true
        | _ -> false
    ...
    member x.CompilationPaths =
        match x with
        | TAccess compilationPaths -> compilationPaths

    override x.ToString() =
        match x with
        | TAccess(paths) ->
            let mangledTextOfCompPath (CompPath(scoref, _, path)) =
                getNameOfScopeRef scoref + "/" + textOfPath (List.map fst path)
            ...
            if paths = "" then scopename else $"{scopename} {paths}"
...
type Typar =
    ...
    member x.SetIsCompatFlex b =
        x.typar_flags <- x.typar_flags.WithCompatFlex b

    member x.SetSupportsNullFlex b =
        x.typar_flags <- x.typar_flags.WithSupportsNullFlex b
    ...
    member x.SetAttribs attribs =
        ...
        | [],
          Some {
               ...
               } when doc.IsEmpty -> x.typar_opt_data <- None
            ...
            let optData =
                {
                    ...
                    typar_attribs = tg.typar_attribs
                    typar_is_contravariant = tg.typar_is_contravariant
                }
    ...
    member x.SetRigidity b =
        ...
        x.typar_flags <-
            TyparFlags(
                ...
                b,
                flags.IsFromError,
                flags.IsCompilerGenerated,
                flags.StaticReq,
                flags.DynamicReq,
                flags.EqualityConditionalOn,
                flags.ComparisonConditionalOn,
                flags.IsSupportsNullFlex
            )
...
type TraitConstraintInfo =
    ...
    member x.CompiledObjectAndArgumentTypes =
        (let (TTrait(objAndArgTys = objAndArgTys)) = x in objAndArgTys)
    ...
    member x.WithMemberKind(kind) =
        (let (TTrait(a, b, c, d, e, f, g)) = x in TTrait(a, b, { c with MemberKind = kind }, d, e, f, g))

    member x.WithSupportTypes(tys) =
        (let (TTrait(_, b, c, d, e, f, g)) = x in TTrait(tys, b, c, d, e, f, g))

    member x.WithMemberName(name) =
        (let (TTrait(a, _, c, d, e, f, g)) = x in TTrait(a, name, c, d, e, f, g))
...
type ValLinkagePartialKey =
    ...
    override x.ToString() =
        "ValLinkagePartialKey(" + x.LogicalName + ")"
...
type ValLinkageFullKey(partialKey: ValLinkagePartialKey, typeForLinkage: TType option) =
    ...
    override x.ToString() =
        "ValLinkageFullKey(" + partialKey.LogicalName + ")"
...
type Val =
    ...
    member x.SigRange =
        match x.val_opt_data with
        | Some {
                   arg_repr_info_for_display = Some { OtherRange = Some m }
               } -> m
    ...
    member x.GetLinkagePartialKey() : ValLinkagePartialKey =
...

        {
            ...
            MemberParentMangledName =
                (if x.IsMember then
                     Some x.MemberApparentEntity.LogicalName
                 else
                     None)
    ...
    member x.IsConstructor =
        ...
        | Some memberInfo when
            ...
            && (memberInfo.MemberFlags.MemberKind = SynMemberKind.Constructor)
            ->
            true
        | _ -> false
    ...
    member x.IsClassConstructor =
        ...
        | Some memberInfo when
            not x.IsExtensionMember
            && (memberInfo.MemberFlags.MemberKind = SynMemberKind.ClassConstructor)
            ->
            true
        | _ -> false
    ...
    member x.IsMutable =
        (match x.val_flags.MutabilityInfo with
         | Immutable -> false
         | Mutable -> true)
    ...
    member x.IsCompiledAsStaticPropertyWithoutField =
        let hasValueAsStaticProperty =
            x.Attribs
            |> List.exists (fun (Attrib(tc, _, _, _, _, _, _)) -> tc.CompiledName = "ValueAsStaticPropertyAttribute")
    ...
    member x.PermitsExplicitTypeInstantiation =
        x.val_flags.PermitsExplicitTypeInstantiation

    /// Indicates if this is a member generated from the de-sugaring of 'let' function bindings in the implicit class syntax?
    member x.IsIncrClassGeneratedMember =
        x.IsCompilerGenerated && x.val_flags.IsIncrClassSpecialMember

    /// Indicates if this is a constructor member generated from the de-sugaring of implicit constructor for a class type?
    member x.IsIncrClassConstructor =
        x.IsConstructor && x.val_flags.IsIncrClassSpecialMember
    ...
    member x.GetterOrSetterIsCompilerGenerated =
        x.MemberInfo
        |> Option.exists (fun m -> m.MemberFlags.GetterOrSetterIsCompilerGenerated)
    ...
    member x.SetLogicalName(nm) = x.val_logical_name <- nm
    ...
    member x.CompiledName(compilerGlobalState: CompilerGlobalState option) =
        ...
        | Some state when
            x.IsCompiledAsTopLevel
            && not x.IsMember
            && (x.IsCompilerGenerated || not x.IsMemberOrModuleBinding)
            ->
    ...
    member x.SetValRec b =
        x.val_flags <- x.val_flags.WithRecursiveValInfo b

    member x.SetIsCompilerGenerated(v) =
        x.val_flags <- x.val_flags.WithIsCompilerGenerated(v)

    member x.SetIsMemberOrModuleBinding() =
        x.val_flags <- x.val_flags.WithIsMemberOrModuleBinding

    member x.SetMakesNoCriticalTailcalls() =
        x.val_flags <- x.val_flags.WithMakesNoCriticalTailcalls

    member x.SetHasBeenReferenced() =
        x.val_flags <- x.val_flags.WithHasBeenReferenced

    member x.SetIsCompiledAsStaticPropertyWithoutField() =
        x.val_flags <- x.val_flags.WithIsCompiledAsStaticPropertyWithoutField
    ...
    member x.SetInlineIfLambda() =
        x.val_flags <- x.val_flags.WithInlineIfLambda

    member x.SetIsImplied() =
        x.val_flags <- x.val_flags.WithIsImplied
    ...
    member x.SetValReprInfoForDisplay info =
        ...
        | _ ->
            x.val_opt_data <-
                Some
                    { Val.NewEmptyValOptData() with
                        val_repr_info_for_display = info
                    }
    ...
    member x.SetArgReprInfoForDisplay info =
        ...
        | _ ->
            x.val_opt_data <-
                Some
                    { Val.NewEmptyValOptData() with
                        arg_repr_info_for_display = info
                    }
    ...
    member x.SetMemberInfo member_info =
        ...
        | _ ->
            x.val_opt_data <-
                Some
                    { Val.NewEmptyValOptData() with
                        val_member_info = Some member_info
                    }
    ...
    member x.SetData(tg: ValData) =
        ...
        | Some tg ->
            x.val_opt_data <-
                Some
                    {
                        val_compiled_name = tg.val_compiled_name
    ...
    member x.IsLinked =
        match box x.val_logical_name with
        | null -> false
        | _ -> true
...
type NonLocalValOrMemberRef =
    ...
    /// For debugging
    override x.ToString() =
        !!x.EnclosingEntity.nlr.ToString() + "::" + x.ItemKey.PartialKey.LogicalName
...
type NonLocalEntityRef =
    ...
    static member TryDerefEntityPathViaProvidedType(ccu: CcuThunk, path: string[], i: int, entity: Entity) =
        ...
        | TProvidedTypeRepr info ->
            ...
            let rec tryResolveNestedTypeOf (parentEntity: Entity, resolutionEnvironment, st: Tainted<ProvidedType>, i) =
                ...
                | Tainted.NonNull st ->
                    let newEntity =
                        Construct.NewProvidedTycon(resolutionEnvironment, st, ccu.ImportProvidedType, false, m)
                    ...
                    if i = path.Length - 1 then
                        ...
                        tryResolveNestedTypeOf (newEntity, resolutionEnvironment, st, i + 1)
        ...
        | TProvidedNamespaceRepr(resolutionEnvironment, resolvers) ->
                ...
                | [ (_, st) ] ->
                    ...
                    let rec injectNamespacesFromIToJ (entity: Entity) k =
                        if k = j then
                            ...
                            let cpath =
                                entity.CompilationPath.NestedCompPath entity.LogicalName (ModuleOrNamespaceKind.Namespace false)
            ...
            let rec tryResolvePrefixes j =
                if j >= path.Length then
                    ValueNone
                else
                    match tryResolvePrefix j with
    ...
    member nleref.DisplayName = String.concat "." nleref.Path
    ...
    member nleref.Deref =
        ...
        | ValueNone ->
            errorR (
                InternalUndefinedItemRef(
                    FSComp.SR.tastUndefinedItemRefModuleNamespace,
                    nleref.DisplayName,
                    nleref.AssemblyName,
                    "<some module on this path>"
                )
            )
...
type EntityRef =
    ...
    member private tcr.Resolve canError =
        ...
        match res with
        | ValueSome r -> tcr.binding <- nullableSlotFull r
        | ValueNone -> ()
    ...
    member tcr.Deref =
        ...
        | null ->
            ...
            | null ->
                error (
                    InternalUndefinedItemRef(
                        FSComp.SR.tastUndefinedItemRefModuleNamespaceType,
                        String.concat "." tcr.nlr.EnclosingMangledPath,
                        tcr.nlr.AssemblyName,
                        tcr.nlr.LastItemMangledName
                    )
                )
            | _ -> tcr.binding
        | _ -> tcr.binding
    ...
    /// Gets the data indicating the compiled representation of a named type or module in terms of Abstract IL data structures.
    member x.CompiledRepresentationForNamedType =
        x.Deref.CompiledRepresentationForNamedType
    ...
    /// Backticks are added implicitly for entities with non-identifier names
    member x.DisplayNameWithStaticParametersAndUnderscoreTypars =
        x.Deref.DisplayNameWithStaticParametersAndUnderscoreTypars
    ...
    /// Gets the immediate interface types of an F# type definition. Further interfaces may be supported through class and interface inheritance.
    member x.ImmediateInterfaceTypesOfFSharpTycon =
        x.Deref.ImmediateInterfaceTypesOfFSharpTycon
    ...
    /// Gets any implicit hash/equals (with comparer argument) methods added to an F# record, union or struct type definition.
    member x.GeneratedHashAndEqualsWithComparerValues =
        x.Deref.GeneratedHashAndEqualsWithComparerValues

    /// Gets any implicit CompareTo (with comparer argument) methods added to an F# record, union or struct type definition.
    member x.GeneratedCompareToWithComparerValues =
        x.Deref.GeneratedCompareToWithComparerValues
    ...
    /// Indicates if we have pre-determined that a type definition has a default constructor.
    member x.PreEstablishedHasDefaultConstructor =
        x.Deref.PreEstablishedHasDefaultConstructor
...
type ValRef =
    ...
    member x.Deref =
        if obj.ReferenceEquals(x.binding, null) then
            let res =
                ...
                let possible =
                    e.ModuleOrNamespaceType.TryLinkVal(nlr.EnclosingEntity.nlr.Ccu, nlr.ItemKey)

                match possible with
                | ValueNone ->
                    error (
                        InternalUndefinedItemRef(
                            FSComp.SR.tastUndefinedItemRefVal,
                            e.DisplayNameWithStaticParameters,
                            nlr.AssemblyName,
                            sprintf "%+A" nlr.ItemKey.PartialKey
                        )
                    )
                | ValueSome h -> h
    ...
    member x.TryDeref =
        if obj.ReferenceEquals(x.binding, null) then
            ...
            match resOpt with
            | ValueNone -> ()
            | ValueSome res -> x.binding <- nullableSlotFull res
    ...
    member x.IsPropertyGetterMethod =
        match x.MemberInfo with
        | None -> false
        | Some(memInfo: ValMemberInfo) ->
            memInfo.MemberFlags.MemberKind = SynMemberKind.PropertyGet
            || memInfo.MemberFlags.MemberKind = SynMemberKind.PropertyGetSet
...
type UnionCaseRef =
    ...
    member x.Index =
        try
            ...
            x.TyconRef.UnionCasesArray
            |> Array.findIndex (fun uc -> uc.LogicalName = x.CaseName)
...
let findLogicalFieldIndexOfRecordField (tcref: TyconRef) (id: string) =
    ...
    let rec go originalIdx skippedItems =
        if originalIdx >= arr.Length then
            error (InternalError(sprintf "field %s not found in type %s" id tcref.LogicalName, tcref.Range))
        else
            let currentItem = arr[originalIdx]

            if currentItem.LogicalName = id then
                (originalIdx - skippedItems)
            else
                go
                    (originalIdx + 1)
                    (skippedItems
                     + (if currentItem.IsCompilerGenerated && currentItem.IsStatic then
                            1
                        else
                            0))
...
type Nullness =
    ...
    override n.ToString() =
        match n.Evaluate() with
        | NullnessInfo.WithNull -> "?"
        | NullnessInfo.WithoutNull -> ""
        | NullnessInfo.AmbivalentToNull -> "%"

    member n.ToFsharpCodeString() =
        match n.Evaluate() with
        | NullnessInfo.WithNull -> " | null "
        | NullnessInfo.WithoutNull -> ""
        | NullnessInfo.AmbivalentToNull -> ""
...
type TType =
    ...
    member x.GetAssemblyName() =
        ...
        | TType_var(tp, _) ->
            tp.Solution
            |> function
                | Some slnTy -> slnTy.GetAssemblyName()
                | None -> ""
    ...
    override x.ToString() =
        ...
        | TType_app(tcref, tinst, nullness) ->
            ...
            + (match tinst with
               | [] -> ""
               | tys -> "<" + String.concat "," (List.map string tys) + ">")
            + nullness.ToString()
...
type AnonRecdTypeInfo =
    ...
    static member Create(ccu: CcuThunk, tupInfo, ids: Ident[]) =
        ...
        let stamp =
            sha1HashInt64
                [|
                    for c in ccu.AssemblyName do
                        yield byte c
                        yield byte (int32 c >>> 8)
                    match tupInfo with
                    | TupInfo.Const b -> yield (if b then 0uy else 1uy)
                    for id in sortedIds do
                        for c in id.idText do
                            yield byte c
                            yield byte (int32 c >>> 8)
    ...
    member x.ILTypeRef =
        let ilTypeName =
            sprintf
                "<>f__AnonymousType%s%u`%d"
                (match x.TupInfo with
                 | TupInfo.Const b -> if b then "1000" else "")
                (uint32 x.IlTypeName)
                x.SortedIds.Length
    ...
    member x.DisplayNameByIdx idx =
        x.SortedNames[idx] |> ConvertLogicalNameToDisplayName
...
type Binding =
    ...
    override x.ToString() =
        sprintf "TBind(%s, ...)" (x.Var.CompiledName None)
...
type ActivePatternElemRef =
    ...
    /// Get a reference to the value for the active pattern being referred to
    member x.ActivePatternRetKind =
        (let (APElemRef(_, _, _, activePatRetKind)) = x in activePatRetKind)
...
type ValReprInfo =
    ...
    member x.AritiesOfArgs =
        (let (ValReprInfo(_, args, _)) = x in List.map List.length args)

    /// Get the kind of each type parameter
    member x.KindsOfTypars =
        (let (ValReprInfo(n, _, _)) = x in n |> List.map (fun (TyparReprInfo(_, k)) -> k))
...
type Expr =
    ...
    | TyLambda of unique: Unique * typeParams: Typars * bodyExpr: Expr * range: range * overallType: TType
    ...
    | App of funcExpr: Expr * formalType: TType * typeArgs: TypeInst * args: Exprs * range: range

    /// Bind a recursive set of values.
    | LetRec of bindings: Bindings * bodyExpr: Expr * range: range * frees: FreeVarsCache

    /// Bind a value.
    | Let of binding: Binding * bodyExpr: Expr * range: range * frees: FreeVarsCache
    ...
    /// operators.
    | StaticOptimization of conditions: StaticOptimization list * expr: Expr * alternativeExpr: Expr * range: range
    ...
    | Op of op: TOp * typeArgs: TypeInst * args: Exprs * range: range
    ...
    ///
    | WitnessArg of traitInfo: TraitConstraintInfo * range: range
    ...
    | TyChoose of typeParams: Typars * bodyExpr: Expr * range: range
    ...
    member expr.ToDebugString(depth: int) : string =
        if depth = 0 then
            ...
            | App(f, _, _, args, _) ->
                "App("
                + f.ToDebugString(depth)
                + ", ["
                + String.concat ", " (args |> List.map (fun e -> e.ToDebugString(depth)))
                + "])"
            | LetRec _ -> "LetRec(..)"
            | Let(bind, body, _, _) ->
                "Let("
                + bind.Var.DisplayName
                + ", "
                + bind.Expr.ToDebugString(depth)
                + ", "
                + body.ToDebugString(depth)
                + ")"
    ...
    member expr.Range =
        ...
        | Expr.Op(_, _, _, m)
        | Expr.Const(_, m, _)
        | Expr.Quote(_, _, _, m, _)
        | Expr.Obj(_, _, _, _, _, _, m)
        | Expr.App(_, _, _, _, m)
        | Expr.Sequential(_, _, _, m)
        | Expr.StaticOptimization(_, _, _, m)
...
type ObjExprMethod =
    ...
    member x.Id =
        let (TObjExprMethod(slotsig, _, _, _, _, m)) = x in mkSynId m slotsig.Name

    [<DebuggerBrowsable(DebuggerBrowsableState.Never)>]
    member x.DebugText = x.ToString()

    override x.ToString() =
        sprintf "TObjExprMethod(%s, ...)" x.Id.idText
...
[<NoEquality; NoComparison; StructuredFormatDisplay("{DebugText}")>]
type SlotParam =
    | TSlotParam of paramName: string option * paramType: TType * isIn: bool * isOut: bool * isOptional: bool * attributes: Attribs
...
type OpenDeclaration =
    ...
    static member Create
        (target: SynOpenDeclTarget, modules: ModuleOrNamespaceRef list, types: TType list, appliedScope: range, isOwnNamespace: bool)
        =
        {
            Target = target
            Range =
...
type CheckedImplFile =
    ...
    member x.QualifiedNameOfFile =
        let (CheckedImplFile(qualifiedNameOfFile = res)) = x in res

    member x.HasExplicitEntryPoint =
        let (CheckedImplFile(hasExplicitEntryPoint = res)) = x in res
    ...
    let rec findInTree (remainingPath: ArraySegment<string>) (finalKey: string) (tree: CcuTypeForwarderTree) : Lazy<EntityRef> option =
        ...
        | true, innerTree ->
            if remainingPath.Count = 0 then
                ...
                findInTree
                    (ArraySegment<string>((!!remainingPath.Array), remainingPath.Offset + 1, remainingPath.Count - 1))
                    finalKey
                    innerTree
        | false, _ -> None
...
type CcuThunk =
    ...
    member ccu.RootModulesAndNamespaces =
        ccu.Contents.ModuleOrNamespaceType.ModuleAndNamespaceDefinitions

    /// The table of type definitions at the "root" of the assembly
    member ccu.RootTypeAndExceptionDefinitions =
        ccu.Contents.ModuleOrNamespaceType.TypeAndExceptionDefinitions

    /// Create a CCU with the given name and contents
    static member Create(nm, x) = { target = x; name = nm }
    ...
    member x.Fixup(avail: CcuThunk) =
        ...
        | _ ->
            ...
            if x.AssemblyName <> "FSharp.Core" then
                errorR (
                    Failure(
                        "internal error: Fixup: the ccu thunk for assembly "
                        + x.AssemblyName
                        + " not delayed!"
                    )
                )
    ...
    member x.CloneWithFinalizedContents(ccuContents) =
        { x with
            target = { x.target with Contents = ccuContents }
        }
...
type CcuResolutionResult =
    ...
    override x.ToString() =
        match x with
        | ResolvedCcu ccu -> ccu.ToString()
        | UnresolvedCcu s -> "unresolved " + s
...
type Construct() =
                        ...
                        st.PUntaint(
                            (fun st ->
                                ...
                                | NonNull x ->
                                    ...
                                    | x -> x.FullName = "System.Delegate" || x.FullName = "System.MulticastDelegate"),
                            m
                        ))
    ...
    static member NewProvidedTycon
        (resolutionEnvironment, st: Tainted<ProvidedType>, importProvidedType, isSuppressRelocate, m, ?access, ?cpath)
        =
        let stamp = newStamp ()
        ...
        let repr =
            Construct.NewProvidedTyconRepr(resolutionEnvironment, st, importProvidedType, isSuppressRelocate, m)
        ...
        Tycon.New
            "tycon"
            {
                ...
                entity_flags =
                    EntityFlags(
                        usesPrefixDisplay = false,
                        isModuleOrNamespace = false,
                        preEstablishedHasDefaultCtor = false,
                        hasSelfReferentialCtor = false,
                ...
                entity_modul_type =
                    MaybeLazy.Lazy(
                        InterruptibleLazy(fun _ -> ModuleOrNamespaceType(Namespace true, QueueList.ofList [], QueueList.ofList []))
                    )
    ...
    static member NewRigidTypar nm m =
        Construct.NewTypar(
            TyparKind.Type,
            TyparRigidity.Rigid,
            SynTypar(mkSynId m nm, TyparStaticReq.None, true),
            false,
            TyparDynamicReq.Yes,
            [],
            false,
    ...
    static member NewTycon
        (
            ...
            doc: XmlDoc,
            usesPrefixDisplay,
            preEstablishedHasDefaultCtor,
            hasSelfReferentialCtor,
            mtyp
        ) =
        let stamp = newStamp ()
        ...
        Tycon.New
            "tycon"
            {
                ...
                entity_flags =
                    EntityFlags(
                        usesPrefixDisplay = usesPrefixDisplay,
                        isModuleOrNamespace = false,
                        preEstablishedHasDefaultCtor = preEstablishedHasDefaultCtor,
                        hasSelfReferentialCtor = hasSelfReferentialCtor,
                        isStructRecordOrUnionType = false
                    )
                ...
                entity_pubpath =
                    cpath
                    |> Option.map (fun (cp: CompilationPath) -> cp.NestedPublicPath(mkSynId m nm))
                entity_cpath = cpath
                entity_il_repr_cache = newCache ()
                entity_opt_data =
    ...
    static member NewILTycon nlpath (nm, m) tps (scoref: ILScopeRef, enc, tdef: ILTypeDef) mtyp =
        let tycon =
            Construct.NewTycon(nlpath, nm, m, taccessPublic, taccessPublic, TyparKind.Type, tps, XmlDoc.Empty, true, false, false, mtyp)
                ...
                { Val.NewEmptyValOptData() with
                    val_compiled_name =
                        (match compiledName with
                         | Some v when v <> logicalName -> compiledName
                         | _ -> None)
                    val_repr_info = arity
                    val_const = konst
                    val_access = access
    ...
    static member NewCcuContents sref m nm mty =
        Construct.NewModuleOrNamespace
            (Some(CompPath(sref, SyntaxAccess.Unknown, [])))
            taccessPublic
            (ident (nm, m))
            XmlDoc.Empty
            []
            (MaybeLazy.Strict mty)
    ...
    static member NewModifiedModuleOrNamespace f orig =
        orig
        |> Construct.NewModifiedTycon(fun d ->
            { d with
                entity_modul_type = MaybeLazy.Strict(f (d.entity_modul_type.Force()))
            })
    ...
    static member NewClonedTycon orig = Construct.NewModifiedTycon id orig
    ...
    static member ComputeDefinitionLocationOfProvidedItem<'T when 'T :> IProvidedCustomAttributeProvider>(p: Tainted<'T>) : range option =
        let attrs =
            p.PUntaintNoFailure(fun x -> x.GetDefinitionLocationAttribute(p.TypeProvider.PUntaintNoFailure id))
