...
module ValReprInfo =
    ...
    let unnamedTopArg1: ArgReprInfo =
        {
            Attribs = []
            Name = None
            OtherRange = None
    ...
    let InferTyparInfo (tps: Typar list) =
        tps |> List.map (fun tp -> TyparReprInfo(tp.Id, tp.Kind))

    let InferArgReprInfo (v: Val) : ArgReprInfo =
        {
            Attribs = []
            Name = Some v.Id
            OtherRange = None
    ...
    let InferArgReprInfos (vs: Val list list) =
        ValReprInfo([], List.mapSquared InferArgReprInfo vs, unnamedRetVal)
...
let tryGetArityOfValForDisplay (v: Val) =
    v.ValReprInfoForDisplay |> Option.orElseWith (fun _ -> v.ValReprInfo)
...
let ERefLocal x : EntityRef =
    {
        binding = x
        nlr = Unchecked.defaultof<_>
    }

let ERefNonLocal x : EntityRef =
    {
...
type EntityRef with
    ...
    member tcref.RecdFieldRefInNestedTycon tycon (id: Ident) =
        RecdFieldRef(tcref.NestedTyconRef tycon, id.idText)
...
let VRefLocal x : ValRef =
    {
        binding = x
        nlr = Unchecked.defaultof<_>
    }

let VRefNonLocal x : ValRef =
    {
        binding = Unchecked.defaultof<_>
        nlr = x
    }
...
let mkNonLocalValRef mp id =
    VRefNonLocal
        {
            EnclosingEntity = ERefNonLocal mp
            ItemKey = id
        }

let mkNonLocalValRefPreResolved x mp id =
    VRefNonLocalPreResolved
        x
        {
            EnclosingEntity = ERefNonLocal mp
            ItemKey = id
...
let copyTypar clearStaticReq (tp: Typar) =
    ...
    let flags =
        if clearStaticReq then
            tp.typar_flags.WithStaticReq(TyparStaticReq.None)
        else
            tp.typar_flags
...
let tryShortcutSolvedUnitPar canShortcut (r: Typar) =
    if r.Kind = TyparKind.Type then
        failwith "tryShortcutSolvedUnitPar: kind=type"
    ...
    | Some(TType_measure unt) ->
        if canShortcut then
            ...
            | Measure.Var r2 ->
                ...
                | None -> ()
                | Some _ as soln -> r.typar_solution <- soln
            | _ -> ()

        unt
    | _ -> failwith "tryShortcutSolvedUnitPar: unsolved"
...
let combineNullness (nullnessOrig: Nullness) (nullnessNew: Nullness) =
    ...
    | Nullness.Variable _, Nullness.Known NullnessInfo.WithoutNull -> nullnessOrig
    | _ ->
...
let nullnessEquiv (nullnessOrig: Nullness) (nullnessNew: Nullness) =
    LanguagePrimitives.PhysicalEquality nullnessOrig nullnessNew
            ...
            if canShortcut then
                ...
                | TType_var(r2, nullness2) when r2.Constraints.IsEmpty ->
                    ...
                    | NullnessInfo.WithoutNull ->
                        match r2.Solution with
                        | None -> ()
                        | Some _ as soln2 -> r.typar_solution <- soln2
                    | _ -> ()
                | _ -> ()

            stripTyparEqnsAux (combineNullness nullness0 nullness) canShortcut soln
        | None -> addNullnessToTy nullness0 ty
    | TType_measure unt -> TType_measure(stripUnitEqnsAux canShortcut unt)
    | _ -> addNullnessToTy nullness0 ty

let stripTyparEqns ty =
    stripTyparEqnsAux KnownWithoutNull false ty
...
let mkNonLocalCcuRootEntityRef ccu (x: Entity) =
    mkNonLocalTyconRefPreResolved x (mkNonLocalEntityRef ccu [||]) x.LogicalName
...
let rescopePubPathToParent viewedCcu (PubPath p) =
    NonLocalEntityRef(viewedCcu, p[0 .. p.Length - 2])
...
let nonLocalRefDefinitelyNotEq (NonLocalEntityRef(_, y1)) (NonLocalEntityRef(_, y2)) = not (arrayPathEq y1 y2)

let pubPathEq (PubPath path1) (PubPath path2) = arrayPathEq path1 path2

let fslibRefEq (nlr1: NonLocalEntityRef) (PubPath path2) = arrayPathEq nlr1.Path path2
...
let fslibValRefEq fslibCcu vref1 vref2 =
    ...
    | VRefLocal e1, VRefLocal e2 ->
        match e1.PublicPath, e2.PublicPath with
        | Some(ValPubPath(pp1, nm1)), Some(ValPubPath(pp2, nm2)) -> pubPathEq pp1 pp2 && (nm1 = nm2)
        | _ -> false
    | _ -> false
...
let primUnionCaseRefEq compilingFSharpCore fslibCcu (UnionCaseRef(tcr1, c1) as uc1) (UnionCaseRef(tcr2, c2) as uc2) =
    uc1 === uc2
    || (primEntityRefEq compilingFSharpCore fslibCcu tcr1 tcr2 && c1 = c2)
...
let inline canAccessCompPathFrom (CompPath(scoref1, _, cpath1)) (CompPath(scoref2, _, cpath2)) =
    ...
    loop cpath1 cpath2 && (scoref1 = scoref2)
...
let isLessAccessible (TAccess aa) (TAccess bb) =
    not (
        aa
        |> List.forall (fun a -> bb |> List.exists (fun b -> canAccessCompPathFrom a b))
    )

/// Given (newPath, oldPath) replace oldPath by newPath in the TAccess.
let accessSubstPaths (newPath, oldPath) (TAccess paths) =
    let subst cpath =
        if cpath = oldPath then newPath else cpath

    TAccess(List.map subst paths)

let compPathOfCcu (ccu: CcuThunk) =
    CompPath(ccu.ILScopeRef, SyntaxAccess.Unknown, [])
...
let taccessPrivate accessPath =
    let (CompPath(sc, _, paths)) = accessPath in TAccess [ CompPath(sc, TypedTree.SyntaxAccess.Private, paths) ]
...
let combineAccess access1 access2 =
    ...
    let combined =
        if access1 = taccessPublic then
            updateSyntaxAccessForCompPath (a1 @ a2) TypedTree.SyntaxAccess.Public
        elif access1 = taccessInternal then
            updateSyntaxAccessForCompPath (a1 @ a2) TypedTree.SyntaxAccess.Internal
        else
            (a1 @ a2)
