...
let mkLet ((v, e), b) =
    CombExpr(LetOp, [], [ e; mkLambda (v, b) ]) (* nb. order preserves source order *)
...
let mkRecdGet (d1, d2, tyargs, args) =
    CombExpr(RecdGetOp(d1, d2), tyargs, args)

let mkRecdSet (d1, d2, tyargs, args) =
    CombExpr(RecdSetOp(d1, d2), tyargs, args)

let mkUnion (d1, d2, tyargs, args) = CombExpr(SumMkOp(d1, d2), tyargs, args)

let mkUnionFieldGet (d1, d2, d3, tyargs, arg) =
    CombExpr(SumFieldGetOp(d1, d2, d3), tyargs, [ arg ])

let mkUnionCaseTagTest (d1, d2, tyargs, arg) =
    CombExpr(SumTagTestOp(d1, d2), tyargs, [ arg ])
...
let mkTryWith (e1, vf, ef, vh, eh) =
    CombExpr(TryWithOp, [], [ e1; mkLambda (vf, ef); mkLambda (vh, eh) ])
...
let mkFieldGet (d1, d2, tyargs, args) =
    CombExpr(FieldGetOp(d1, d2), tyargs, args)

let mkFieldSet (d1, d2, tyargs, args) =
    CombExpr(FieldSetOp(d1, d2), tyargs, args)
...
let mkMethodCallW (d1, d2, d3, tyargs, args) =
    CombExpr(MethodCallWOp(d1, d2, d3), tyargs, args)

let mkAttributedExpression (e, attr) = AttrExpr(e, [ attr ])

let isAttributedExpression e =
    match e with
    | AttrExpr _ -> true
    | _ -> false
...
module SimplePickle =
    ...
    type Table<'T when 'T: not null> =
        ...
        member tbl.FindOrAdd x =
            if tbl.tbl.ContainsKey x then tbl.tbl[x] else tbl.Add x
    ...
    let inline p_tup2 p1 p2 (a, b) (st: QuotationPickleOutState) =
        (p1 a st: unit)
        (p2 b st: unit)

    let inline p_tup3 p1 p2 p3 (a, b, c) st =
        (p1 a st: unit)
        (p2 b st: unit)
        (p3 c st: unit)

    let inline p_tup4 p1 p2 p3 p4 (a, b, c, d) st =
        (p1 a st: unit)
        (p2 b st: unit)
        (p3 c st: unit)
        (p4 d st: unit)

    let inline p_tup5 p1 p2 p3 p4 p5 (a, b, c, d, e) st =
        (p1 a st: unit)
        (p2 b st: unit)
        (p3 c st: unit)
        (p4 d st: unit)
...
let rec p_type x st =
    ...
    | AppType(c, ts) ->
        p_byte 1 st
        p_tup2 p_tycon p_types (c, ts) st

and p_types x st = p_list p_type x st

let p_varDecl (v: ValData) st =
    p_tup3 p_string p_type p_bool (v.Name, v.Type, v.IsMutable) st
...
let p_CombOp x st =
    ...
    | RecdGetOp(x, y) ->
        p_byte 4 st
        p_recdFieldSpec (x, y) st
    | SumMkOp(x, y) ->
        p_byte 5 st
        p_ucaseSpec (x, y) st
    | SumFieldGetOp(a, b, c) ->
        p_byte 6 st
        p_tup2 p_ucaseSpec p_int ((a, b), c) st
    | SumTagTestOp(x, y) ->
        p_byte 7 st
        p_ucaseSpec (x, y) st
    ...
    | FieldGetOp(a, b) ->
        p_byte 37 st
        p_tup2 p_NamedType p_string (a, b) st
...
let rec p_expr x st =
    ...
    | CombExpr(c, ts, args) ->
        p_byte 0 st
        p_tup3 p_CombOp p_types (p_list p_expr) (c, ts, args) st
    | VarExpr v ->
        p_byte 1 st
        p_int v st
    | LambdaExpr(v, e) ->
        p_byte 2 st
        p_tup2 p_varDecl p_expr (v, e) st
    | HoleExpr(ty, idx) ->
        p_byte 3 st
        p_type ty st
        p_int idx st
    ...
    | AttrExpr(e, attrs) ->
        p_byte 5 st
        p_tup2 p_expr (p_list p_expr) (e, attrs) st
    | ThisVarExpr(ty) ->
        p_byte 6 st
        p_type ty st
    | QuoteRawExpr(tm) ->
