...
and typeAEquivAux erasureFlag g aenv ty1 ty2 =
    ...
    | TType_tuple(tupInfo1, l1), TType_tuple(tupInfo2, l2) -> structnessAEquiv tupInfo1 tupInfo2 && typesAEquivAux erasureFlag g aenv l1 l2
    ...
    | TType_anon(anonInfo1, l1), TType_anon(anonInfo2, l2) -> anonInfoEquiv anonInfo1 anonInfo2 && typesAEquivAux erasureFlag g aenv l1 l2
...
let mkArrayElemAddress g (readonly, ilInstrReadOnlyAnnotation, isNativePtr, shape, elemTy, exprs, m) =
    Expr.Op(
        TOp.ILAsm([ I_ldelema(ilInstrReadOnlyAnnotation, isNativePtr, shape, mkILTyvarTy 0us) ], [ mkByrefTyWithFlag g readonly elemTy ]),
        [ elemTy ],
        exprs,
        m
    )
...
let CollectTyparsAndLocalsWithStackGuard () =
    let stackGuard =
        StackGuard(AccFreeVarsStackGuardDepth, "AccFreeVarsStackGuardDepth")
...
module PrettyTypes =
    ...
    let PrettifyInstAndSig g x =
        PrettifyThings g (fun f -> foldTriple (foldTyparInst f, List.fold f, f)) (fun f -> mapTriple (mapTyparInst g f, List.map f, f)) x
...
let TryFindFSharpAttributeOpt g tref attrs =
    match tref with
    | None -> None
    | Some tref -> List.tryFind (IsMatchingFSharpAttribute g tref) attrs

let HasFSharpAttributeOpt g trefOpt attrs =
    match trefOpt with
    | Some tref -> List.exists (IsMatchingFSharpAttribute g tref) attrs
    | _ -> false

let IsMatchingFSharpAttributeOpt g attrOpt (Attrib(tcref2, _, _, _, _, _, _)) =
    match attrOpt with
    | Some(AttribInfo(_, tcref)) -> tyconRefEq g tcref tcref2
    | _ -> false
...
let TryFindFSharpBoolAttribute g nm attrs =
    TryFindFSharpBoolAttributeWithDefault true g nm attrs

let TryFindFSharpBoolAttributeAssumeFalse g nm attrs =
    TryFindFSharpBoolAttributeWithDefault false g nm attrs
...
let TryBindTyconRefAttribute
    ...
    | ProvidedTypeMetadata info ->
        ...
        match
            provAttribs.PUntaint((fun a -> a.GetAttributeConstructorArgs(provAttribs.TypeProvider.PUntaintNoFailure id, atref.FullName)), m)
        with
        | Some args -> f3 args
        | None -> None
#endif
...
module DebugPrint =
        ...
        let str =
            ...
            | Const.Single d ->
                (let s = d.ToString("g12", System.Globalization.CultureInfo.InvariantCulture)
                 ...
                 if String.forall (fun c -> System.Char.IsDigit c || c = '-') s then
                     s + ".0"
                 else
                     s)
                + "f"
    ...
    and exprWrapL isAtomic expr =
        ...
        let lay =
            ...
            | Expr.Op(TOp.TryFinally _, _tyargs, args, _) ->
                wordL (tagText ("unexpected-try-finally")) ---- aboveListL (List.map atomL args)
...
and accFreeInTarget opts (TTarget(vs, expr, flags)) acc =
    ...
    | Some xs ->
        List.foldBack2 (fun v isStateVar acc -> if isStateVar then acc else boundLocalVal opts v acc) vs xs (accFreeInExpr opts expr acc)
...
and remapArgData ctxt tmenv (argInfo: ArgReprInfo) : ArgReprInfo =
    {
        Attribs = remapAttribs ctxt tmenv argInfo.Attribs
        Name = argInfo.Name
...
let remapAttrib g tmenv attrib =
    let ctxt =
        {
            g = g
            stackGuard = StackGuard(RemapExprStackGuardDepth, "RemapExprStackGuardDepth")
        }
...
let tnameCompilationSourceNameAttr =
    FSharpLib.Core + ".CompilationSourceNameAttribute"

let tnameCompilationArgumentCountsAttr =
    FSharpLib.Core + ".CompilationArgumentCountsAttribute"
...
let tname_SignatureDataVersionAttr =
    FSharpLib.Core + ".FSharpInterfaceDataVersionAttribute"
...
let TyconHasUseNullAsTrueValueAttribute g (tycon: Tycon) =
    HasUseNullAsTrueValueAttribute g tycon.Attribs
...
let TypeNullIsExtraValue g m ty =
    if isILReferenceTy g ty || isDelegateTy g ty then
        ...
        | ValueSome tcref ->
            ...
            TryFindTyconRefBoolAttribute g m g.attrib_AllowNullLiteralAttribute tcref
            <> Some false
        | _ ->
    ...
    || (isStructTy g ty
        ...
        (if isFSharpStructTy g ty then
             ...
             let flds =
                 ...
                 |> List.filter (fun fld ->
                     TryFindFSharpBoolAttribute g g.attrib_DefaultValueAttribute fld.FieldAttribs
                     <> Some false)
...
let CompileAsEvent g attrs =
    HasFSharpAttribute g g.attrib_CLIEventAttribute attrs
...
and rewriteLinearExpr env expr contf =
    ...
    | None ->
        ...
        | Expr.DebugPoint(dpm, innerExpr) -> rewriteLinearExpr env innerExpr (contf << (fun innerExprR -> Expr.DebugPoint(dpm, innerExprR)))
...
let (|IntegralRange|_|) g expr =
    ...
    | ValApp g g.range_char_op_vref ([], [ start; finish ], _) ->
        ValueSome(g.char_ty, (start, Expr.Const(Const.Char '\001', Text.Range.range0, g.char_ty), finish))
    | ValApp g g.range_op_vref (ty :: _, [ start; finish ], _) when isIntegerTy g ty || typeEquivAux EraseMeasures g ty g.char_ty ->
        ValueSome(ty, (start, mkTypedOne g Text.Range.range0 ty, finish))
...
let (|ConstCount|_|) (start, step, finish) =
    ...
    | Expr.Const(value = Const.Byte start), Expr.Const(value = Const.Byte step), Expr.Const(value = Const.Byte finish) when start <= finish ->
    ...
    | Expr.Const(value = Const.Char start), Expr.Const(value = Const.Char step), Expr.Const(value = Const.Char finish) when start <= finish ->
        ValueSome(Const.UInt32(uint (finish - start) / uint step + 1u))
...
let tryAddExtensionAttributeIfNotAlreadyPresentForModule
    ...
    if Option.isSome (tryFindExtensionAttribute g moduleEntity.Attribs) then
        ...
        | Some extensionAttrib ->
            { moduleEntity with
                entity_attribs = extensionAttrib :: moduleEntity.Attribs
            }
...
let tryAddExtensionAttributeIfNotAlreadyPresentForType
    ...
    if Option.isSome (tryFindExtensionAttribute g typeEntity.Attribs) then
        ...
        | Some extensionAttrib ->
            ...
            |> Option.iter (fun e -> e.entity_attribs <- extensionAttrib :: e.Attribs)
