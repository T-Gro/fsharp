...
type NiceNameGenerator() =
    let basicNameCounts =
        ConcurrentDictionary<string, int ref>(max Environment.ProcessorCount 1, 127)
...
type StableNiceNameGenerator() =
    ...
    let niceNames =
        ConcurrentDictionary<string * int64, string>(max Environment.ProcessorCount 1, 127)
...
let newUnique () =
    System.Threading.Interlocked.Increment &uniqueCount
