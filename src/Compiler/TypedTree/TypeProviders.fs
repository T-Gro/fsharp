...
let toolingCompatiblePaths () =
    Internal.Utilities.FSharpEnvironment.toolingCompatiblePaths ()
...
let GetTypeProviderImplementationTypes (runTimeAssemblyFileName, designTimeAssemblyNameString, m: range, compilerToolPaths: string list) =
    ...
    let raiseError designTimeAssemblyPathOpt (e: exn) =
        ...
        | None ->
            let msg =
                FSComp.SR.etProviderHasWrongDesignerAssemblyNoPath (attrName, designTimeAssemblyNameString, exnTypeName, exnMsg)

            raise (TypeProviderError(msg, runTimeAssemblyFileName, m))
        | Some designTimeAssemblyPath ->
            let msg =
                FSComp.SR.etProviderHasWrongDesignerAssembly (
                    attrName,
                    designTimeAssemblyNameString,
                    designTimeAssemblyPath,
                    exnTypeName,
                    exnMsg
                )
    ...
    let designTimeAssemblyOpt =
        getTypeProviderAssembly (runTimeAssemblyFileName, designTimeAssemblyNameString, compilerToolPaths, raiseError)
    ...
    | Some loadedDesignTimeAssembly ->
        ...
        with e ->
            ...
            | :? FileLoadException ->
                let msg =
                    FSComp.SR.etProviderHasDesignerAssemblyDependency (designTimeAssemblyNameString, folder, exnTypeName, exnMsg)

                raise (TypeProviderError(msg, runTimeAssemblyFileName, m))

            | _ ->
                let msg =
                    FSComp.SR.etProviderHasDesignerAssemblyException (designTimeAssemblyNameString, folder, exnTypeName, exnMsg)
...
let CreateTypeProvider
    ...
    let protect f =
        ...
        with err ->
            ...
            raise (
                TypeProviderError(FSComp.SR.etTypeProviderConstructorException (e.Message), !!typeProviderImplementationType.FullName, m)
            )
    ...
    let providerSpecs =
        try
            ...
            [
                ...
                | Some designTimeAssemblyName, Some path when
                    String.Compare(designTimeAssemblyName.Name, Path.GetFileNameWithoutExtension path, StringComparison.OrdinalIgnoreCase) = 0
                    ->
                    ()

                | Some _, _ ->
                    let provImplTypes =
                        GetTypeProviderImplementationTypes(runtimeAssemblyFilename, designTimeName, m, compilerToolPaths)
...
let TryTypeMemberArray (st: Tainted<_>, fullName, memberName, m, f) =
    ...
    with :? TypeProviderError as tpe ->
        tpe.Iter(fun e ->
            error (Error(FSComp.SR.etUnexpectedExceptionFromProvidedTypeMember (fullName, memberName, e.ContextualErrorMessage), m)))

        [||]

/// Try to access a member on a provided type, catching and reporting errors and checking the result is non-null,
let TryTypeMemberNonNull<'T, 'U when 'U: not null and 'U: not struct>
    (st: Tainted<'T>, fullName, memberName, m, recover: 'U, (f: 'T -> 'U | null))
    : Tainted<'U> =
...
let TryMemberMember (mi: Tainted<_>, typeName, memberName, memberMemberName, m, recover, f) =
    ...
    with :? TypeProviderError as tpe ->
        tpe.Iter(fun e ->
            errorR (
                Error(
                    FSComp.SR.etUnexpectedExceptionFromProvidedMemberMember (
                        memberMemberName,
                        typeName,
                        memberName,
                        e.ContextualErrorMessage
                    ),
                    m
                )
            ))
...
type ProvidedTypeComparer() =
    ...
    interface IEqualityComparer<ProvidedType> with
        ...
        member _.Equals(ty1: ProvidedType, ty2: ProvidedType) =
            nullSafeEquality ty1 ty2 (fun ty1 ty2 -> key ty1 = key ty2)
...
type ProvidedTypeContext =
    ...
    member ctxt.GetDictionaries() =
        match ctxt with
        | NoEntries ->
            ConcurrentDictionary<ProvidedType, ILTypeRef>(ProvidedTypeComparer.Instance),
            ConcurrentDictionary<ProvidedType, obj>(ProvidedTypeComparer.Instance)
        | Entries(lookupILTR, lookupILTCR) -> lookupILTR, lookupILTCR.Force()
        ...
        | Entries(d1, d2) ->
            Entries(
                ...
                InterruptibleLazy(fun _ ->
                    ...
                    for KeyValue(st, tcref) in d2.Force() do
                        dict.TryAdd(st, f tcref) |> ignore
...
type ProvidedType(x: Type, ctxt: ProvidedTypeContext) =
    ...
    let provide () =
        ProvidedCustomAttributeProvider(fun _ -> x.CustomAttributes) :> IProvidedCustomAttributeProvider
    ...
    member _.IsSuppressRelocate =
        (x.Attributes &&& enum (int32 TypeProviderTypeAttributes.SuppressRelocate))
        <> enum 0

    member _.IsErased =
        (x.Attributes &&& enum (int32 TypeProviderTypeAttributes.IsErased)) <> enum 0
    ...
    member _.GetInterfaces() =
        x.GetInterfaces() |> ProvidedType.CreateArray ctxt

    member _.GetMethods() =
        x.GetMethods bindingFlags |> ProvidedMethodInfo.CreateArray ctxt

    member _.GetEvents() =
        x.GetEvents bindingFlags |> ProvidedEventInfo.CreateArray ctxt

    member _.GetEvent nm =
        x.GetEvent(nm, bindingFlags) |> ProvidedEventInfo.Create ctxt

    member _.GetProperties() =
        x.GetProperties bindingFlags |> ProvidedPropertyInfo.CreateArray ctxt

    member _.GetProperty nm =
        x.GetProperty(nm, bindingFlags) |> ProvidedPropertyInfo.Create ctxt

    member _.GetConstructors() =
        x.GetConstructors bindingFlags |> ProvidedConstructorInfo.CreateArray ctxt

    member _.GetFields() =
        x.GetFields bindingFlags |> ProvidedFieldInfo.CreateArray ctxt

    member _.GetField nm =
        x.GetField(nm, bindingFlags) |> ProvidedFieldInfo.Create ctxt

    member _.GetAllNestedTypes() =
        x.GetNestedTypes(bindingFlags ||| BindingFlags.NonPublic)
        |> ProvidedType.CreateArray ctxt

    member _.GetNestedTypes() =
        x.GetNestedTypes bindingFlags |> ProvidedType.CreateArray ctxt

    /// Type.GetNestedType(string) can return null if there is no nested type with given name
    member _.GetNestedType nm =
        x.GetNestedType(nm, bindingFlags) |> ProvidedType.Create ctxt

    /// Type.GetGenericTypeDefinition() either returns type or throws exception, null is not permitted
    member _.GetGenericTypeDefinition() =
        x.GetGenericTypeDefinition()
        |> ProvidedType.CreateWithNullCheck ctxt "GenericTypeDefinition"

    /// Type.BaseType can be null when Type is interface or object
    member _.BaseType = x.BaseType |> ProvidedType.Create ctxt

    member _.GetStaticParameters(provider: ITypeProvider) : ProvidedParameterInfo ProvidedArray =
        provider.GetStaticParameters x |> ProvidedParameterInfo.CreateArray ctxt

    /// Type.GetElementType can be null if i.e. Type is not array\pointer\byref type
    member _.GetElementType() =
        x.GetElementType() |> ProvidedType.Create ctxt

    member _.GetGenericArguments() =
        x.GetGenericArguments() |> ProvidedType.CreateArray ctxt

    member _.ApplyStaticArguments(provider: ITypeProvider, fullTypePathAfterArguments, staticArgs: objnull[]) =
        provider.ApplyStaticArguments(x, fullTypePathAfterArguments, staticArgs)
        |> ProvidedType.Create ctxt

    member _.IsVoid =
        (Type.op_Equality (x, typeof<Void>)
         || (x.Namespace = "System" && x.Name = "Void"))
    ...
    member _.MakePointerType() =
        ProvidedType.CreateNoContext(x.MakePointerType())

    member _.MakeByRefType() =
        ProvidedType.CreateNoContext(x.MakeByRefType())

    member _.MakeArrayType() =
        ProvidedType.CreateNoContext(x.MakeArrayType())

    member _.MakeArrayType rank =
        ProvidedType.CreateNoContext(x.MakeArrayType(rank))
    ...
    member _.AsProvidedVar name =
        ProvidedVar.CreateNonNull ctxt (Quotations.Var(name, x))
    ...
    static member CreateNoContext(x: Type) =
        ProvidedType.CreateNonNull ProvidedTypeContext.Empty x
    ...
    override _.Equals y =
        ...
        match y with
        | :? ProvidedType as y -> x.Equals y.Handle
        | _ -> false
...
type ProvidedCustomAttributeProvider(attributes: ITypeProvider -> seq<CustomAttributeData>) =
    ...
    [<return: Struct>]
    let (|Member|_|) (s: string) (x: CustomAttributeNamedArgument) =
        if x.MemberName = s then
            ValueSome x.TypedValue
        else
            ValueNone

    [<return: Struct>]
    let (|Arg|_|) (x: CustomAttributeTypedArgument) =
        match x.Value with
        | null -> ValueNone
        | v -> ValueSome v

    let findAttribByName tyFullName (a: CustomAttributeData) =
        ((!!a.Constructor.DeclaringType).FullName = tyFullName)
    ...
    interface IProvidedCustomAttributeProvider with
        ...
        member _.GetDefinitionLocationAttribute provider =
            ...
            |> Option.map (fun a ->
                let filePath: string MaybeNull =
                    defaultArg
                        (a.NamedArguments
                         |> Seq.tryPick (function
                             | Member "FilePath" (Arg(:? string as v)) -> Some v
                             | _ -> None))
                        null
                ...
                let line =
                    defaultArg
                        (a.NamedArguments
                         |> Seq.tryPick (function
                             | Member "Line" (Arg(:? int as v)) -> Some v
                             | _ -> None))
                        0
                ...
                let column =
                    defaultArg
                        (a.NamedArguments
                         |> Seq.tryPick (function
                             | Member "Column" (Arg(:? int as v)) -> Some v
                             | _ -> None))
                        0
...
type ProvidedParameterInfo(x: ParameterInfo, ctxt) =
    ...
    member _.ParameterType =
        ProvidedType.CreateWithNullCheck ctxt "ParameterType" x.ParameterType
    ...
    override _.Equals y =
        ...
        match y with
        | :? ProvidedParameterInfo as y -> x.Equals y.Handle
        | _ -> false
...
type ProvidedAssembly(x: Assembly) =
    ...
    static member Create(x: Assembly MaybeNull) : ProvidedAssembly MaybeNull =
        match x with
        | null -> null
        | t -> ProvidedAssembly(t)
    ...
    override _.Equals y =
        ...
        match y with
        | :? ProvidedAssembly as y -> x.Equals y.Handle
        | _ -> false
...
type ProvidedMethodBase(x: MethodBase, ctxt) =
    ...
    member _.GetParameters() =
        x.GetParameters() |> ProvidedParameterInfo.CreateArray ctxt
    ...
    static member TaintedGetHashCode(x: Tainted<ProvidedMethodBase>) =
        Tainted.GetHashCodeTainted(
            x.PApplyNoFailure(fun st ->
                (st.Name,
                 (nonNull<ProvidedAssembly> (nonNull<ProvidedType> st.DeclaringType).Assembly).FullName,
    ...
    member _.GetStaticParametersForMethod(provider: ITypeProvider) : ProvidedParameterInfo ProvidedArray =
        ...
        let staticParams =
            match provider with
            | :? ITypeProvider2 as itp2 -> itp2.GetStaticParametersForMethod x
            | _ ->
                ...
                let meth =
                    provider.GetType().GetMethod("GetStaticParametersForMethod", bindingFlags, null, [| typeof<MethodBase> |], null)
                ...
                if isNull meth then
                    ...
                    let paramsAsObj =
                        try
                            (!!meth).Invoke(provider, bindingFlags ||| BindingFlags.InvokeMethod, null, [| box x |], null)
                        with err ->
                            raise (StripException(StripException err))
    ...
    member _.ApplyStaticArgumentsForMethod(provider: ITypeProvider, fullNameAfterArguments: string, staticArgs: objnull[]) =
        ...
        let mb =
            match provider with
            | :? ITypeProvider2 as itp2 -> itp2.ApplyStaticArgumentsForMethod(x, fullNameAfterArguments, staticArgs)
            | _ ->
                ...
                let meth =
                    provider
                        ...
                        .GetMethod(
                            ...
                            bindingFlags,
                            null,
                            [| typeof<MethodBase>; typeof<string>; typeof<obj[]> |],
                            null
                        )
                    ...
                    let mbAsObj =
                        try
                            meth.Invoke(
                                ...
                                null,
                                [| box x; box fullNameAfterArguments; box staticArgs |],
                                null
                            )
                        with err ->
                            raise (StripException(StripException err))
...
type ProvidedFieldInfo(x: FieldInfo, ctxt) =
    ...
    override _.Equals y =
        ...
        match y with
        | :? ProvidedFieldInfo as y -> x.Equals y.Handle
        | _ -> false
...
type ProvidedMethodInfo(x: MethodInfo, ctxt) =
    ...
    member _.ReturnType =
        x.ReturnType |> ProvidedType.CreateWithNullCheck ctxt "ReturnType"

    static member CreateNonNull ctxt (x: MethodInfo) : ProvidedMethodInfo = ProvidedMethodInfo(x, ctxt)
    ...
    override _.Equals y =
        ...
        match y with
        | :? ProvidedMethodInfo as y -> x.Equals y.Handle
        | _ -> false
...
type ProvidedPropertyInfo(x: PropertyInfo, ctxt) =
    ...
    member _.GetGetMethod() =
        x.GetGetMethod() |> ProvidedMethodInfo.Create ctxt

    member _.GetSetMethod() =
        x.GetSetMethod() |> ProvidedMethodInfo.Create ctxt

    member _.CanRead = x.CanRead

    member _.CanWrite = x.CanWrite

    member _.GetIndexParameters() =
        x.GetIndexParameters() |> ProvidedParameterInfo.CreateArray ctxt

    /// PropertyInfo.PropertyType cannot be null
    member _.PropertyType =
        x.PropertyType |> ProvidedType.CreateWithNullCheck ctxt "PropertyType"
    ...
    override _.Equals y =
        ...
        match y with
        | :? ProvidedPropertyInfo as y -> x.Equals y.Handle
        | _ -> false
...
type ProvidedEventInfo(x: EventInfo, ctxt) =
    ...
    member _.GetAddMethod() =
        x.GetAddMethod() |> ProvidedMethodInfo.Create ctxt

    member _.GetRemoveMethod() =
        x.GetRemoveMethod() |> ProvidedMethodInfo.Create ctxt

    /// EventInfo.EventHandlerType cannot be null
    member _.EventHandlerType =
        x.EventHandlerType |> ProvidedType.CreateWithNullCheck ctxt "EventHandlerType"
    ...
    override _.Equals y =
        ...
        match y with
        | :? ProvidedEventInfo as y -> x.Equals y.Handle
        | _ -> false
...
type ProvidedConstructorInfo(x: ConstructorInfo, ctxt) =
    ...
    override _.Equals y =
        ...
        match y with
        | :? ProvidedConstructorInfo as y -> x.Equals y.Handle
        | _ -> false
...
type ProvidedExpr(x: Expr, ctxt) =
    ...
    member _.GetExprType() =
        ...
        | Patterns.NewObject(ctor, args) ->
            Some(
                ProvidedNewObjectExpr(
                    ProvidedConstructorInfo.CreateNonNull ctxt ctor,
                    [| for a in args -> ProvidedExpr.CreateNonNull ctxt a |]
                )
            )
        ...
        | Patterns.NewDelegate(ty, vs, expr) ->
            Some(
                ProvidedNewDelegateExpr(
                    ProvidedType.CreateNonNull ctxt ty,
                    ProvidedVar.CreateArray ctxt (List.toArray vs),
                    ProvidedExpr.CreateNonNull ctxt expr
                )
            )
        | Patterns.Call(objOpt, meth, args) ->
            Some(
                ProvidedCallExpr(
                    (match objOpt with
                     | None -> None
                     | Some obj -> Some(ProvidedExpr.CreateNonNull ctxt obj)),
                    ProvidedMethodInfo.CreateNonNull ctxt meth,
                    [| for a in args -> ProvidedExpr.CreateNonNull ctxt a |]
                )
            )
        | Patterns.DefaultValue ty -> Some(ProvidedDefaultExpr(ProvidedType.CreateNonNull ctxt ty))
        | Patterns.Value(obj, ty) -> Some(ProvidedConstantExpr(obj, ProvidedType.CreateNonNull ctxt ty))
        | Patterns.Coerce(arg, ty) -> Some(ProvidedTypeAsExpr(ProvidedExpr.CreateNonNull ctxt arg, ProvidedType.CreateNonNull ctxt ty))
        | Patterns.NewTuple args -> Some(ProvidedNewTupleExpr(ProvidedExpr.CreateArray ctxt (Array.ofList args)))
        | Patterns.TupleGet(arg, n) -> Some(ProvidedTupleGetExpr(ProvidedExpr.CreateNonNull ctxt arg, n))
        ...
        | Patterns.Lambda(v, body) -> Some(ProvidedLambdaExpr(ProvidedVar.CreateNonNull ctxt v, ProvidedExpr.CreateNonNull ctxt body))
        ...
        | Patterns.TryWith(b, v1, e1, v2, e2) ->
            Some(
                ProvidedTryWithExpr(
                    ProvidedExpr.CreateNonNull ctxt b,
                    ProvidedVar.CreateNonNull ctxt v1,
                    ProvidedExpr.CreateNonNull ctxt e1,
                    ProvidedVar.CreateNonNull ctxt v2,
                    ProvidedExpr.CreateNonNull ctxt e2
                )
            )
        | Patterns.TypeTest(e, ty) -> Some(ProvidedTypeTestExpr(ProvidedExpr.CreateNonNull ctxt e, ProvidedType.CreateNonNull ctxt ty))
        | Patterns.Let(v, e, b) ->
        ...
        | Patterns.ForIntegerRangeLoop(v, e1, e2, e3) ->
            Some(
                ProvidedForIntegerRangeLoopExpr(
                    ProvidedVar.CreateNonNull ctxt v,
                    ProvidedExpr.CreateNonNull ctxt e1,
                    ProvidedExpr.CreateNonNull ctxt e2,
                    ProvidedExpr.CreateNonNull ctxt e3
                )
            )
        | Patterns.VarSet(v, e) -> Some(ProvidedVarSetExpr(ProvidedVar.CreateNonNull ctxt v, ProvidedExpr.CreateNonNull ctxt e))
        | Patterns.IfThenElse(g, t, e) ->
            Some(
                ProvidedIfThenElseExpr(
                    ProvidedExpr.CreateNonNull ctxt g,
                    ProvidedExpr.CreateNonNull ctxt t,
                    ProvidedExpr.CreateNonNull ctxt e
                )
            )
        | Patterns.Var v -> Some(ProvidedVarExpr(ProvidedVar.CreateNonNull ctxt v))
        | _ -> None
    ...
    static member CreateNonNull ctxt t : ProvidedExpr = ProvidedExpr(t, ctxt)
    ...
    override _.Equals y =
        match y with
        | :? ProvidedExpr as y -> x.Equals y.Handle
        | _ -> false
...
type ProvidedVar(x: Var, ctxt) =
    ...
    static member CreateNonNull ctxt t = ProvidedVar(t, ctxt)
    ...
    override _.Equals y =
        match y with
        | :? ProvidedVar as y -> x.Equals y.Handle
        | _ -> false
...
let GetInvokerExpression (provider: ITypeProvider, methodBase: ProvidedMethodBase, paramExprs: ProvidedVar[]) =
    provider.GetInvokerExpression(methodBase.Handle, [| for p in paramExprs -> Quotations.Expr.Var p.Handle |])
    |> ProvidedExpr.Create methodBase.Context
...
let CheckAndComputeProvidedNameProperty (m, st: Tainted<ProvidedType>, proj, propertyString) =
    let name: string MaybeNull =
        ...
        with :? TypeProviderError as tpe ->
            let newError =
                tpe.MapText((fun msg -> FSComp.SR.etProvidedTypeWithNameException (propertyString, msg)), st.TypeProviderDesignation, m)
...
let ValidateAttributesOfProvidedType (m, st: Tainted<ProvidedType>) =
    let fullName =
        CheckAndComputeProvidedNameProperty(m, st, (fun st -> st.FullName), "FullName")

    if
        TryTypeMember(st, fullName, "IsGenericType", m, false, fun st -> st.IsGenericType)
        |> unmarshal
    then
        errorR (Error(FSComp.SR.etMustNotBeGeneric fullName, m))

    if
        TryTypeMember(st, fullName, "IsArray", m, false, fun st -> st.IsArray)
        |> unmarshal
    then
        errorR (Error(FSComp.SR.etMustNotBeAnArray fullName, m))

    TryTypeMemberNonNull<ProvidedType, ProvidedType[]>(st, fullName, "GetInterfaces", m, [||], fun st -> st.GetInterfaces())
    |> ignore
...
let ValidateExpectedName m expectedPath expectedName (st: Tainted<ProvidedType>) =
    ...
    let namespaceName =
        TryTypeMember(st, name, "Namespace", m, ("": _ | null), fun st -> st.Namespace)
        |> unmarshal
...
let ValidateProvidedTypeAfterStaticInstantiation (m, st: Tainted<ProvidedType>, expectedPath: string[], expectedName: string) =
    ...
    let fullName, namespaceName, usedMembers =
        ...
        let namespaceName =
            TryTypeMember<_, string | null>(st, name, "Namespace", m, FSComp.SR.invalidNamespaceForProvidedType (), fun st -> st.Namespace)
            |> unmarshal

        let fullName =
            TryTypeMemberNonNull(st, name, "FullName", m, FSComp.SR.invalidFullNameForProvidedType (), fun st -> st.FullName)
            |> unmarshal
    ...
    for mi in usedMembers do
        ...
        | Tainted.NonNull _ ->
            let memberName =
                TryMemberMember(mi, fullName, "Name", "Name", m, "invalid provided type member name", fun mi -> mi.Name)
                |> unmarshal
            ...
            if String.IsNullOrEmpty memberName then
                ...
                let miDeclaringType =
                    TryMemberMember(
                        ...
                        "DeclaringType",
                        m,
                        (ProvidedType.CreateNoContext(typeof<obj>) |> withNull),
                        fun mi -> mi.DeclaringType
                    )
                ...
                | Tainted.Null -> errorR (Error(FSComp.SR.etNullMemberDeclaringType (fullName, memberName), m))
                | Tainted.NonNull miDeclaringType ->
                    ...
                    if not (ProvidedType.TaintedEquals(st, miDeclaringType)) then
                        errorR (
                            Error(
                                FSComp.SR.etNullMemberDeclaringTypeDifferentFromProvidedType (fullName, memberName, miDeclaringTypeFullName),
                                m
                            )
                        )
                ...
                | Some mi ->
                    let isPublic =
                        TryMemberMember(mi, fullName, memberName, "IsPublic", m, true, fun mi -> mi.IsPublic)
                        |> unmarshal

                    let isGenericMethod =
                        TryMemberMember(mi, fullName, memberName, "IsGenericMethod", m, true, fun mi -> mi.IsGenericMethod)
                        |> unmarshal
                ...
                | None ->
                    ...
                    | None ->
                        ...
                        | Some pi ->
                            ...
                            let canRead =
                                TryMemberMember(pi, fullName, memberName, "CanRead", m, expectRead, fun pi -> pi.CanRead)
                                |> unmarshal

                            let canWrite =
                                TryMemberMember(pi, fullName, memberName, "CanWrite", m, expectWrite, fun pi -> pi.CanWrite)
                                |> unmarshal
                        ...
                        | None ->
                            ...
                            | Some ei ->
                                ...
                                let adder =
                                    TryMemberMember(ei, fullName, memberName, "GetAddMethod", m, null, fun ei -> ei.GetAddMethod())

                                let remover =
                                    TryMemberMember(ei, fullName, memberName, "GetRemoveMethod", m, null, fun ei -> ei.GetRemoveMethod())
                            ...
                            | None ->
                                ...
                                | None ->
                                    ...
                                    | None -> errorR (Error(FSComp.SR.etUnsupportedMemberKind (memberName, fullName), m))
...
let ValidateProvidedTypeDefinition (m, st: Tainted<ProvidedType>, expectedPath: string[], expectedName: string) =
    ...
    let staticParameters =
        st.PApplyWithProvider((fun (st, provider) -> st.GetStaticParameters provider), range = m)
...
let ResolveProvidedType (resolver: Tainted<ITypeProvider>, m, moduleOrNamespace: string[], typeName) : Tainted<ProvidedType MaybeNull> =
    ...
    let rec tryNamespace (providedNamespace: Tainted<IProvidedNamespace>) =
        ...
        let providedNamespaceName =
            providedNamespace.PUntaint((fun providedNamespace -> providedNamespace.NamespaceName), range = m)
        ...
        if displayName = providedNamespaceName then
            let resolvedType =
                providedNamespace.PApply(
                    (fun providedNamespace -> ProvidedType.Create ProvidedTypeContext.Empty (providedNamespace.ResolveTypeName typeName)),
                    range = m
                )
            ...
            let providedNamespaces =
                providedNamespace.PApplyArray(
                    (fun providedNamespace -> providedNamespace.GetNestedNamespaces()),
                    "GetNestedNamespaces",
                    range = m
                )
    ...
    let providedNamespaces =
        resolver.PApplyArray((fun resolver -> resolver.GetNamespaces()), "GetNamespaces", range = m)
    ...
    let defaultArgValues =
        staticParams.PApply(
            (fun ps ->
                ...
                |> Array.map (fun sp ->
                    sp.Name,
                    (if sp.IsOptional then
                         Some(string sp.RawDefaultValue)
                     else
                         None))),
            range = m
        )
...
let TryApplyProvidedMethod (methBeforeArgs: Tainted<ProvidedMethodBase>, staticArgs: objnull[], m: range) =
    if staticArgs.Length = 0 then
        ...
        let mangledName =
            ...
            let mangledName =
                ComputeMangledNameForApplyStaticParameters(nm, staticArgs, staticParams, m)

            mangledName

        match
            methBeforeArgs.PApplyWithProvider(
                (fun (mb, provider) -> mb.ApplyStaticArgumentsForMethod(provider, mangledName, staticArgs)),
                range = m
            )
        with
        | Tainted.Null -> None
        | Tainted.NonNull methWithArguments ->
            ...
            if actualName <> mangledName then
                error (
                    Error(
                        FSComp.SR.etProvidedAppliedMethodHadWrongName (methWithArguments.TypeProviderDesignation, mangledName, actualName),
                        m
                    )
                )
...
let TryApplyProvidedType
    (typeBeforeArguments: Tainted<ProvidedType>, optGeneratedTypePath: string list option, staticArgs: objnull[], m: range)
    =
    if staticArgs.Length = 0 then
        ...
        let fullTypePathAfterArguments =
            ...
            | None ->
                ...
                let staticParams =
                    typeBeforeArguments.PApplyWithProvider((fun (st, resolver) -> st.GetStaticParameters resolver |> nonNull), range = m)
        ...
        match
            typeBeforeArguments.PApplyWithProvider(
                (fun (typeBeforeArguments, provider) ->
                    typeBeforeArguments.ApplyStaticArguments(provider, Array.ofList fullTypePathAfterArguments, staticArgs)),
                range = m
            )
        with
        | Tainted.Null -> None
        | Tainted.NonNull typeWithArguments ->
            ...
            let checkTypeName () =
                let expectedTypeNameAfterArguments =
                    fullTypePathAfterArguments[fullTypePathAfterArguments.Length - 1]

                if actualName <> expectedTypeNameAfterArguments then
                    error (
                        Error(
                            FSComp.SR.etProvidedAppliedTypeHadWrongName (
                                typeWithArguments.TypeProviderDesignation,
                                expectedTypeNameAfterArguments,
                                actualName
                            ),
                            m
                        )
...
let TryLinkProvidedType (resolver: Tainted<ITypeProvider>, moduleOrNamespace: string[], typeLogicalName: string, range: range) =
    ...
    let typeBeforeArguments =
        ResolveProvidedType(resolver, range0, moduleOrNamespace, typeName)
    ...
    | Tainted.NonNull typeBeforeArguments ->
        ...
        let staticParameters =
            typeBeforeArguments.PApplyWithProvider(
                (fun (typeBeforeArguments, resolver) -> typeBeforeArguments.GetStaticParameters resolver),
                range = range0
            )
        ...
        let staticArgs =
            staticParameters
            |> Array.map (fun sp ->
                let typeBeforeArgumentsName =
                    typeBeforeArguments.PUntaint((fun st -> st.Name), range)
                ...
                | _ ->
                    if sp.PUntaint((fun sp -> sp.IsOptional), range) then
                        ...
                        | null ->
                            error (
                                Error(
                                    FSComp.SR.etStaticParameterRequiresAValue (
                                        spName,
                                        typeBeforeArgumentsName,
                                        typeBeforeArgumentsName,
                                        spName
                                    ),
                                    range0
...
let GetFSharpPathToProvidedType (st: Tainted<ProvidedType>, range) =
    ...
    let namespaceParts =
        GetPartsOfNamespaceRecover(st.PUntaint((fun st -> st.Namespace), range))

    let rec walkUpNestedClasses (st: Tainted<ProvidedType MaybeNull>, soFar) =
        match st with
        | Tainted.Null -> soFar
        | Tainted.NonNull st ->
            walkUpNestedClasses (st.PApply((fun st -> st.DeclaringType), range), soFar)
            @ [ st.PUntaint((fun st -> st.Name), range) ]
...
let GetOriginalILTypeRefOfProvidedType (st: Tainted<ProvidedType>, range) =
    ...
    let aref =
        GetOriginalILAssemblyRefOfProvidedAssembly(st.PApply((fun st -> nonNull<ProvidedAssembly> st.Assembly), range), range) // NULLNESS TODO: why is explicit instantiation needed here
...
type ProvidedAssemblyStaticLinkingMap =
    ...
    static member CreateNew() = { ILTypeMap = Dictionary() }
